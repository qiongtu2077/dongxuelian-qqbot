...
9. **Dashboard 部署面板的双路径陷阱。** 部署面板从本地 `packages/` 目录推送代码到远程服务器。如果手工 SCP 修改了远程的 `node_modules/` 但没有同步更新 `packages/`，下次用部署面板推送时会用 `packages/` 的旧代码覆盖掉修复。**任何时候改代码，必须同时更新 `packages/` 和 `node_modules/`**。最稳妥的方式是只在本地 `packages/` 里改，然后用部署面板推送，不动远程的 `node_modules/`。

9. **Dashboard 部署面板的双路径陷阱。** 部署面板从本地 `packages/` 目录推送代码到远程服务器。如果手工 SCP 修改了远程的 `node_modules/` 但没有同步更新 `packages/`，下次用部署面板推送时会用 `packages/` 的旧代码覆盖掉修复。**任何时候改代码，必须同时更新 `packages/` 和 `node_modules/`**。最稳妥的方式是只在本地 `packages/` 里改，然后用部署面板推送，不动远程的 `node_modules/`。

10. **新状态新 IO 就拆模块。** 小于 50 行但引入新 `Map`、文件 IO、定时器、协议、缓存所有权，优先独立模块。超过 80 行的新功能必须先提拆分方案。

11. **遇到未知先收窄，不扩大战场。** 测试或阅读代码时暴露问题，先分类再处理：
