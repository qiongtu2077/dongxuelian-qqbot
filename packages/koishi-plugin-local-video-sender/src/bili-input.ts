...
async function resolvePublicBiliHost(input: string): Promise<{ address: string; family: number }> {
  if (!isAllowedBiliRedirectUrl(input)) throw new Error('short link redirect escaped Bilibili allowlist')
  const hostname = new URL(input).hostname
  const addresses = await dns.lookup(hostname, { all: true, verbatim: true })
  if (!addresses.length || addresses.some(item => isPrivateIpAddress(item.address))) throw new Error('short link redirect resolved to private or invalid address')
  return addresses[0]
}
...
export async function resolveBiliShortLink(input: string, requestRedirect: typeof requestRedirectLocation = requestRedirectLocation): Promise<string> {
