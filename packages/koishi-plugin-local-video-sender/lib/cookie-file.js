...
exports.MAX_BILI_COOKIE_FILE_BYTES = void 0;
exports.resolveBiliCookiePath = resolveBiliCookiePath;
exports.validateNetscapeCookieFile = validateNetscapeCookieFile;
exports.decodeStrictBase64 = decodeStrictBase64;
exports.getBiliCookieHealth = getBiliCookieHealth;
exports.clearBiliCookieHealthCache = clearBiliCookieHealthCache;
exports.replaceBiliCookieFileAtomic = replaceBiliCookieFileAtomic;
/**
...
exports.MAX_BILI_COOKIE_FILE_BYTES = 4 * 1024 * 1024;
const NETSCAPE_COOKIE_HEADER = '# Netscape HTTP Cookie File';
const cookieHealthCache = new Map();
// 将运行数据目录和可选环境变量解析为视频插件与 Dashboard 共用的唯一 Cookie 路径。
function resolveBiliCookiePath(dataDir, envValue = '') {
...
function validateNetscapeCookieFile(buffer) {
    if (!Buffer.isBuffer(buffer) || buffer.length === 0)
        return { ok: false, code: 'empty_file' };
    if (buffer.length > exports.MAX_BILI_COOKIE_FILE_BYTES)
...
function getBiliCookieHealth(filePath) {
    ...
    try {
        ...
        const code = String(error?.code || 'file_unavailable');
    ...
    try {
        ...
        validation = { ok: false, code: String(error?.code || 'read_failed') };
    }
    const health = validation.ok
...
function clearBiliCookieHealthCache() {
    cookieHealthCache.clear();
}
// 删除本次事务的临时文件；清理失败交由原始写入异常继续向上报告。
function removeTemporaryFile(fsApi, filePath) {
    try {
...
function restorePreviousCookieFile(fsApi, target, backup, hadPrevious) {
    if (hadPrevious) {
...
function replaceBiliCookieFileAtomic(filePath, buffer, options = {}) {
    const validation = validateNetscapeCookieFile(buffer);
    if (!validation.ok)
        throw Object.assign(new Error(validation.code), { code: validation.code, line: validation.line });
    ...
    try {
        ...
        if (!temporaryValidation.ok)
            throw Object.assign(new Error(temporaryValidation.code), { code: temporaryValidation.code, line: temporaryValidation.line });
        ...
        if (!finalStat.isFile() || !finalValidation.ok || finalStat.size !== buffer.length)
            throw new Error('cookie_file_post_write_verification_failed');
        const observedMode = finalStat.mode & 0o777;
        if (process.platform !== 'win32' && observedMode !== 0o600)
            throw new Error('cookie_file_mode_verification_failed');
        ...
        if (targetReplaced)
            restorePreviousCookieFile(fsApi, target, backup, hadPrevious);
        throw error;
    }
    finally {
