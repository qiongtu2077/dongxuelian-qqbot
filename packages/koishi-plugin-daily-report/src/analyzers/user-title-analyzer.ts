...
async function analyzeUserTitles(
  ...
  messages: Array<{ userId?: unknown, user?: unknown, content?: unknown }>,
  topMembers: Array<{ userId: string, name: string, msgCount: number }>,
): Promise<{ userTitles: Array<{ name: string, userId: string, title: string, reason: string, mbti: string }>, tokenUsage: { promptTokens: number, completionTokens: number, totalTokens: number } }> {
...
输出JSON格式：
{
  ...
  try {
    ...
    if (jsonMatch) {
      const data = JSON.parse(jsonMatch[0]) as { userTitles?: UserTitlePayload[], tokenUsage?: TokenUsage }
      return {
        userTitles: (data.userTitles || []).map(t => createUserTitle(t.name as string, t.userId as string, t.title as string, t.reason as string, t.mbti as string | undefined)),
        tokenUsage: data.tokenUsage || { promptTokens: 0, completionTokens: 0, totalTokens: 0 },
      }
    }
  } catch (err) {
    console.error('[user-title-analyzer] 分析失败:', getErrorMessage(err))
  }

  return { userTitles: [], tokenUsage: { promptTokens: 0, completionTokens: 0, totalTokens: 0 } }
}

export = { analyzeUserTitles }
