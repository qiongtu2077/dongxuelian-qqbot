...
const inFlightReports = new Map<string | number, number>()
...
function trimRuntimeMaps(now = Date.now()): void {
...
async function safeSendDailyReport(ctx: DailyReportContextLike, session: DailyReportSessionLike, message: unknown, label = 'message'): Promise<boolean> {
  try {
...
function apply(ctx: DailyReportContextLike): void {
  ctx.on('ready', () => {
