...
async function acquireRendererSlot() {
    ...
    while (activeRenderers >= MAX_RENDERERS) {
        if (Date.now() - startedAt > RENDER_QUEUE_TIMEOUT)
            throw new Error('daily report render queue timeout');
...
async function applyProfileNameDots(page) {
    await page.evaluate(() => {
        ...
        for (const container of names) {
            ...
            if (container.scrollWidth <= container.clientWidth || !chars.length)
                continue;
            let low = 0;
...
function buildQuotesHtml(goldenQuotes) {
    if (!goldenQuotes || !goldenQuotes.length)
        return '';
