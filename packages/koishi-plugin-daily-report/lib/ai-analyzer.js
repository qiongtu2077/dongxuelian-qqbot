...
function extractJsonCandidate(text) {
    ...
    for (let i = start; i < candidateSource.length; i++) {
        ...
        if (ch === close) {
            depth--;
            if (depth === 0)
                return candidateSource.slice(start, i + 1);
        }
    }
    return '';
}
...
function normalizeTopics(topics) {
    ...
    for (let i = 0; i < topics.length; i++) {
        ...
        if (!Object.keys(item).length)
            continue;
        result.push(createTopic(Number.isFinite(Number(item.id)) ? Number(item.id) : i + 1, normalizeString(item.title, `话题${i + 1}`), normalizeString(item.summary, '本段群聊内容还可以继续细化。'), normalizeStringArray(item.participants)));
    }
    return result;
}
...
function normalizeGoldenQuotes(quotes, messages) {
    ...
    for (const raw of quotes) {
        ...
        if (!userId && sender && nameToUserId.has(sender))
            userId = nameToUserId.get(sender) || '';
        if (userId && userIdToName.has(userId))
            sender = userIdToName.get(userId) || sender;
        if (!sender || isGenericSpeaker(sender))
            continue;
        result.push(createGoldenQuote(truncateText(content, 90), sender, normalizeString(item.reason, '这句很有代表性，适合当作今日金句。'), userId));
    }
    return result;
}
...
function normalizeUserTitles(userTitles, messages) {
    if (!Array.isArray(userTitles) || !userTitles.length)
        return [];
    const { nameToUserId, userIdToName } = buildMessageMaps(messages);
    const result = [];
    for (const raw of userTitles) {
        ...
        if (!resolvedName && !resolvedUserId)
            continue;
        result.push(createUserTitle(resolvedName || '群友', resolvedUserId, normalizeString(item.title, '活跃群友'), normalizeString(item.reason, '今天的发言记录还能继续细化。'), normalizeString(item.mbti)));
    }
    return result;
}
...
function normalizeQualityReview(review) {
    ...
    const dimensions = Array.isArray(record.dimensions)
        ? record.dimensions.map((raw, index) => {
            const item = asRecord(raw);
            if (!Object.keys(item).length)
                return null;
...
function buildFallbackGoldenQuotes(data) {
    ...
    for (let index = 0; index < messages.length; index++) {
        ...
        if (/[！？!?]/.test(content))
            score += 15;
        if (/哈哈|笑|233|乐|绷|绝了|有点东西/.test(content))
            score += 12;
        if (topIds.has(userId))
            score += 6;
    ...
    for (const item of scored) {
        ...
        if (seen.has(key))
            continue;
        seen.add(key);
        result.push(createGoldenQuote(truncateText(item.content, 90), sender, '这句很有代表性，适合当作今日金句。', userId));
        if (result.length >= 3)
            break;
    }
    if (!result.length) {
...
async function compressMessages(messages, meta) {
    ...
    for (let i = 0; i < limited.length; i += COMPRESS_BATCH_SIZE) {
        ...
        try {
            const summary = await callAI('你是群聊摘要助手。将以下群聊记录压缩成100字以内的摘要，保留主要话题和有趣对话。不要评价，只摘要。', batchText.slice(0, 4000), 200, { _timeoutMs: REPORT_COMPRESS_TIMEOUT_MS });
            if (summary)
  ...
  "goldenQuotes": [{"sender":"昵称","userId":"QQ号","content":"原话","reason":"点评"}]
    ...
    try {
        ...
        if (meta) {
            ...
            if (usedFallback)
                addMetaWarning(meta, '基础分析返回的 JSON 不完整，已启用话题/金句兜底。');
        }
        return result;
    }
    catch (err) {
  ...
  "qualityReview": {
    ...
    try {
        ...
        if (meta) {
            ...
            if (usedFallback)
                addMetaWarning(meta, '详细分析返回的 JSON 不完整，已启用画像/锐评兜底。');
        }
        return result;
    }
    catch (err) {
