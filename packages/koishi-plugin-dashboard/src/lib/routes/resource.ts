...
function recordDashboardMemorySample(mods: ResourceModuleSet, snapshot): void {
  const now = Date.now()
...
// 合成资源中心总览状态。
function buildResourceStatus(mods: ResourceModuleSet) {
...
export = { routes, buildResourceStatus, sanitizeTask, collectMemoryHistory, normalizeMemoryRange, getCachedMemoryHistory }
