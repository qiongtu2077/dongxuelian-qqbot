...
const { buildDashboardRelease } = require('../release') as typeof import('../release')
...
function readVerifiedRemoteRelease(server: string, appDir: string): RemoteReleaseManifest {
  ...
  const manifest = JSON.parse(String(output || '{}')) as RemoteReleaseManifest
  if (manifest.schemaVersion !== 1 || !/^[a-z0-9-]+$/.test(String(manifest.releaseId || '')) || !/^[a-f0-9]{64}$/.test(String(manifest.manifestHash || '')) || !/^[a-f0-9]{64}$/.test(String(manifest.contentHash || ''))) throw new Error('远端发布清单结构无效')
  return manifest
}
...
function handleGetCheckUpdate(req: IncomingMessage, res: ServerResponse): void {
  ...
  try {
    const saved = JSON.parse(fs.readFileSync(DEPLOY_CONFIG_FILE, 'utf8')) as Record<string, unknown>
    const cfg = dh.validateDeployTarget(saved)
    if (!cfg.server || !cfg.appDir) throw new Error('未保存远端发布目标')
  ...
  collectBody(req, res, (body) => {
    try {
      ...
      exec(npmCommand, { cwd: repoRoot, timeout: 10 * 60 * 1000, maxBuffer: 8 * 1024 * 1024 }, (buildError, stdout, stderr) => {
        if (stdout) try { fs.appendFileSync(path.join(DEPLOY_TASKS_DIR, taskId + '.log'), stdout.trim() + '\n', 'utf8') } catch { /* progress log is best effort */ }
        if (stderr) try { fs.appendFileSync(path.join(DEPLOY_TASKS_DIR, taskId + '.log'), stderr.trim() + '\n', 'utf8') } catch { /* progress log is best effort */ }
        if (buildError) return finishRemoteDeployTask(task, 'failed', 'build_all_plugins', buildError)
        try {
