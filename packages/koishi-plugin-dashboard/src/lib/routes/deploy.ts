...
const remoteRelease = require('../remote-release') as typeof import('../remote-release')
const cookieFile = require('koishi-plugin-local-video-sender/lib/cookie-file') as typeof import('koishi-plugin-local-video-sender/lib/cookie-file')
const {
  decodeStrictBase64,
  replaceBiliCookieFileAtomic,
  resolveBiliCookiePath,
  validateNetscapeCookieFile,
} = cookieFile
...
type RemoteDeployTaskState = 'running' | 'success' | 'failed'

interface RemoteDeployTask {
  taskId: string
  state: RemoteDeployTaskState
  stage: string
  error: string
  startedAt: number
  updatedAt: number
...
function sanitizeDeployError(error: unknown): string {
...
function deployTaskStateFile(taskId: string): string {
...
function writeRemoteDeployTask(task: RemoteDeployTask): void {
...
function setRemoteDeployStage(task: RemoteDeployTask, stage: string): void {
...
function finishRemoteDeployTask(task: RemoteDeployTask, state: Exclude<RemoteDeployTaskState, 'running'>, stage: string, error: unknown = ''): void {
  if (task.state !== 'running') return
  task.state = state
  task.stage = stage
...
function readRemoteDeployTask(taskId: string): RemoteDeployTask | null {
  try {
    const task = JSON.parse(fs.readFileSync(deployTaskStateFile(taskId), 'utf8')) as RemoteDeployTask
    if (task.state === 'running' && Date.now() >= task.expiresAt) finishRemoteDeployTask(task, 'failed', 'timeout', '远程部署超过服务端 30 分钟期限')
    return task
  } catch {
    return null
  }
...
interface RemoteActivationResult {
...
function writeSuccessfulReleaseRecord(task: RemoteDeployTask): void {
  let cfg: Record<string, unknown> = {}
  try { cfg = JSON.parse(fs.readFileSync(DEPLOY_CONFIG_FILE, 'utf8')) as Record<string, unknown> } catch { /* missing config is recreated from the task */ }
  Object.assign(cfg, {
    server: task.server,
    appDir: task.appDir,
    mode: 'update',
...
function refreshRemoteDeployTask(task: RemoteDeployTask): void {
  if (task.state !== 'running' || !task.server || !task.appDir) return
  try {
    ...
    const result = JSON.parse(String(output || '{}')) as RemoteActivationResult
    task.stage = String(result.stage || task.stage)
    task.updatedAt = Date.now()
    task.rolledBack = !!result.rolledBack
    if (result.rollbackState) task.rollbackState = result.rollbackState
    if (result.releaseId) task.releaseId = result.releaseId
    if (result.manifestHash) task.manifestHash = result.manifestHash
    if (result.contentHash) task.contentHash = result.contentHash
...
function runSafeDeployCommands(task: RemoteDeployTask, commands: Array<{ stage: string; command: string; timeout: number }>, onComplete: () => void): void {
  let index = 0
  const runNext = (): void => {
    if (index >= commands.length) return onComplete()
...
function handleGetDeployConfig(req: IncomingMessage, res: ServerResponse): void {
  ...
  } catch { return json(res, { server: '', appDir: DEFAULT_REMOTE_APP_DIR, botRunning: false }) }
}

// Builds and returns one frozen, read-only deployment preview for an administrator.
function handlePostDeployPreview(req: IncomingMessage, res: ServerResponse): void {
  if (!requireAdmin(req, res)) return
  collectBody(req, res, (body) => {
    try {
      const input = JSON.parse(body) as Record<string, unknown>
      const cfg = dh.validateDeployTarget(input)
      if (cfg.mode === 'install') return json(res, { ok: false, message: '首次安装请使用 setup.sh 或本地部署器；Web 远程发布只更新已有部署' }, 400)
...
function handlePostSafeDeployRun(req: IncomingMessage, res: ServerResponse): void {
  if (!requireAdmin(req, res)) return
  collectBody(req, res, (body) => {
    try {
      const input = JSON.parse(body) as Record<string, unknown>
      ...
      if (!release || !preview.target.release) throw new Error('部署预览没有完整的冻结发布物或远端基线')
...
function handleGetDeployProgress(req: IncomingMessage, res: ServerResponse, pathname: string): void {
  ...
  try {
    ...
    if (!task || !fs.existsSync(logFile)) return json(res, { ok: false, message: '部署任务不存在', code: 'DEPLOY_TASK_NOT_FOUND' }, 404)
...
function handlePostDeployUpload(req: IncomingMessage, res: ServerResponse): void {
  ...
  collectBody(req, res, (body) => {
    try {
      ...
      if (!validation.ok) return json(res, { ok: false, code: validation.code, line: validation.line }, 400)
      ...
      const candidate = e as { code?: unknown, line?: unknown }
