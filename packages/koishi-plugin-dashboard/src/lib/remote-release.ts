...
const { shellQuote } = require('./utils') as typeof import('./utils')
const { buildDashboardRelease, verifyReleaseManifest, RELEASE_PACKAGES } = require('./release') as typeof import('./release')
...
interface RemoteProbe {
  ...
  releaseError: string
  lock: { present: boolean, owner: string }
}
...
function inspectGitSource(repoRoot: string): GitSourceState {
  ...
  if (actualRoot !== requestedRoot) throw new Error('发布源不是独立 Git 仓库根目录')
  const commit = String(execFileSync('git', ['rev-parse', 'HEAD'], { cwd: actualRoot, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] })).trim()
  if (!/^[a-f0-9]{40,64}$/i.test(commit)) throw new Error('无法读取完整 Git 提交号')
...
function buildTrackedArtifacts(repoRoot: string): void {
...
function isAllowedBuildChange(statusLine: string): boolean {
  const paths = String(statusLine || '').slice(3).split(' -> ').map(value => value.replace(/\\/g, '/'))
  return paths.length > 0 && paths.every(relativePath => {
    if (relativePath.startsWith('packages/koishi-plugin-dashboard/frontend/dist/')) return true
    if (['packages/koishi-plugin-dashboard/index.js', 'packages/koishi-plugin-dashboard/index.d.ts', 'packages/koishi-plugin-dashboard/standalone.js', 'packages/koishi-plugin-dashboard/standalone.d.ts'].includes(relativePath)) return true
...
function getBuildDependencyLinks(repoRoot: string, worktreeRoot: string): Array<{ source: string, target: string }> {
  return [
...
function linkBuildDependencies(repoRoot: string, worktreeRoot: string): void {
  for (const link of getBuildDependencyLinks(repoRoot, worktreeRoot)) {
    if (!fs.existsSync(link.source)) throw new Error(`发布源缺少构建依赖目录: ${path.relative(repoRoot, link.source).replace(/\\/g, '/')}`)
...
function removeBuildWorktree(repoRoot: string, worktreeRoot: string): void {
  ...
  try {
    ...
    try { fs.rmSync(worktreeRoot, { recursive: true, force: true }) } catch { /* prune clears the registration when filesystem cleanup is unavailable */ }
    try { execFileSync('git', ['worktree', 'prune'], { cwd: repoRoot, stdio: ['ignore', 'pipe', 'pipe'] }) } catch { /* later previews use a unique path */ }
  }
}

// Builds a committed snapshot in an isolated worktree and freezes its exact artifacts.
function buildFrozenRelease(repoRoot: string, worktreeRoot: string, releasesRoot: string, commit: string): FrozenBuildResult {
...
function describeRemoteProbeFailure(value: unknown): string {
  const raw = String(value || 'SSH 只读探测失败').trim().slice(-1000)
  if (/REMOTE HOST IDENTIFICATION HAS CHANGED|Host key verification failed/i.test(raw)) return `SSH 主机密钥校验失败，目标身份可能已变化：${raw}`
  if (/Permission denied/i.test(raw)) return `SSH 密钥认证失败：${raw}`
  if (/Could not resolve hostname|Name or service not known/i.test(raw)) return `SSH 目标主机无法解析：${raw}`
  if (/Connection timed out|Operation timed out/i.test(raw)) return `SSH 连接超时：${raw}`
  if (/Connection refused/i.test(raw)) return `SSH 连接被拒绝：${raw}`
  return raw
}

// Performs one fixed, read-only remote probe through the validated SSH target.
function probeRemoteTarget(server: string, appDir: string): RemoteProbe {
  ...
  const raw = JSON.parse(String(output || '{}')) as RemoteProbe & { ok?: boolean, error?: string }
  if (!raw.ok) throw new Error(String(raw.error || '远端应用目录探测失败'))
  if (!raw.hostname || !raw.appDir || !Number.isFinite(Number(raw.availableBytes))) throw new Error('远端探测结果结构无效')
...
function normalizeReleaseIdentity(value: unknown): ReleaseIdentity | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null
  const raw = value as Partial<ReleaseIdentity>
  if (raw.schemaVersion !== 1 || !/^[a-z0-9-]+$/.test(String(raw.releaseId || '')) || !/^[a-f0-9]{40,64}$/.test(String(raw.commit || '')) || !/^[a-f0-9]{64}$/.test(String(raw.manifestHash || '')) || !/^[a-f0-9]{64}$/.test(String(raw.contentHash || '')) || !Array.isArray(raw.files)) return null
  const files = raw.files.map(item => ({ path: String(item.path || ''), size: Number(item.size), sha256: String(item.sha256 || '') }))
  if (files.some(item => !item.path || !Number.isFinite(item.size) || item.size < 0 || !/^[a-f0-9]{64}$/.test(item.sha256))) return null
...
function summarizeReleaseChanges(nextFiles: ReleaseFile[], currentFiles: ReleaseFile[]): ReleaseChangeSummary {
  ...
  for (const file of nextFiles) {
    ...
    if (!previous) added += 1
    else if (previous.size !== file.size || previous.sha256 !== file.sha256) modified += 1
    else unchanged += 1
...
function isSelfDeploy(sourceHostname: string, server: string, targetHostname: string): boolean {
...
function collectPreviewBlockers(source: GitSourceState, server: string, requestedAppDir: string, target: RemoteProbe | null, release: PreviewRelease | null, remoteError = ''): string[] {
  const blockers: string[] = []
  if (!source.clean) blockers.push(`发布源工作区不干净（${source.changes.length} 项），请提交或移除改动后重新预览`)
  if (remoteError) blockers.push(`远端只读探测失败：${remoteError}`)
  if (target) {
    if (isSelfDeploy(source.hostname, server, target.hostname)) blockers.push('目标主机与发布源相同，禁止控制台给自身发布')
    if (target.appDir !== requestedAppDir) blockers.push(`远端应用目录解析为 ${target.appDir}，与配置值不一致`)
    if (!target.release) blockers.push(`远端当前发布无法完整校验：${target.releaseError || '尚无不可变发布记录'}`)
    if (target.lock.present) blockers.push(`远端存在发布锁${target.lock.owner ? `：${target.lock.owner}` : ''}`)
    if (release) {
      const requiredBytes = release.totalBytes + Math.max(DISK_SAFETY_BYTES, Math.ceil(release.totalBytes * 0.1))
      if (target.availableBytes < requiredBytes) blockers.push(`远端磁盘不足：需要至少 ${requiredBytes} 字节，可用 ${target.availableBytes} 字节`)
    }
  }
  return blockers
}
...
function previewFile(previewsDir: string, previewId: unknown): string {
  const id = String(previewId || '')
  if (!/^[a-f0-9]{32}$/.test(id)) throw new Error('无效 previewId')
...
function writePreview(previewsDir: string, preview: RemoteReleasePreview): void {
...
function isPreviewReleaseDirectory(releasesRoot: string, releaseDir: string, releaseId: string): boolean {
...
function cleanupExpiredPreviews(previewsDir: string, releasesRoot: string, now = Date.now()): void {
  let entries: string[] = []
  try { entries = fs.readdirSync(previewsDir) } catch { return }
  for (const name of entries) {
    if (!/^[a-f0-9]{32}\.json$/.test(name)) continue
    const file = path.join(previewsDir, name)
    try {
      const preview = JSON.parse(fs.readFileSync(file, 'utf8')) as RemoteReleasePreview
      const startedStillActive = preview.startedAt > 0 && now - preview.startedAt <= STARTED_PREVIEW_RETENTION_MS
      if (preview.expiresAt > now || startedStillActive) continue
      if (preview.release?.releaseDir && isPreviewReleaseDirectory(releasesRoot, preview.release.releaseDir, preview.release.releaseId)) fs.rmSync(preview.release.releaseDir, { recursive: true, force: true })
...
function createRemoteReleasePreview(options: CreatePreviewOptions): RemoteReleasePreview {
  ...
  if (source.clean) {
    ...
    if (frozen.changes.length) source = { ...source, clean: false, changes: frozen.changes }
  }

  let target: RemoteProbe | null = null
  let remoteError = ''
  try { target = probeRemoteTarget(targetConfig.server, targetConfig.appDir) }
  catch (error) { remoteError = String((error as { message?: unknown })?.message || error || '远端只读探测失败').slice(0, 1000) }
  ...
  const preview: RemoteReleasePreview = {
    schemaVersion: 1,
    previewId,
    createdAt: Date.now(),
...
function readRemoteReleasePreview(previewsDir: string, previewId: unknown): RemoteReleasePreview {
  const preview = JSON.parse(fs.readFileSync(previewFile(previewsDir, previewId), 'utf8')) as RemoteReleasePreview
  if (preview.schemaVersion !== 1 || preview.previewId !== String(previewId) || !Number.isFinite(preview.createdAt) || !Number.isFinite(preview.expiresAt) || !Number.isFinite(preview.startedAt) || !preview.source || !preview.target || !Array.isArray(preview.blockers) || preview.blockers.some(item => typeof item !== 'string')) throw new Error('部署预览记录结构无效')
  if (preview.release && (!/^[a-z0-9-]+$/.test(String(preview.release.releaseId || '')) || !/^[a-f0-9]{64}$/.test(String(preview.release.manifestHash || '')) || !/^[a-f0-9]{64}$/.test(String(preview.release.contentHash || '')) || !Array.isArray(preview.release.files))) throw new Error('部署预览发布身份无效')
  return preview
}

// Confirms the second remote probe still matches every frozen target identity field.
function compareRemoteBaseline(preview: RemoteReleasePreview, target: RemoteProbe): string[] {
  const changes: string[] = []
  if (target.hostname !== preview.target.hostname) changes.push('远端主机名已变化')
  if (target.appDir !== preview.target.appDir) changes.push('远端应用目录已变化')
  if (target.lock.present) changes.push(`远端出现发布锁${target.lock.owner ? `：${target.lock.owner}` : ''}`)
  const before = preview.target.release
  const current = target.release
  if (!before || !current || before.releaseId !== current.releaseId || before.manifestHash !== current.manifestHash || before.contentHash !== current.contentHash) changes.push('远端当前发布基线已变化')
  if (target.availableBytes < preview.requiredBytes) changes.push('远端可用磁盘已低于预览要求')
  return changes
}

// Enforces confirmation, expiry, single use and blocker state before side effects.
function assertPreviewCanStart(preview: RemoteReleasePreview, confirmed: unknown, now = Date.now()): void {
  if (confirmed !== true) throw new Error('必须确认目标主机、目录和短暂停机影响')
  if (now >= preview.expiresAt) throw new Error('部署预览已过期，请重新生成')
  if (preview.startedAt > 0) throw new Error('部署预览已经使用，请重新生成')
  if (preview.blockers.length) throw new Error(`部署预览存在阻止项：${preview.blockers.join('；')}`)
}

// Revalidates the source, frozen release and remote baseline before creating a task.
function validateRemoteReleasePreview(options: ValidatePreviewOptions): RemoteReleasePreview {
  ...
  if (!preview.release) throw new Error('部署预览没有冻结发布物')
  if (!isPreviewReleaseDirectory(options.releasesRoot, preview.release.releaseDir, preview.release.releaseId)) throw new Error('冻结发布目录超出允许范围')

  const source = inspectGitSource(options.repoRoot)
  if (!source.clean || source.commit !== preview.source.commit || source.repoRoot !== preview.source.repoRoot) throw new Error('发布源提交或工作区已变化，请重新生成预览')
  const manifest = verifyReleaseManifest(preview.release.releaseDir)
  if (manifest.releaseId !== preview.release.releaseId || manifest.commit !== preview.release.commit || manifest.manifestHash !== preview.release.manifestHash || manifest.contentHash !== preview.release.contentHash) throw new Error('冻结发布物已变化，请重新生成预览')
  ...
  if (baselineChanges.length) throw new Error(`${baselineChanges.join('；')}，请重新生成预览`)
...
function toPublicRemoteReleasePreview(preview: RemoteReleasePreview): Record<string, unknown> {
  return {
    ok: true,
