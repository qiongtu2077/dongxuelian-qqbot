...
const WORKER_KINDS: Record<string, string[]> = {
  agent: [
    'agent_task',
    'dashboard_agent',
    'agent_memory',
...
const MEDIA_RISK_SEVERITY: Record<MediaRiskCode, number> = {
  idle: 0,
  queued: 1,
  near_limit: 2,
  at_limit: 3,
...
export function resolveWorkerType(worker: ReadableRecord): 'agent' | 'daily' | 'media' | 'unknown' {
...
// 返回全局后台当前成立的全部暂停原因。
export function buildBackgroundPauseReasons(snapshot: ReadableRecord): BackgroundPauseReason[] {
  const reasons: BackgroundPauseReason[] = []
  if (snapshot.maintenance === true || String(snapshot.botMode || '') === 'maintenance') reasons.push('maintenance')
  if (String(snapshot.resourceState || '') === 'red') reasons.push('resource_critical')
  if (snapshot.backgroundAllowed === false && snapshot.toolActive === true) reasons.push('browser_active')
  if (snapshot.backgroundAllowed === false && snapshot.renderActive === true) reasons.push('daily_render_active')
  return reasons
}
...
function countResourceWorkerTasks(workerType: 'agent' | 'daily', tasks: ReadableRecord[]): WorkerCounts {
...
function findWorkerRunningTask(worker: ReadableRecord, workerType: string, tasks: ReadableRecord[]): ReadableRecord | null {
  if (workerType === 'media' || workerType === 'unknown') return null
  const currentTaskId = String(worker.currentTaskId || worker.taskId || '')
  const workerName = String(worker.name || '')
  return tasks.find(task => String(task.status || '') === 'running' && (
    (currentTaskId && String(task.id || '') === currentTaskId)
    || (!currentTaskId && workerName && String(task.claimedBy || '') === workerName)
  )) || null
}

// 只保留处理器诊断详情需要的安全任务字段。
function sanitizeRunningTask(task: ReadableRecord | null): ReadableRecord | null {
  if (!task) return null
  return {
    id: task.id || '',
    kind: task.kind || '',
...
function buildWorkerPauseReasons(
  worker: ReadableRecord,
  workerType: string,
  snapshot: ReadableRecord,
  globalReasons: BackgroundPauseReason[],
): BackgroundPauseReason[] {
  const reasons = new Set<BackgroundPauseReason>(globalReasons)
  if (worker.parked === true && snapshot.toolActive === true && (workerType === 'media' || workerType === 'daily')) {
    reasons.add('browser_active')
  }
  if (worker.parked === true && snapshot.renderActive === true) reasons.add('daily_render_active')
  return Array.from(reasons)
}

// 按计划固定的严重程度顺序选择处理器主健康状态。
function resolveWorkerHealthCode(
  worker: ReadableRecord,
  counts: WorkerCounts,
  runningTask: ReadableRecord | null,
  pauseReasons: BackgroundPauseReason[],
  resolveTaskTimeoutMs: (task: ReadableRecord) => number,
  now: number,
): WorkerHealthCode {
...
export function buildReadableWorkers(input: ResourceReadabilityInput, globalReasons: BackgroundPauseReason[]): ReadableRecord[] {
...
// 根据单类队列占用比例生成媒体风险状态码。
export function resolveMediaRisk(queueTotal: unknown, queueLimit: unknown): MediaRiskCode {
  const total = countValue(queueTotal)
  const limit = countValue(queueLimit)
  if (total === 0) return 'idle'
  if (limit > 0 && total >= limit) return 'at_limit'
  if (limit > 0 && total >= Math.ceil(limit * 0.8)) return 'near_limit'
  return 'queued'
}

// 为三类媒体队列生成各自风险与整体最严重状态。
export function buildMediaRisk(media: ReadableRecord): ReadableRecord {
  const queues = media.queues && typeof media.queues === 'object' && !Array.isArray(media.queues)
    ? media.queues as ReadableRecord
    : {}
  const mediaRiskByKind: Record<string, MediaRiskCode> = {}
  ...
  const mediaRiskCode = Object.values(mediaRiskByKind)
    .reduce<MediaRiskCode>((worst, code) => MEDIA_RISK_SEVERITY[code] > MEDIA_RISK_SEVERITY[worst] ? code : worst, 'idle')
...
export function buildResourceReadability(input: ResourceReadabilityInput): ReadableRecord {
