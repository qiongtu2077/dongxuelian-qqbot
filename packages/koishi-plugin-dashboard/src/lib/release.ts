...
function assertReleaseRelativePath(relativePath: string): void {
  const normalized = String(relativePath || '').replace(/\\/g, '/')
  if (!normalized || normalized.startsWith('/') || normalized.includes('\0') || normalized.split('/').includes('..')) throw new Error('发布文件路径无效')
}

// Resolves a verified release-relative path under one explicit root.
function resolveReleasePath(root: string, relativePath: string): string {
  assertReleaseRelativePath(relativePath)
  const target = path.resolve(root, relativePath)
  const relative = path.relative(path.resolve(root), target)
  if (!relative || relative.startsWith('..' + path.sep) || path.isAbsolute(relative)) throw new Error('发布文件超出发布目录')
  return target
}
...
function listReleaseFiles(root: string): ReleaseFile[] {
...
function computeManifestHash(manifest: Omit<ReleaseManifest, 'manifestHash'>): string {
  return crypto.createHash('sha256').update(JSON.stringify(manifest)).digest('hex')
}

// Retries a same-directory Windows rename only for transient file-indexing contention.
function renameReleaseDirectory(source: string, target: string): void {
  for (let attempt = 0; attempt < 10; attempt += 1) {
    try {
      ...
      if (process.platform !== 'win32' || !['EPERM', 'EBUSY'].includes(code) || attempt === 9) throw error
...
function copyReleaseEntry(repoRoot: string, releaseRoot: string, relativePath: string, required = false): void {
  const source = resolveReleasePath(repoRoot, relativePath)
  const target = resolveReleasePath(releaseRoot, relativePath)
  if (!fs.existsSync(source)) {
    if (required) throw new Error(`发布源缺失: ${relativePath}`)
    return
  }
...
function copyReleasePayload(repoRoot: string, releaseRoot: string): void {
  for (const packageName of RELEASE_PACKAGES) {
    ...
    for (const optional of ['assets', 'templates']) copyReleaseEntry(repoRoot, releaseRoot, `${packageRoot}/${optional}`)
  }
  const dashboardRoot = `packages/${DASHBOARD_PACKAGE}`
  for (const file of ['index.js', 'index.d.ts', 'standalone.js', 'standalone.d.ts']) copyReleaseEntry(repoRoot, releaseRoot, `${dashboardRoot}/${file}`, true)
  for (const entry of ['frontend/src', 'frontend/public', 'frontend/dist', 'frontend/index.html', 'frontend/package.json', 'frontend/vite.config.ts', 'frontend/tsconfig.json']) copyReleaseEntry(repoRoot, releaseRoot, `${dashboardRoot}/${entry}`, true)
  for (const script of ['activate-dashboard-release.sh', 'verify-release-manifest.js', 'restart-bot.sh', 'seal-data-dir.sh', 'watchdog.sh', 'install-dashboard-service.sh', 'install-koishi-service.sh', 'install-logrotate.sh']) copyReleaseEntry(repoRoot, releaseRoot, `scripts/${script}`, true)
  for (const packageName of RELEASE_PACKAGES) {
...
function buildDashboardRelease(repoRoot: string, outputRoot: string): BuildReleaseResult {
  ...
  if (fs.existsSync(nextDir) || fs.existsSync(releaseDir)) throw new Error('发布版本目录已存在')
  ...
  try {
    ...
    const manifestBase: Omit<ReleaseManifest, 'manifestHash'> = { schemaVersion: 1, releaseId, version, commit, builtAt: new Date().toISOString(), files, contentHash }
    const manifest: ReleaseManifest = { ...manifestBase, manifestHash: computeManifestHash(manifestBase) }
    ...
    try { fs.rmSync(nextDir, { recursive: true, force: true }) } catch { /* preserve the original build error */ }
    throw error
  }
}
...
function verifyBrowserAssets(releaseRoot: string, manifest: ReleaseManifest): void {
  ...
  for (const reference of assetReferences) {
    ...
    if (cleanReference.startsWith('/') && !cleanReference.startsWith('/dashboard/')) throw new Error(`浏览器入口引用超出 Dashboard 路径: ${reference}`)
    ...
    if (!fs.existsSync(resolveReleasePath(releaseRoot, relativeAsset)) || !listed.has(relativeAsset)) throw new Error(`浏览器入口引用未进入发布清单: ${reference}`)
  }
}

// Verifies the manifest hash, exact payload file set, sizes, hashes, and browser entry references.
function verifyReleaseManifest(releaseRoot: string): ReleaseManifest {
  ...
  if (manifest.schemaVersion !== 1 || !/^[a-z0-9-]+$/.test(String(manifest.releaseId || '')) || !/^[a-f0-9]{40,64}$/i.test(String(manifest.commit || '')) || !/^[a-f0-9]{64}$/.test(String(manifest.contentHash || '')) || !/^[a-f0-9]{64}$/.test(String(manifest.manifestHash || '')) || !Array.isArray(manifest.files)) throw new Error('发布清单结构无效')
  const { manifestHash, ...manifestBase } = manifest
  if (computeManifestHash(manifestBase) !== manifestHash) throw new Error('发布清单哈希不一致')
  const actualFiles = listReleaseFiles(root)
  if (JSON.stringify(actualFiles) !== JSON.stringify(manifest.files)) throw new Error('发布物文件清单不一致')
  if (crypto.createHash('sha256').update(JSON.stringify(actualFiles)).digest('hex') !== manifest.contentHash) throw new Error('发布内容哈希不一致')
