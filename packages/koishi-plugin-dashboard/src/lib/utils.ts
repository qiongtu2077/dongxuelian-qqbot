...
function describeFsError(e: unknown, fallback = '') {
  const err = e as Partial<FsError>
  const code = err && err.code ? String(err.code) : ''
  if (code === 'ENOTDIR') return '路径冲突：目标路径的某一级已经是文件，不是目录。请删除冲突文件后重试。' + (err.path ? ` 冲突路径：${err.path}` : '')
  if (code === 'EACCES' || code === 'EPERM') return '权限不足或文件被占用。请关闭占用程序，或把部署器移动到可写目录后重试。' + (err.path ? ` 路径：${err.path}` : '')
  if (code === 'EBUSY' || code === 'ENOTEMPTY') return '文件正在被占用或目录未能清空。请关闭 NapCat/QQ/Node 相关进程后重试。' + (err.path ? ` 路径：${err.path}` : '')
...
function removePathWithRetry(targetPath, options: RemovePathOptions = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    try {
      ...
      lastError = new Error('路径仍然存在，可能被占用：' + targetPath) as FsError
      lastError.code = 'EBUSY'
      lastError.path = targetPath
    } catch (error) {
      lastError = error as FsError
...
function collectBody(req, res, callback, options: CollectBodyOptions = {}) {
  ...
  req.on('error', () => {
    if (rejected) return
    rejected = true
    try { json(res, { ok: false, message: '请求读取失败' }, 400) } catch { /* non-critical: response may already be closed */ }
  })
}
