...
process.on('unhandledRejection', (reason) => {
  console.error('[dashboard] UNHANDLED REJECTION:', (reason as { stack?: string })?.stack || reason)
})
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname.startsWith('/agent/')) {
    ...
    try { agentReqPath = decodeURIComponent(agentReqPath) } catch { /* non-critical: keep raw request path */ }
  ...
  try { reqPath = decodeURIComponent(reqPath) } catch { /* non-critical: keep raw request path */ }
