...
const { PORT, HOST, DIST_DIR, AGENT_CONSOLE_DIST_DIR, KOISHI_PID_FILE, DATA_DIR, RESET_TOKEN_FILE, ADMIN_PWD_FILE, ACCESS_PWD_FILE, isGlobalLocalMode, } = require('./lib/paths');
const { isLocalAuthBypass, requireAdmin, shouldGenerateResetTokenOnStartup, getResetToken, generateResetToken, ensureInitialCredentials, applyCorsHeaders, rejectCrossSiteRequest, } = require('./lib/auth');
...
const server = http.createServer(async (req, res) => {
    ...
    if (isSensitiveRequest && rejectCrossSiteRequest(req, res))
        return;
    ...
    if (router.dispatch(req, res, pathname, url))
        return;
    ...
    const serveStaticFile = (rootDir, filePath) => {
        ...
        catch { /* non-critical: static file probe fallback */ }
        return false;
    };
    if (pathname.startsWith('/agent/')) {
        let agentReqPath = pathname.replace(/^\/agent\/?/, '');
        try {
            agentReqPath = decodeURIComponent(agentReqPath);
        }
        catch { /* non-critical: keep raw request path */ }
        if (serveStaticFile(AGENT_CONSOLE_DIST_DIR, path.join(AGENT_CONSOLE_DIST_DIR, agentReqPath || 'index.html')))
            return;
        ...
        if (serveStaticFile(AGENT_CONSOLE_DIST_DIR, path.join(AGENT_CONSOLE_DIST_DIR, 'index.html')))
            return;
    ...
    if (serveStaticFile(DIST_DIR, path.join(DIST_DIR, reqPath || 'index.html')))
        return;
    ...
    if (!pathname.startsWith('/dashboard/api/') && serveStaticFile(DIST_DIR, path.join(DIST_DIR, 'index.html')))
        return;
    res.writeHead(404);
    res.end('Not Found');
});
if (require.main === module) {
    ...
    server.on('error', (err) => {
        if (err && err.code === 'EADDRINUSE')
            ...
            console.error('[dashboard] HTTP server error:', err.stack || err.message || err);
        process.exit(1);
    });
