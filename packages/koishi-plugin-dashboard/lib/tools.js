...
function getLocalToolEnv(extra = {}) {
    ...
    if (fs.existsSync(nodeDir))
        env[pathKey] = [nodeDir, env[pathKey]].filter(Boolean).join(path.delimiter);
    return env;
}
...
function normalizeProxyValue(value) {
    ...
    if (!text || /^(?:null|undefined|false)$/i.test(text))
        return '';
    return text;
}
...
function checkPortState(port) {
    ...
    if (!Number.isInteger(value) || value < 1 || value > 65535)
        return { available: false, status: 'invalid', reason: '端口号无效' };
    const script = `
...
function resolveKoishiListenPort() {
    ...
    catch { /* non-critical: koishi.yml may be absent */ }
    return 5140;
}
module.exports = {
    getCommandVersion,
