    ...
    collectBody(req, res, (body) => {
        try {
            ...
            buildFrontendDist({ log: taskLog, updateStatus: status => setRebuildStatus(status) }, (buildErr) => {
                ...
                cmds.push(dh.sshCommand(s, `chmod +x ${[dh.remoteJoin(d, 'restart.sh'), dh.remoteJoin(scriptsDir, 'seal-data-dir.sh'), dh.remoteJoin(scriptsDir, 'watchdog.sh'), dh.remoteJoin(scriptsDir, 'install-dashboard-service.sh')].map(shellQuote).join(' ')} 2>/dev/null || true`));
