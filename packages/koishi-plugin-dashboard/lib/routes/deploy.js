...
function handleGetCheckUpdate(req, res) {
  ...
  let deployed = ''
  try { deployed = JSON.parse(fs.readFileSync(DEPLOY_CONFIG_FILE, 'utf8')).deployFingerprint || '' } catch {}
  ...
  collectBody(req, res, (body) => {
    try {
      ...
      buildFrontendDist({ log: taskLog, updateStatus: status => setRebuildStatus(status) }, (buildErr) => {
        ...
        for (const name of ['index.html', 'package.json', 'package-lock.json', 'vite.config.js']) { const localFile = path.join(FE_DIR, name); if (fs.existsSync(localFile)) cmds.push(dh.scpCommand(localFile, dh.scpRemoteTarget(s, dh.remoteJoin(dashboardFrontendDir, name)))) }
        ...
        cmds.push(dh.sshCommand(s, `chmod +x ${[dh.remoteJoin(d, 'restart.sh'), dh.remoteJoin(scriptsDir, 'seal-data-dir.sh'), dh.remoteJoin(scriptsDir, 'watchdog.sh')].map(shellQuote).join(' ')} 2>/dev/null || true`))
