...
function stopKoishiProcesses() {
    ...
    if (pid > 0) {
        if (process.platform === 'win32') {
            ...
            catch { /* non-critical: process may already be stopped */ }
        }
        else {
            try {
    ...
    if (process.platform === 'win32') {
        ...
        execSync(`powershell -NoProfile -Command "$d=${dirLit}; Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -and $_.CommandLine.Contains([string]$d) -and ($_.CommandLine -match 'koishi') } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"`, { timeout: 8000, stdio: 'ignore' });
        waitKoishiPortFree();
        return;
    }
...
function getLegacyNapcatStatus() {
    ...
    try {
        ...
        processLines = output.split(/\r?\n/).map(line => line.trim()).filter(line => {
            if (!line)
                return false;
            if (/\/opt\/QQ\/qq(?:\s|$)/.test(line))
                return true;
            if (/\bxvfb-run\b/.test(line) && /\/opt\/QQ\/qq/.test(line))
                return true;
            if (/\bXvfb\b/.test(line))
                return true;
            if (/\bSCREEN\b/.test(line) && /\bnapcat\b/i.test(line))
                return true;
            return false;
        });
    }
...
function handlePostBotStart(req, res) {
    ...
    if (!fs.existsSync(restartScript))
        return json(res, { ok: false, message: '启动脚本不存在，请检查部署目录' }, 400);
    exec(`bash "${restartScript.replace(/\\/g, '/')}"`, { maxBuffer: 512 * 1024 }, (err) => {
        if (err)
            log('start bot failed: ' + err.message);
    });
    ...
    collectBody(req, res, (body) => {
        try {
            ...
            if (enabled)
                ...
                catch { /* non-critical: stale token cleanup */ }
...
function handleGetThrottle(req, res) {
    try {
        ...
        return json(res, { maxPerMinute: 20 });
    }
}
...
function handlePostNapcatRestart(req, res) {
    ...
    if (!qq)
        return json(res, { ok: false, message: '未配置 QQ 号，请设置 DASHBOARD_QQ_NUMBER/QQ_NUMBER 或在 koishi.yml 写入 selfId' }, 400);
...
function handleGetBotActivity(req, res, pathname, url) {
    ...
    try {
        ...
        return json(res, { entries: [], lines: [], total: 0 });
    }
}
const routes = {
