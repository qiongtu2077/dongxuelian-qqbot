...
function safeCompare(a, b) {
    ...
    if (bufA.length !== bufB.length)
        return crypto.timingSafeEqual(Buffer.alloc(32), Buffer.alloc(32)) && false;
...
function getAccessPasswordRecord() {
    ...
    if (current)
        return { value: current, sourceFile: ACCESS_PWD_FILE, legacy: false };
    const legacy = readFileContent(LEGACY_ACCESS_PWD_FILE);
    if (legacy)
        return { value: legacy, sourceFile: LEGACY_ACCESS_PWD_FILE, legacy: true };
...
async function removeLegacyAccessPasswordAfterUpgrade(record, input) {
    if (!record || !record.legacy || record.sourceFile !== LEGACY_ACCESS_PWD_FILE)
        return false;
    const upgraded = readFileContent(ACCESS_PWD_FILE);
    if (!isBcryptHash(upgraded))
        return false;
    ...
    if (!upgradedMatches)
        return false;
    const legacy = readFileContent(LEGACY_ACCESS_PWD_FILE);
    if (!legacy || !safeCompare(legacy, record.value))
        return false;
    try {
...
function ensurePassword(file, envValue, label, options = {}) {
    ...
    if (isGlobalLocalMode() && !options.force)
        return '';
...
function validateScopedToken(scope, token) {
    ...
    const now = Date.now();
    if (!Number.isFinite(timestamp) || timestamp > now + 60 * 1000 || now - timestamp > TOKEN_EXPIRY_MS)
        return false;
...
function trimLoginFailMap(now = Date.now()) {
    ...
    if (loginFailMap.size <= LOGIN_FAIL_MAX_ENTRIES)
        return;
    const ordered = Array.from(loginFailMap.entries()).sort((a, b) => Number(a[1]?.firstFail || 0) - Number(b[1]?.firstFail || 0));
    for (const [ip] of ordered.slice(0, Math.max(0, loginFailMap.size - LOGIN_FAIL_MAX_ENTRIES)))
        loginFailMap.delete(ip);
}
module.exports = {
    TOKEN_EXPIRY_MS,
