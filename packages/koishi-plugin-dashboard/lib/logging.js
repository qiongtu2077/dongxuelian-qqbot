...
function readLoggingConfig() {
    try {
        return normalizeLoggingConfig(JSON.parse(fs.readFileSync(DEBUG_LOG_CONFIG_FILE, 'utf8') || '{}'));
    }
    catch { /* non-critical: logging config fallback */ }
...
function readLastLogItems(file, limit = MAX_LOG_LIMIT) {
    ...
    for (let cursor = lineStart; cursor < buffer.length;) {
        ...
        if (lineEnd > cursor) {
            ...
            if (raw)
                items.push({ id: startOffset + cursor, text: raw });
        }
        cursor = lineEnd + 1;
    }
...
function classifyLogLevel(line = '') {
    if (/\[D\]|\bdebug\b|debug:/i.test(line))
        return 'D';
    if (/\[E\]|\berror\b|uncaught|exception|failed|fail:/i.test(line))
        return 'E';
    if (/\[W\]|\bwarn\b|warning/i.test(line))
        return 'W';
    return 'I';
}
...
function readLastLogEntries(file) {
    try {
        const stat = fs.statSync(file);
        if (logEntryCache.file === file && logEntryCache.size === stat.size && logEntryCache.mtimeMs === stat.mtimeMs)
            return logEntryCache.entries;
...
function getFilteredLogEntries(options = {}) {
    ...
    if (errorsOnly)
        entries = entries.filter(item => item.level === 'E');
    else
        entries = entries.filter(item => levels.has(item.level));
    if (moduleFilter && moduleFilter !== 'all')
        entries = entries.filter(item => item.module.toLowerCase().includes(moduleFilter) || item.text.toLowerCase().includes(moduleFilter));
    if (query)
        entries = entries.filter(item => item.text.toLowerCase().includes(query) || item.message.toLowerCase().includes(query));
