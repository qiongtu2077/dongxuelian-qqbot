...
function getLocalWorkDirSafety() {
    ...
    const reasons = [];
    if (values.some(item => item === tempDir || item.startsWith(tempDir + path.sep.toLowerCase())))
        reasons.push('工作目录位于系统临时目录');
    if (values.some(item => /[\\/]resources[\\/]app(?:[\\/]|$)/i.test(item)))
        reasons.push('工作目录位于 Electron 资源临时目录');
...
function ensurePackagedWorkspace(options = {}) {
    if (!isPackagedLocalWorkspace())
        return { ok: true, skipped: true, workspaceRoot: path.resolve(KOISHI_DIR), resourceRoot: getResourceRoot() };
    const resourceRoot = getResourceRoot();
    const workspaceRoot = path.resolve(process.env.LIANLIAN_WORKSPACE_ROOT || KOISHI_DIR);
    if (workspaceRoot.toLowerCase() === resourceRoot.toLowerCase())
        return { ok: true, skipped: true, workspaceRoot, resourceRoot };
    ensureWritableDir(workspaceRoot);
    for (const dir of ['packages', 'scripts'])
        copyWorkspaceResource(resourceRoot, workspaceRoot, dir, { replace: true });
    for (const file of ['package.json', 'package-lock.json', 'start.js', 'koishi.example.yml'])
        copyWorkspaceResource(resourceRoot, workspaceRoot, file, { replace: true });
    ...
    if (options.includeNapcat !== false)
        dirs.push(path.join(workspaceRoot, 'runtime', 'napcat'));
    for (const dir of dirs)
        fs.mkdirSync(dir, { recursive: true });
    let version = '';
    try {
        version = JSON.parse(fs.readFileSync(path.join(resourceRoot, 'package.json'), 'utf8')).version || '';
    }
    catch { /* non-critical: packaged version fallback */ }
...
function writeRuntimeLayout(options = {}) {
    ...
    if (includeNapcat)
        dirs.push(runtimePath('napcat'));
    if (includeNodeModules)
        dirs.push(path.join(KOISHI_DIR, 'node_modules'));
    for (const dir of dirs)
        fs.mkdirSync(dir, { recursive: true });
}
...
function inspectChinesePathWrite(dir) {
    ...
    catch { /* non-critical: write probe reports failure */ }
...
function getContentDispositionFileName(header) {
    ...
    if (star?.[1])
        return sanitizeDownloadName(star[1]);
...
function validateDownloadedFile(filePath, options = {}) {
    ...
    if (minBytes && stat.size < minBytes)
        throw new Error(`下载文件过小：${stat.size} 字节，可能是网络错误页或下载不完整`);
    if (stat.size > MAX_DOWNLOAD_BYTES)
        throw new Error(`下载文件过大：${stat.size} bytes`);
    const expectsZip = /\.zip$/i.test(String(options.expectedExt || '')) || /zip/i.test(String(options.expectedContentType || '')) || /\.zip$/i.test(filePath);
    if (expectsZip && !hasZipMagic(filePath))
        throw new Error('下载文件不是有效 zip 包，可能下载到了 HTML 错误页或被代理改写');
...
function getDownloadFileName(parsed, response, options = {}) {
    ...
    if ((!path.extname(name) || /^[0-9a-f-]{16,}$/i.test(name)) && /zip/i.test(contentType))
...
function downloadToRuntime(url, options = {}, callback) {
    ...
    const finish = (err, filePath, detail) => {
        ...
        if (err && filePath) {
            ...
            catch { /* non-critical: partial download cleanup */ }
        }
        if (callback)
    ...
    try {
        ...
        finish(new Error('下载地址无效'));
        return;
    }
    if (!['http:', 'https:'].includes(parsed.protocol)) {
        finish(new Error('只支持 http/https 下载地址'));
        return;
    }
    if (isBlockedDownloadHost(parsed.hostname)) {
        finish(new Error('blocked: private or local download host'));
        return;
    }
    try {
        ...
        finish(new Error('准备本地部署工作目录失败：' + describeFsError(e)));
        return;
    }
...
function diagnoseNpmProxy(diagnostics = {}) {
    ...
    for (const item of loopback) {
        ...
        if (portState.status !== 'occupied')
            staleLoopback.push({ ...item, portState });
    }
...
function buildNpmInstallFailureGuide(logLines = [], diagnostics = null) {
    ...
    if (/EAI_AGAIN|ENOTFOUND/i.test(text))
        return { code: 'NPM_DNS_FAILED', title: 'npm 域名解析失败', summary: 'npm 无法解析 registry 域名，通常是 DNS、网络或代理配置问题。', fixSteps: ['确认电脑可以打开 npm registry 或 npm 镜像源网站。', '切换网络或 DNS 后重试。', '如果使用代理，请确认代理软件已启动。'], commands: [formatLocalNpmCommand(['config', 'set', 'registry', 'https://registry.npmmirror.com'])], diagnostics: diag };
    if (/ETIMEDOUT|ESOCKETTIMEDOUT|network timeout/i.test(text))
        return { code: 'NPM_TIMEOUT', title: 'npm 下载超时', summary: 'npm registry 响应太慢或网络被代理/防火墙阻断。', fixSteps: ['先确认网络稳定。', '可以切换到 npm 镜像源后重试。', '如果使用代理，请确认代理软件运行正常。'], commands: [formatLocalNpmCommand(['config', 'set', 'registry', 'https://registry.npmmirror.com'])], diagnostics: diag };
    if (/SELF_SIGNED_CERT|CERT_HAS_EXPIRED|unable to verify the first certificate/i.test(text))
        return { code: 'NPM_CERT_FAILED', title: 'npm 证书校验失败', summary: '网络代理或证书环境让 npm 无法校验证书。', fixSteps: ['优先检查代理软件的 HTTPS 解密/证书设置。', '确认系统时间正确。', '不要随意关闭 strict-ssl，除非你明确知道当前网络环境需要这样做。'], commands: [formatLocalNpmCommand(['config', 'get', 'strict-ssl'])], diagnostics: diag };
    if (/\bEACCES\b|\bEPERM\b|permission denied/i.test(text))
        return { code: 'NPM_PERMISSION_FAILED', title: 'npm 写入文件失败', summary: 'npm 没有权限写入项目目录，或文件正被其他进程占用。', fixSteps: ['关闭正在占用项目目录的终端、编辑器或杀毒拦截。', '确认部署器所在目录可写，不要放在 Program Files 等系统目录。', '重新打开部署器后再试一次。'], commands: [], diagnostics: diag };
    if (/\bE401\b|\bE403\b|unauthorized|forbidden/i.test(text))
        return { code: 'NPM_AUTH_FAILED', title: 'npm registry 权限错误', summary: '当前 registry 拒绝访问，可能是私有源认证过期或 registry 配错。', fixSteps: ['检查 npm registry 是否应为公开源。', '如果不需要私有源，切换到 npm 镜像源后重试。'], commands: [formatLocalNpmCommand(['config', 'get', 'registry']), formatLocalNpmCommand(['config', 'set', 'registry', 'https://registry.npmmirror.com'])], diagnostics: diag };
    if (/npm error/i.test(text))
...
function readLocalDeployManifest() {
    try {
        return JSON.parse(fs.readFileSync(LOCAL_DEPLOY_MANIFEST_FILE, 'utf8'));
    }
    catch {
...
function getNapcatLoginHint() {
    ...
    if (/Usage:\s*\.\\NapCatWinBootMain\.exe\s+<quickLogin>|Error Code:\s*2|Process Path:.*QQ\.exe/i.test(lines))
        return { status: 'failed', reason: 'NapCat 启动入口失败：当前包可能缺少 bootmain/QQ.exe，或启动脚本缺少 quickLogin 参数。请重新安装官方 Windows 包后重试。' };
    if (/登录成功|已登录|login\s+success|account.*online/i.test(lines))
        return { status: 'ok', reason: '日志显示 NapCat 已登录' };
    if (/二维码|扫码|qrcode|scan|login/i.test(lines))
        return { status: 'waiting', reason: 'NapCat 已启动，等待扫码或登录确认' };
...
function getLocalKoishiDeployStatus() {
    const target = getLocalDeployTarget();
    if (!target.canRunWindowsLocalDeploy)
        return getBlockedLocalTaskStatus('koishi', { port: { available: false, status: 'unsupported', reason: target.blockedReason }, loaded: false, url: '' });
...
function buildLocalConfigPreview() {
    ...
    for (const rel of ['koishi.yml', 'start-local.bat']) {
        if (!byPath.has(rel))
            byPath.set(rel, { path: rel, deleteByDefault: true, kind: 'config', reason: '标准本地部署文件' });
    }
    if (fs.existsSync(LOCAL_DEPLOY_MANIFEST_FILE))
        byPath.set(toProjectRel(LOCAL_DEPLOY_MANIFEST_FILE), { path: toProjectRel(LOCAL_DEPLOY_MANIFEST_FILE), deleteByDefault: true, kind: 'manifest', reason: '本地部署清单' });
    for (const item of byPath.values()) {
        ...
        catch { /* non-critical: config preview missing file */ }
        if (!stat) {
            files.push({ path: item.path, action: 'missing', reason: '文件不存在' });
            continue;
        }
        if (!stat.isFile()) {
            files.push({ path: item.path, action: 'keep', size: stat.size, reason: '不是普通文件' });
            continue;
        }
        if (item.sensitive || item.deleteByDefault === false) {
            files.push({ path: item.path, action: 'keep', size: stat.size, reason: '受保护文件' });
            continue;
        }
        const currentHash = fileSha256(filePath);
        if (item.sha256 && currentHash && item.sha256 !== currentHash) {
            files.push({ path: item.path, action: 'keep', size: stat.size, reason: '文件已被手动修改，默认保留', sha256: currentHash });
            continue;
        }
...
function findFilesRecursive(root, matcher, maxDepth = 6, maxCount = 600) {
    ...
    function walk(dir, depth) {
        if (depth > maxDepth || count > maxCount)
            return;
        let entries = [];
        try {
    ...
    try {
        for (const entry of fs.readdirSync(rDir, { withFileTypes: true })) {
            if (entry.isDirectory() && entry.name.startsWith(prefix)) {
                try {
                    removePathWithRetry(path.join(rDir, entry.name));
                }
                catch { /* non-critical: staging cleanup best effort */ }
            }
        }
    }
...
function extractZipArchive(archivePath, destinationDir) {
    if (!fs.existsSync(archivePath))
        throw new Error('解压源文件不存在：' + archivePath);
    const stat = fs.statSync(archivePath);
    if (!stat.isFile())
        throw new Error('解压源路径不是文件：' + archivePath);
    if (stat.size <= 0)
        throw new Error('解压源文件为空：' + archivePath);
    if (!hasZipMagic(archivePath))
        throw new Error('解压源文件不是有效 zip 包：' + archivePath);
    ...
    try {
        ...
        catch { /* non-critical: failed extraction cleanup */ }
...
function runNapcatInstallerIfPresent(stagingDir) {
    ...
    if (!installers.length)
        return { ran: false, ok: false, reason: '未找到 NapCatInstaller.exe，可手动运行解压目录内的安装器' };
...
function downloadNapcatWindowsRelease(installDir, callback) {
    httpsGetJson('https://api.github.com/repos/NapNeko/NapCatQQ/releases/latest', (apiErr, release) => {
        ...
        const releaseInfo = release && typeof release === 'object' ? release : {};
        const asset = pickNapcatWindowsAsset(releaseInfo);
        if (!asset?.browser_download_url) {
            return callback(new Error('未找到可自动安装的 Windows zip 资产' + ((releaseInfo.assets || []).map(item => item.name).filter(Boolean).join(', ') ? '，候选：' + (releaseInfo.assets || []).map(item => item.name).filter(Boolean).join(', ') : '')));
        }
        downloadToRuntime(asset.browser_download_url, { preferredName: asset.name, expectedExt: '.zip', minBytes: 128 * 1024 }, (downloadErr, filePath, download) => {
            ...
            try {
                ...
                if (!content.length)
                    throw new Error('NapCat zip 解压后目录为空');
                ...
                catch { /* non-critical: failed install cleanup */ }
                ...
                try {
                    removePathWithRetry(stagingDir);
                }
                catch { /* non-critical: staging cleanup best effort */ }
            }
        });
...
function pickNodeWindowsRelease(releases) {
    ...
    if (!arch)
        throw new Error('当前架构暂不支持自动安装便携 Node：' + process.arch);
...
function findExtractedNodeRoot(stagingDir) {
    ...
    for (const entry of entries) {
        if (entry.isDirectory() && fs.existsSync(path.join(stagingDir, entry.name, 'node.exe')))
...
function installPortableNodeWindows(callback) {
    if (process.platform !== 'win32')
        return callback(new Error('便携 Node/npm 自动安装只支持 Windows 本地部署器'));
    ...
    httpsGetJson('https://nodejs.org/dist/index.json', (apiErr, releases) => {
        ...
        downloadToRuntime(asset.url, { preferredName: asset.fileName, expectedExt: '.zip', minBytes: 1024 * 1024 }, (downloadErr, archivePath, download) => {
            ...
            try {
                ...
                if (!nodeRoot)
                    throw new Error('Node zip 解压后未找到 node.exe');
                ensureCleanDirectory(targetDir);
                copyRecursiveSync(nodeRoot, targetDir);
                for (const rel of ['node.exe', 'npm.cmd', 'npx.cmd']) {
                    if (process.platform === 'win32' && !fs.existsSync(path.join(targetDir, rel)))
                        throw new Error('便携 Node/npm 安装不完整，缺少：' + rel);
                }
                const node = getCommandInfo('node', 18);
                const npm = getCommandInfo('npm');
                if (!node.ok || !node.ownedByProject)
                    throw new Error('便携 Node 校验失败：' + (node.reason || 'node 不可用'));
                if (!npm.found || !npm.ownedByProject)
                    throw new Error('便携 npm 校验失败：' + (npm.reason || 'npm 不可用'));
                ...
                catch { /* non-critical: failed node install cleanup */ }
...
function prepareNpmInstallRun(options = {}) {
    ...
    if (shouldClean)
        repair.actions = repairNpmProxyConfig(env);
