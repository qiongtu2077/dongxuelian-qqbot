...
function buildLocalUninstallPreview() {
  ...
  if (fs.existsSync(DATA_DIR) && isInsidePath(KOISHI_DIR, DATA_DIR)) {
    ...
    for (const entry of entries) {
      ...
      if (/(?:key|token|pwd|password|cookie)/i.test(entry.name)) { const rel = path.posix.join('data', entry.name); excludedData.add(rel); keyTargets.push(projectTarget(rel)) }
    }
  }
...
function pruneEmptyProjectDirs(removeWorkspaceRoot = false) {
  ...
  if (removeWorkspaceRoot && isPackagedLocalWorkspace()) { try { fs.rmdirSync(path.resolve(KOISHI_DIR)) } catch {} }
}
