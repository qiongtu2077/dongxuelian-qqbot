...
interface ContextLike {
  ...
  middleware(handler: MiddlewareLike, prepend?: boolean | { prepend?: boolean }): unknown
  logger(name: string): LoggerLike
}

interface DefenseSessionLike {
  selfId?: string | number
  bot?: { selfId?: string | number }
  isDirect?: boolean
  content?: string
...
// 从消息源码中提取所有 @ 的用户 ID。
function extractAtIds(content: string = ''): string[] {
  const ids: string[] = []
...
// 判断当前消息是否直接 @ 了机器人。
function isDirectAtBot(session: DefenseSessionLike): boolean {
...
function detectComboAttack(plain: string = ''): string {
...
// 命中明确的高置信攻击规则。
function detectDirectAttack(plain: string = ''): string {
...
// 对需要机器人回应的消息执行前置防护检测。
function detectDefenseReply(session: DefenseSessionLike): string {
...
function apply(ctx: ContextLike): void {
  ...
  ctx.middleware(async (session, next) => {
    ...
    if (response) {
      await session.send(response).catch(() => { /* non-critical: failed defense reply should not reject middleware */ })
      return
    }
    return next()
  }, true)
...
export = { name, apply, promptDefense, promptDefenseAbusive }
