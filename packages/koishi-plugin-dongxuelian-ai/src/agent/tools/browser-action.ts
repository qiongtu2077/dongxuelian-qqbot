...
function ignoreBrowserPromiseFailure(error: unknown): void {
  void error
}

function formatBrowserArtifactResult(label: string, file: string, stat: { size?: number } | null = null): string {
  const name = path.basename(file)
  const size = stat && Number.isFinite(Number(stat.size)) ? `${Number(stat.size)} bytes` : '大小未知'
  return `${label}已保存（${name}，${size}）`
}

function warnBrowserActionBackgroundFailure(action: string, error: unknown): void {
...
function asBrowserActionParams(value: unknown): BrowserActionParams {
...
async function validateUrl(raw: unknown): Promise<string> {
  ...
  let originalHash = ''
  try { originalHash = new URL(value).hash } catch { /* non-critical: core URL validator reports malformed URL below */ }
  const parsed = validatePublicHttpUrl(value)
  if (originalHash) parsed.hash = originalHash
  try {
    await resolveAndValidateHostname(parsed)
  } catch (error) {
    if (/拒绝访问/.test(getBrowserActionErrorMessage(error))) throw error
  }
  return parsed.toString()
}
...
async function getSnapshot() {
  ...
  const snapshot = await p.evaluate(() => {
    ...
    const buttons = Array.from(document.querySelectorAll('button,input[type="button"],input[type="submit"]')).slice(0, 40).map((el, index) => {
      const item = el as HTMLElement & { value?: string }
...
async function extractSearchResults(query: unknown): Promise<string> {
  ...
  const candidates = await p.evaluate((): BrowserSearchCandidate[] => {
    ...
    function pickSnippet(item: Element, titleText: string): string {
      ...
      return cleanText((item as HTMLElement).innerText).replace(titleText, '').trim().slice(0, 420)
    }
    const selectors = [
      '#b_results li.b_algo',
      '#b_results .b_algo',
    ...
    const fromContainers = items.map(item => {
      ...
      const titleEl = item.querySelector('h2 a, h3 a, .result__a, .t a') as HTMLAnchorElement | null
      const titleLink = titleEl || links.find(a => ((a as HTMLAnchorElement).innerText || '').trim().length > 8) as HTMLAnchorElement | undefined || links[0] as HTMLAnchorElement | undefined
      ...
      return {
        ...
        snippet: cleanText(snippetEl && (snippetEl as HTMLElement).innerText) || pickSnippet(item, title),
        text: cleanText((item as HTMLElement).innerText),
      }
    })
    const fromLinks = Array.from(document.querySelectorAll('a[href]')).map(a => {
      ...
      return {
        ...
        text: container ? cleanText((container as HTMLElement).innerText) : title,
      }
    })
...
async function dragElement(params: BrowserActionParams = {}) {
  const p = await ensurePage()
  const selector = requireSelector(params.selector)
...
async function scrollPage(params: BrowserActionParams = {}) {
  const p = await ensurePage()
...
async function inspectDom(action: string, params: BrowserActionParams = {}) {
  const p = await ensurePage()
  const selector = requireSelector(params.selector)
  ...
  if (action === 'extract') {
    ...
    const items = await p.$$eval(selector, (els, max) => els.slice(0, max).map(el => {
      const item = el as HTMLElement & { href?: string; value?: string }
...
async function setViewport(params: BrowserActionParams = {}) {
  const p = await ensurePage()
...
async function savePdf(params: BrowserActionParams = {}) {
  const p = await ensurePage()
  const dir = path.join(DATA_DIR, 'agent-browser')
  fs.mkdirSync(dir, { recursive: true })
...
async function manageCookies(action: string, params: BrowserActionParams = {}) {
  const p = await ensurePage()
...
async function uploadFile(params: BrowserActionParams = {}) {
  const p = await ensurePage()
  const selector = requireSelector(params.selector)
...
async function configureDownload(params: BrowserActionParams = {}) {
  const p = await ensurePage()
...
async function clearBrowserCache() {
  ...
  if (failures.length) throw new Error(`浏览器缓存清理不完整：${failures.join('; ')}`)
  networkLog = []
  consoleLog = []
...
async function executeSingleAction(params: BrowserActionParams = {}): Promise<string> {
  const action = String(params.action || '').trim().toLowerCase()
  ...
  if (action === 'html') return getHtml(String(params.selector || ''))
...
async function runBatch(steps: unknown = []): Promise<string> {
...
async function fillForm(params: BrowserActionParams = {}): Promise<string> {
  const p = await ensurePage()
  const fields = Array.isArray(params.fields) ? params.fields as BrowserFormField[] : []
  if (!fields.length) {
  ...
  for (const field of fields) {
    ...
    await p.$eval(selector, el => { if ('value' in el) el.value = ''; else el.textContent = '' })
    await p.type(selector, value, { delay: 2 })
  }
  return `已填写表单字段：${fields.length} 个`
}

function runQueued(action: () => Promise<string>): Promise<string> {
