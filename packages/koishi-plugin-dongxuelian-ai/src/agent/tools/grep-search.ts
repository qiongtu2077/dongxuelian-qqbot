...
function grepWildcardToRegExp(pattern: unknown = '*'): RegExp {
  const escaped = String(pattern || '*').replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*').replace(/\?/g, '.')
  return new RegExp('^' + escaped + '$', 'i')
}

async function collectFiles(dir: string, matcher: RegExp, out: string[], maxFiles: number = 2000): Promise<void> {
...
export = {
  ...
  async execute(params: GrepSearchParams = {}): Promise<string> {
    ...
    const stat = await fs.stat(checked.abs) as FileStatLike
    const files: string[] = []
