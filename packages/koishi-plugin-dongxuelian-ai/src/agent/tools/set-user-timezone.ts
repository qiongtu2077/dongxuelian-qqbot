...
const TZ_RE: RegExp = /^[A-Za-z]+(?:[ _-][A-Za-z]+)*(?:\/[A-Za-z0-9_+.-]+)+$/
const MAX_TZ_FILE_BYTES = 128 * 1024
const MAX_TZ_ENTRIES = 2000

async function readMap(): Promise<Record<string, string>> {
  try {
    const stat = await fs.stat(TZ_FILE) as FileStatLike
    if (!stat.isFile() || stat.size > MAX_TZ_FILE_BYTES) return {}
    const data = JSON.parse(await fs.readFile(TZ_FILE, 'utf8')) as Record<string, unknown>
    const result: Record<string, string> = {}
    for (const [key, value] of Object.entries(data)) result[key] = String(value || '')
    return result
  } catch {
    /* non-critical: timezone preference file is optional */
    return {}
  }
}
...
export = {
  ...
  async execute(params: SetUserTimezoneParams = {}): Promise<string> {
