...
const TZ_FILE: string = path.join(DATA_DIR, 'agent-user-timezones.json')
const MAX_TZ_FILE_BYTES = 128 * 1024

function getTimezoneValue(value: unknown): string {
  return String(value || '')
}

function getUserTimezone(userId: unknown): string {
  if (!userId) return ''
  try {
    const stat = fs.statSync(TZ_FILE)
    if (!stat.isFile() || stat.size > MAX_TZ_FILE_BYTES) return ''
    const data = JSON.parse(fs.readFileSync(TZ_FILE, 'utf8')) as Record<string, unknown>
    return String(data[String(userId)] || '')
  } catch {
    /* non-critical: timezone preference is optional */
    return ''
  }
}
...
export = {
  ...
  async execute(params: GetTimeParams = {}): Promise<string> {
    const tz = getTimezoneValue(params.timezone) || getUserTimezone(params.userId) || 'Asia/Shanghai'
    const now = new Date()
