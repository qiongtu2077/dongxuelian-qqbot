...
const { checkShellCommand, summarizeShellCommand } = require('./shell-guard') as typeof import('./shell-guard')
...
interface ShellGuardError extends Error {
  code?: string
...
function formatGuardError(command: string, guardResult: ReturnType<typeof checkShellCommand>): ShellGuardError {
...
function buildSafeShellEnv(source: NodeJS.ProcessEnv = {}): ShellEnv {
...
export = {
  ...
  async execute(params: ShellParams = {}): Promise<string> {
    ...
    return new Promise(resolve => {
      const child = execFile(
        ...
        (err: ExecFileError | null, stdout: string, stderr: string) => {
          const parts: string[] = []
