...
type JsonishValue = string | number | boolean | null | JsonishValue[] | { [key: string]: JsonishValue | undefined }

const BLOCKED: RegExp = /\b(?:require|import|process|child_process|fs|global|globalThis|__dirname|__filename|module|exports|Function|eval)\b/
const MAX_CODE_CHARS = 12000
const MAX_RESULT_CHARS = 8000
const MAX_JSONIFY_CHARS = 64000

function safeStringifyResult(result: unknown): string | undefined {
  const seen = new WeakSet()
  return JSON.stringify(result === undefined ? null : result, (_key: string, value: JsonishValue) => {
...
export = {
  ...
  async execute(params: ExecuteJavascriptParams = {}): Promise<string | undefined> {
