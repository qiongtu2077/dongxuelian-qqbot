...
function getCalculatorErrorMessage(error: unknown): string {
...
export = {
  ...
  async execute(params: CalculateToolParams = {}): Promise<string> {
    ...
    const normalized = clean.replace(/Math\.([a-zA-Z]+)\(/g, (_match: string, name: string) => {
