...
interface AgentSkillReadResult {
  name: string
  kind: string
  description?: string
  file: string
...
const { readAgentSkill, findRelevantAgentSkills } = require('../skills') as typeof import('../skills')
const { getEnabledSkills } = require('../config') as typeof import('../config')

function isSkillEnabled(name: unknown, context: ReadAgentSkillContext = {}): boolean {
...
export = {
  ...
  async execute(params: ReadAgentSkillParams = {}, context: ReadAgentSkillContext = {}): Promise<string> {
