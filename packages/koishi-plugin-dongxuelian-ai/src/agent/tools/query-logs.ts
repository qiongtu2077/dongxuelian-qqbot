...
const LOG_DIRS: string[] = [path.resolve(process.cwd(), 'runtime', 'logs'), path.join(DATA_DIR, 'logs')]
const FALLBACK_LOG_FILE: string = path.resolve(process.cwd(), 'koishi.log')
...
function isUnsafeRegexQuery(query: string = ''): boolean {
  return /([+*{])\1|\(\?[^)]*[+*]|\([^)]*[+*?{][^)]*\)[+*?{]|\[[^\]]*\][+*?{]\s*[+*?{]/.test(String(query || ''))
}

function buildQueryRegExp(query: string = ''): RegExp {
  try {
    if (isUnsafeRegexQuery(query)) throw new Error('unsafe regex')
    return new RegExp(query, 'i')
  } catch {
    /* non-critical: invalid user regex falls back to literal search */
    return new RegExp(escapeRegExp(query), 'i')
  }
}

async function collectLogFiles(): Promise<string[]> {
  const files: string[] = []
  for (const dir of LOG_DIRS) {
    let entries: DirentLike[] = []
    try { entries = await fs.readdir(dir, { withFileTypes: true }) } catch {
      /* non-critical: missing log directory is skipped */
      continue
    }
  ...
  try {
    const st = await fs.stat(FALLBACK_LOG_FILE) as FileStatLike
    if (st.isFile() && !files.includes(FALLBACK_LOG_FILE)) files.push(FALLBACK_LOG_FILE)
  } catch {
    /* non-critical: fallback log file is optional */
  }
  return files
}
...
async function readLogTail(file: string, stat: FileStatLike): Promise<string> {
...
export = {
  ...
  async execute(params: QueryLogsParams = {}): Promise<string> {
    const hasQuery = typeof params.query === 'string' && params.query.trim().length > 0
    const query = hasQuery ? String(params.query).trim() : String(params.level || '')
    const limit = Math.max(1, Math.min(200, parseInt(String(params.limit), 10) || 80))
