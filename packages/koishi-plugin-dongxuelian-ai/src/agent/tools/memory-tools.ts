...
function getUserId(context: MemoryToolContext = {}): string {
  return String(context.userId || 'dashboard')
}

function checkMemoryAccess(context: MemoryToolContext = {}): string | null {
  const cfg: AgentMemoryConfig['memory'] = getAgentConfig().memory || {}
...
const rememberMemoryTool = {
  ...
  async execute(params: MemoryToolParams = {}, context: MemoryToolContext = {}): Promise<string> {
