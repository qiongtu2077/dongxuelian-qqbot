...
const { compilePersonaRuntimePlan, resolvePersonaRuntimePlan } = require('../persona/persona-runtime-plan') as typeof import('../persona/persona-runtime-plan')
...
function parseAgentPersonaPositiveInt(value: unknown, fallback: number, min: number, max: number): number {
...
function removeAgentFrontmatter(text: unknown = ''): string {
  return String(text || '').replace(/^---\r?\n[\s\S]*?\r?\n---\s*/, '').trim()
}

function loadAgentPromptFile(file: string): string {
  try {
    const stat = fs.statSync(file)
    if (!stat.isFile() || stat.size > MAX_AGENT_PERSONA_FILE_BYTES) return ''
    const content = fs.readFileSync(file, 'utf8').trim()
    return content ? removeAgentFrontmatter(content) : ''
  } catch { /* non-critical: optional agent prompt file may be absent or unreadable */
    return ''
  }
}

function loadAgentSkillFileByName(dir: string, fileName: string): string {
  return loadAgentPromptFile(path.join(dir, fileName))
}

function loadAgentNamedPrompt(dir: string, wantedName: unknown, fallbackFile?: string): string {
  ...
  } catch { /* non-critical: optional prompt directory falls back to default prompt file */ }
  return fallback
}

function getAgentDefaultPersonaPrompt(): string {
...
function extractAgentPersonaLore(personaContent: unknown = '', personaName: unknown = '', plan: PersonaRuntimePlanLike | null = null): string {
  if (plan && plan.lore) {
...
function buildAgentPersonaSystemMessage({ personaName = '', personaContent = '', source = 'default', channel = 'qq', plan = null }: BuildAgentPersonaSystemMessageOptions = {}): string {
  const core = loadAgentNamedPrompt(SKILLS_CORE_DIR, 'persona-core', 'SKILL.persona-core.md')
  const runtimePlan: PersonaRuntimePlanLike = plan || (personaContent
    ? compilePersonaRuntimePlan({ personaName: String(personaName || ''), personaContent: String(personaContent || ''), source: String(source || ''), type: personaName ? 'persona' : 'default' })
    : resolvePersonaRuntimePlan({ personaName: String(personaName || ''), source: String(source || '') }))
  const personaBody = String(runtimePlan.prompt?.body || '')
...
function buildAgentPersonaContext(options: BuildAgentPersonaContextOptions = {}): AgentSystemMessage[] {
  ...
  const messages: AgentSystemMessage[] = []
  if (agentMode) {
    messages.push(
...
function mergeAgentSystemExtra(...groups: unknown[]): AgentSystemMessage[] {
  const result: AgentSystemMessage[] = []
  for (const group of groups) {
    if (!Array.isArray(group)) continue
    for (const item of group) {
      if (item && typeof item === 'object' && (item as { role?: unknown }).role === 'system' && typeof (item as { content?: unknown }).content === 'string' && (item as { content: string }).content.trim()) {
        result.push({ role: 'system', content: (item as { content: string }).content })
      }
    }
  }
  return result
...
function listAgentPersonasForConsole(): AgentPersonaConsoleEntry[] {
  const personas: AgentPersonaConsoleEntry[] = []
  try {
    ...
    for (const entry of entries) {
      ...
      if (meta.name) {
        personas.push({
          ...
          file: entry.name,
          lore: String(meta.lore || ''),
        })
      }
    }
  } catch { /* non-critical: console persona list is empty when persona directory is unavailable */ }
  return personas
}
