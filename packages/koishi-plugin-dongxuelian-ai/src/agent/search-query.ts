...
type ScoredSearchCandidate<T extends SearchCandidate> = T & { score: number }

const TRUSTED_DOMAIN_RULES: TrustedDomainRule[] = [
...
function cleanExplicitSearchQuery(text: unknown = ''): string {
  return String(text || '')
...
function isWuwaLatestRoleQuery(query: unknown = ''): boolean {
  const value = String(query || '')
  return WUWA_RE.test(value) && LATEST_ROLE_RE.test(value)
}

function pushSearchQuery(queries: string[], value: unknown): void {
  const query = String(value || '').replace(/\s+/g, ' ').trim()
  if (query) queries.push(query)
}

function isMinecraftUpdateQuery(query: unknown = ''): boolean {
  const value = String(query || '')
  return MINECRAFT_RE.test(value) && /(?:最新|当前|现在|更新|版本|update|release|snapshot|pre-release|正式版|latest|version)/i.test(value)
}

function isHotVideoQuery(query: unknown = ''): boolean {
  return HOT_VIDEO_RE.test(String(query || ''))
}

function isResourceVideoQuery(query: unknown = ''): boolean {
  return RESOURCE_VIDEO_RE.test(String(query || ''))
}

function buildSearchQueries(rawQuery: unknown = ''): string[] {
...
function getDirectSearchCandidates(query: unknown = ''): SearchCandidate[] {
  const value = String(query || '')
...
function getSearchHostname(url: unknown = ''): string {
  try {
...
function scoreSearchResult(item: SearchCandidate = {}, query: string = ''): number {
...
function isLowQualitySearchResult(item: SearchCandidate = {}): boolean {
...
function sortSearchResults<T extends SearchCandidate>(results: T[] = [], query: string = ''): Array<ScoredSearchCandidate<T>> {
  return results
