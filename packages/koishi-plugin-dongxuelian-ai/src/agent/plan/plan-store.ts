...
const PLAN_DIR: string = path.join(DATA_DIR, 'agent-plans')
const ACTIVE_FILE: string = path.join(PLAN_DIR, 'active.json')
...
function parsePlanStorePositiveInt(value: unknown, fallback: number, min: number, max: number): number {
  const parsed = parseInt(String(value), 10)
  if (!Number.isFinite(parsed)) return fallback
  return Math.max(min, Math.min(max, parsed))
}

function safePlanId(id: unknown = ''): string {
  return String(id || '').replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 80)
}

function buildPlanId(now: number = Date.now()): string {
  const date = new Date(now).toISOString().slice(0, 10).replace(/-/g, '')
  return `plan_${date}_${now.toString(36)}`
}

function getPlanFile(id: unknown): string {
...
function isPlanTaskState(value: unknown): value is PlanTaskState {
...
function toRawPlanTask(task: unknown): RawPlanTask {
  return task && typeof task === 'object' ? task as RawPlanTask : { desc: task }
}

function toRawPlan(plan: unknown): RawPlan {
  return plan && typeof plan === 'object' ? plan as RawPlan : {}
}

function normalizeTask(task: unknown, index: number): PlanTask {
...
async function readPlanJson<T>(file: string, fallback: T): Promise<T> {
  return readJsonFile<T>(file, fallback, { maxBytes: MAX_PLAN_FILE_BYTES })
}

async function writePlanJson(file: string, data: unknown): Promise<void> {
  await ensurePlanDir()
  await writeJsonFile(file, data)
}

async function savePlan(plan: unknown): Promise<Plan> {
  const raw = toRawPlan(plan)
...
async function loadPlan(id: unknown): Promise<Plan | null> {
  const safeId = safePlanId(id)
...
async function listPlans(limit: unknown = 50): Promise<Plan[]> {
  await ensurePlanDir()
  let files: string[] = []
  try {
...
async function readActiveIndex(): Promise<ActivePlanIndex> {
  const data = await readPlanJson<{ plans?: unknown[] }>(ACTIVE_FILE, { plans: [] })
  return { plans: Array.isArray(data.plans) ? data.plans.map(safePlanId).filter(Boolean) : [] }
}

async function updateActiveIndex(plan: Plan): Promise<void> {
...
async function listActivePlans(): Promise<Plan[]> {
  const index = await readActiveIndex()
  const result: Plan[] = []
...
function getPlanStorageInfo(): { dir: string; activeFile: string; exists: boolean } {
  return {
    dir: PLAN_DIR,
