...
const { SKILLS_DIR, SKILLS_LORE_DIR } = require('../core/constants') as typeof import('../core/constants')
const { ensureRuntimeSkillSeeds } = require('../persona/skills/skill-seeds') as typeof import('../persona/skills/skill-seeds')
const { SKILL_POOL_DIR } = require('./skills/store') as typeof import('./skills/store')
...
function parseAgentSkillPositiveInt(value: unknown, fallback: number, min: number, max: number): number {
  const parsed = parseInt(String(value), 10)
  if (!Number.isFinite(parsed)) return fallback
  return Math.max(min, Math.min(max, parsed))
}

function parseFrontmatter(text: unknown): SkillFrontmatter {
...
function stripFrontmatter(text: unknown = ''): string {
  return String(text || '').replace(/^---\r?\n[\s\S]*?\r?\n---\s*/, '').trim()
}

function skillNormalizeName(name: unknown = ''): string {
  return String(name || '').trim()
}

function skillNameKey(name: unknown = ''): string {
  return skillNormalizeName(name).toLowerCase()
}

function skillSearchText(skill: unknown = ''): string {
  if (!skill || typeof skill !== 'object') return ''
  const item = skill as Partial<AgentSkill>
  return [item.name, item.kind, item.description, item.path, ...(item.references || [])].join('\n').toLowerCase()
}

function skillQueryTerms(query: unknown = ''): string[] {
  return String(query || '')
    .toLowerCase()
...
function skillPathKey(value: unknown = ''): string {
...
function skillReadText(file: string): string {
  try {
    const stat = fs.statSync(file)
    if (!stat.isFile() || stat.size > MAX_SKILL_INDEX_FILE_BYTES) return ''
    return fs.readFileSync(file, 'utf8')
  } catch { /* non-critical: unreadable skill index file is skipped */ return '' }
}

function skillRelativePath(file: string): string {
  const relative = path.relative(SKILLS_DIR, file).replace(/\\/g, '/')
  return relative && !relative.startsWith('..') ? relative : path.basename(file)
}

function skillListReferences(rootDir: string, primaryFile: string): string[] {
  ...
  function walk(dir: string, depth: number): void {
    ...
    try { entries = fs.readdirSync(dir, { withFileTypes: true }) } catch { /* non-critical: unreadable reference directory is skipped */ return }
...
function skillBuildEntry(group: SkillGroup, file: string, options: SkillBuildOptions = {}): AgentSkill | null {
...
function skillCollectEntry(skills: AgentSkill[], group: SkillGroup, entry: import('fs').Dirent, baseDir: string): void {
  ...
  if (entry.isDirectory()) {
    ...
    let nested: import('fs').Dirent[] = []
    try { nested = fs.readdirSync(full, { withFileTypes: true }) } catch { /* non-critical: unreadable nested skill directory is skipped */ return }
...
function listAgentSkills(): AgentSkill[] {
  ensureRuntimeSkillSeeds()
  const skills: AgentSkill[] = []
  for (const group of SKILL_DIRS) {
    let entries: import('fs').Dirent[] = []
    try { entries = fs.readdirSync(group.dir, { withFileTypes: true }) } catch { /* non-critical: optional skill group directory may be absent */ continue }
  ...
  try {
    ...
    for (const entry of poolEntries) {
      ...
      const legacyFiles = (() => { try { return fs.readdirSync(path.join(SKILL_POOL_DIR, entry.name)).filter((f: string) => LEGACY_SKILL_RE.test(f)) } catch { /* non-critical: unreadable pool skill directory is skipped */ return [] as string[] } })()
      if (fs.existsSync(directorySkill)) {
  ...
  } catch { /* non-critical: optional skill pool directory may be absent */ }
  return skills.sort((a, b) => `${a.kind}:${a.name}`.localeCompare(`${b.kind}:${b.name}`, 'zh-Hans-CN'))
}

function findAgentSkill(name: unknown): AgentSkill | null {
  const target = skillNameKey(name)
  if (!target) return null
  return listAgentSkills().find(skill => skillNameKey(skill.name) === target) || null
}

function skillResolveRequestedFile(skill: AgentSkill, requestedFile: unknown = ''): string {
...
function readAgentSkill(name: unknown, options: ReadAgentSkillOptions = {}): ReadAgentSkillResult {
  ...
  const maxChars = Math.max(1000, Math.min(MAX_READ_CHARS, parseInt(String(options.maxChars), 10) || DEFAULT_READ_CHARS))
...
function findRelevantAgentSkills(query: unknown = '', options: FindRelevantAgentSkillsOptions = {}): AgentSkill[] {
  const queryText = String(query || '')
  const terms = skillQueryTerms(query)
  if (!terms.length) return []
  const limit = Math.max(1, Math.min(16, parseInt(String(options.limit), 10) || 8))
  return listAgentSkills()
    .map(skill => {
      ...
      if (/前端|dashboard|后台|控制台|源码|代码|文件|目录|agent|skill|技能|浏览器|browser|pdf|ppt|docx/i.test(queryText) && skill.name === 'QA_source_index') score += 6
      if (/浏览器|网页|页面|截图|browser|cdp/i.test(queryText) && /^browser_/i.test(skill.name)) score += 8
      if (/联网|搜索|搜|查(一下|找)?|最新|新闻|资讯|官网|官方|来源|网页|资料|web[_-]?search|search/i.test(queryText) && skill.name === 'web_search_strategy') score += 12
      if (/ppt|pptx|演示|幻灯片/i.test(queryText) && /^pptx$/i.test(skill.name)) score += 8
      if (/pdf/i.test(queryText) && /^pdf$/i.test(skill.name)) score += 8
      if (/docx|word|文档/i.test(queryText) && /^docx$/i.test(skill.name)) score += 8
      if (/读文件|读日志|看日志|查日志|查看文件|查看日志|log|文件内容|tail/i.test(queryText) && skill.name === 'file_reader') score += 10
      if (/计划|规划|分步骤|怎么做|步骤|方案|遇阻|放弃|换方法/i.test(queryText) && skill.name === 'make_plan') score += 10
      return { skill, score }
    })
...
function buildAgentSkillSummary(enabledNames: unknown[] = [], options: BuildAgentSkillSummaryOptions = {}): string {
