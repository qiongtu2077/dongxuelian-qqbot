...
const { listResourceTasks } = require('../resource-workers/task-store') as typeof import('../resource-workers/task-store')
...
function countActiveAgentWorkerTasks(kind: string, channelKey: string, userId: string): number {
...
// 返回用户可见的准入拒绝/延期提示。
function formatAdmissionBlockedMessage(admission: AdmissionDecisionLike | null | undefined, taskId = ''): string {
  const decision = String(admission?.decision || '')
  const reason = String(admission?.reason || decision || 'resource unavailable')
  if (decision === 'defer') return `当前资源紧张，Agent 任务已记录为延期任务${taskId ? `：${taskId}` : ''}。`
  if (decision === 'reject' || decision === 'silent_drop') return `当前资源不足，Agent 暂时不能执行：${reason}`
