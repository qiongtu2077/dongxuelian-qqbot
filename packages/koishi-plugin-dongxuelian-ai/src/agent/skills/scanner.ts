...
interface ScanRule {
  id: string
  category: string
  severity: Exclude<SkillSeverity, 'SAFE'>
  pattern: RegExp
  description: string
}
...
function getMaxSeverity(findings: SkillFinding[]): SkillSeverity {
  if (!findings.length) return 'SAFE'
  let max: SkillSeverity = 'SAFE'
...
function isSymlink(filePath: string): boolean {
  try {
    ...
    /* non-critical: unreadable path is treated as non-symlink and handled by stat/read guards */
    return false
  }
}
...
function canonicalWhitelistKey(dirPath: string): string {
  return path.resolve(dirPath)
}

function readWhitelistSync(): SkillWhitelist {
  try {
...
async function writeWhitelistAtomic(obj: SkillWhitelist): Promise<void> {
...
function isWhitelistedAndUnchanged(skillDir: string): boolean {
 ...
 */
async function addToWhitelist(dirPath: string, reason: unknown): Promise<void> {
 ...
 */
async function removeFromWhitelist(dirPath: string): Promise<void> {
  const key = canonicalWhitelistKey(dirPath)
  const data = readWhitelistSync()
...
function scanSkillDirectory(skillDir: string): SkillScanResult {
  ...
  for (const file of files) {
    ...
    try {
      ...
      /* non-critical: file disappeared during scan */
      continue
    }
    ...
    try {
      ...
      /* non-critical: unreadable file is skipped during bounded scan */
      continue
    }

    for (const rule of SCAN_RULES as ScanRule[]) {
...
function scanSkillFile(filePath: string): SkillScanResult {
