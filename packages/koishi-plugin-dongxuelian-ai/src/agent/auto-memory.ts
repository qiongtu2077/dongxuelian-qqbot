...
function getDailyFile(userId: unknown): string {
...
async function readDailyFile(userId: unknown): Promise<string> {
  ...
  } catch { /* non-critical: missing or unreadable daily memory file only removes duplicate context */
    return ''
  }
}
...
async function getDailyTotalSize(userId: unknown): Promise<number> {
  try {
    ...
    return total
  } catch { /* non-critical: no daily memory directory means size is zero */
    return 0
  }
}

async function extractMemory(recentMessages: AutoMemoryMessage[], userId: unknown): Promise<string | null> {
...
async function onAgentReplyComplete({ userId, channel, messages }: AgentReplyCompleteInput = {}): Promise<void> {
  ...
  try {
    ...
    if (totalSize > 20 * 1024) {
      try {
        const { runDreamIfNeeded } = require('./dream') as typeof import('./dream')
        await runDreamIfNeeded(userId)
      } catch { /* non-critical: dream consolidation can retry on a later auto-memory trigger */ }
    }
  } catch { /* non-critical: automatic memory extraction must never block or fail the user reply */ }
}
