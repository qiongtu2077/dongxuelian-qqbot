...
const WORKSPACE_ALIASES: WorkspaceAlias[] = [
  {
    name: 'Dashboard 后台',
...
const PATH_NOISE_SUFFIXES: string[] = ['文件夹', '目录', '文件', '里面', '内容']
...
function pushUniqueCandidate(out: WorkspaceCandidate[], item: WorkspaceCandidate): void {
...
function pathHasExistingParent(candidate: string): boolean {
...
function joinExistingRoot(root: string, relativePath: string): string {
  return path.resolve(root, relativePath.replace(/[\\/]+/g, path.sep))
}

function getAliasMatches(text: unknown = ''): WorkspaceAlias[] {
  const normalized = normalizeIntentText(text)
  if (!normalized) return []
  return WORKSPACE_ALIASES.filter(alias => alias.terms.some(term => normalized.includes(normalizeIntentText(term))))
}

function getAliasPathCandidates(value: unknown = '', roots: string[] = []): WorkspaceCandidate[] {
...
function resolveAgentPathInput(value: unknown = '', roots: string[] = [], options: ResolveAgentPathOptions = {}): WorkspaceCandidate {
...
function getWorkspaceSemanticCandidates(text: unknown = '', roots: string[] = []): WorkspaceCandidate[] {
...
function formatWorkspaceContext(candidates: WorkspaceCandidate[] = [], roots: string[] = []): string {
  const lines = [
    '【控制台工作区-内部参考】',
...
async function buildAgentWorkspaceContext({ userMessage = '', channel = 'qq', roots = [] }: BuildAgentWorkspaceContextInput = {}): Promise<WorkspaceSystemMessage[]> {
