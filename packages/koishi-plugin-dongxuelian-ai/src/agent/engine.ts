...
const { getToolDefinitions, executeTool, toolRegistry } = require('./tools/registry') as typeof import('./tools/registry')
const { estimateTokens, externalizeToolResult, compactWithLLM } = require('./context') as typeof import('./context')
const { recordCall } = require('./stats') as typeof import('./stats')
const safety = require('./safety') as typeof import('./safety')
const pending = require('./pending') as typeof import('./pending') & { summarizePendingArgs?: (toolName: string, args: unknown) => string }
const { isChannelEnabled, isToolEnabled, getEnabledSkills } = require('./config') as typeof import('./config')
const { buildAgentSkillSummary } = require('./skills') as typeof import('./skills')
const { buildAgentMessages } = require('./messages') as typeof import('./messages')
const { buildAgentPersonaContext, mergeAgentSystemExtra } = require('./persona-context') as typeof import('./persona-context')
const { buildAgentWorkspaceContext } = require('./workspace-context') as typeof import('./workspace-context')
const { getAgentPathAllowedRoots } = require('./path-guard') as typeof import('./path-guard')
const { recordAgentSession } = require('./sessions') as typeof import('./sessions')
const { onAgentReplyComplete } = require('./auto-memory') as typeof import('./auto-memory')
const { MAX_TOOL_ROUNDS } = require('../core/constants') as typeof import('../core/constants')
const { getRecentFilesCached } = require('../media/file/file-store') as typeof import('../media/file/file-store')

type ApiConfig = Awaited<ReturnType<typeof loadConfig>>
...
interface RoundSummary {
  ...
  toolResults: Array<{ name: string; result: string; status: string }>
}

interface FallbackToolLike {
  name: string
  args?: unknown
}

interface ExecuteAgentToolCallInput {
  tc: AgentToolCall
...
type ToolCallOutcome =
  | { status: 'pending'; reply: string; pendingId: string; toolCount: number; result?: undefined; fallbackCall?: undefined }
  | { status: 'done'; result: unknown; toolCount: number; reply?: undefined; pendingId?: undefined; fallbackCall?: undefined }
  | { status: 'fallback'; result: unknown; fallbackCall: AgentToolCall; toolCount: number; reply?: undefined; pendingId?: undefined }

interface ContinueAgentInput {
  messages: AgentMessage[]
  config: ApiConfig
...
const EXTERNAL_TOOL_NAMES: Set<string> = new Set(['web_search', 'web_fetch', 'browser_action'])
const EMPTY_AGENT_REPLY_RE = /^\(Agent 未获取到有效回复\)/i

function isFallbackTool(value: unknown): value is FallbackToolLike {
  return !!value && typeof value === 'object' && typeof (value as { name?: unknown }).name === 'string'
}

function getAgentEngineErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : String(error || '')
}

function asApiMessages(messages: AgentMessage[]): Parameters<typeof requestChatCompletions>[0] {
  return messages as unknown as Parameters<typeof requestChatCompletions>[0]
}

function asToolDefinitions(tools: ToolDefinition[]): Parameters<typeof requestChatCompletions>[3] {
  return tools as unknown as Parameters<typeof requestChatCompletions>[3]
}

function normalizeToolDefinitions(tools: ReturnType<typeof getToolDefinitions>): ToolDefinition[] {
...
function buildFileHintContext(channelKey: string): FileHintMessage[] {
...
function normalizeToolCall(toolName: string, args: unknown = {}): AgentToolCall {
  return {
...
function parseAgentEngineToolArguments(raw: string): unknown {
  try { return JSON.parse(raw || '{}') } catch { /* non-critical: invalid tool arguments fall back to empty object */
    return {}
  }
}

async function executeAgentToolCall({ tc, messages, allowedToolNames, channel, channelKey, userId, userName, userMessage, toolCount, bot, isAdmin = false, resourceTaskId = '' }: ExecuteAgentToolCallInput): Promise<ToolCallOutcome> {
  ...
  try {
    ...
    const execResult = await executeTool(toolName, args as Record<string, unknown>, { channel, channelKey, userId, userName, userMessage, bot, isAdmin, resourceTaskId, taskId: resourceTaskId })
...
function compressOldToolResults(messages: AgentMessage[], currentRound: number): void {
  let toolMsgCount = 0
...
function toAgentToolCalls(toolCalls: unknown): AgentToolCall[] {
  return Array.isArray(toolCalls) ? toolCalls as AgentToolCall[] : []
}

function normalizeAgentFinalReply(text: unknown = ''): string {
  const value = String(text || '').trim()
  if (!value || EMPTY_AGENT_REPLY_RE.test(value)) return ''
  return value
}

async function continueAgent({ messages, config, tools, allowedToolNames, channel, channelKey, userId, userName, userMessage, toolCount = 0, toolResults = [], onProgress, bot, enableThinking = false, isAdmin = false, resourceTaskId = '' }: ContinueAgentInput): Promise<AgentRunResult> {
  let reply = ''
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    const roundToolResults: Array<{ name: string; result: string; status: string }> = []

    for (const tc of activeToolCalls) {
      let currentCall: AgentToolCall | null = tc
      let fallbackDepth = 0
    ...
    if (estimated > 60000) {
      messages.splice(0, messages.length, ...await compactWithLLM(messages, config as unknown as Record<string, unknown>, requestAgentCompaction) as AgentMessage[])
  ...
  if (!reply && toolResults.length > 0) {
    try {
      const synthMessages: AgentMessage[] = [
        ...messages,
 ...
 */
function ensureToolDefinition(tools: ToolDefinition[], toolName: string): ToolDefinition[] {
...
function getForceToolSet(forceTools: unknown, channel: string = 'qq'): Set<string> {
...
function normalizeContextPolicy(policy: unknown = {}): ContextPolicy {
  if (!policy || typeof policy !== 'object' || Array.isArray(policy)) return { allowExternalTools: true, allowedTools: [] }
  const record = policy as { allowExternalTools?: unknown; allowedTools?: unknown }
...
function applyContextPolicyToTools(tools: ToolDefinition[] = [], contextPolicy: unknown = {}): ToolDefinition[] {
...
function getScheduledContextPolicy(scheduledTask: unknown): unknown {
  return scheduledTask && typeof scheduledTask === 'object' && !Array.isArray(scheduledTask)
    ? (scheduledTask as { contextPolicy?: unknown }).contextPolicy
    : undefined
}

function normalizePreExecuteTools(value: unknown): Array<{ name?: string; args?: unknown }> {
  return Array.isArray(value) ? value as Array<{ name?: string; args?: unknown }> : []
}

async function runAgent({ userMessage, userName, userId, channelKey, channel = 'qq', systemExtra = [], history = [], forceTools = [], preExecuteTools = [], onProgress, bot, enableThinking = false, agentMode = false, scheduledTask = null, contextPolicy = null, isAdmin = false, resourceTaskId = '' }: RunAgentOptions): Promise<AgentRunResult> {
  ...
  if (!isChannelEnabled(safeChannel)) return { reply: '(Agent 已关闭)', toolCalls: 0, pendingId: null, toolResults: [] }
  ...
  for (const toolName of forceToolSet) {
    if (effectiveContextPolicy.allowExternalTools === false && EXTERNAL_TOOL_NAMES.has(toolName)) continue
    if (effectiveContextPolicy.allowedTools.length && !effectiveContextPolicy.allowedTools.includes(toolName)) continue
  ...
  const messages: AgentMessage[] = buildAgentMessages({ userMessage: safeUserMessage, userName: safeUserName, tools, systemExtra: allSystemExtra as SystemMessage[], history, agentMode: !!agentMode })
...
function pendingResumeRecord(value: unknown): Record<string, unknown> {
  return value && typeof value === 'object' && !Array.isArray(value) ? value as Record<string, unknown> : {}
}

async function resumePending({ channelKey, userId, channel = 'qq', expectedId = '', onProgress, bot, isAdmin = false, resourceTaskId = '' }: ResumePendingOptions): Promise<AgentRunResult | AgentPendingResult> {
  const safeChannelKey = String(channelKey || '')
  const safeUserId = String(userId || '')
  const safeChannel = String(channel || 'qq')
  const safeResourceTaskId = String(resourceTaskId || '')
  ...
  const messages: AgentMessage[] = Array.isArray(resume.messages) ? resume.messages.slice() as AgentMessage[] : []
  ...
  return continueAgent({
    ...
    toolCount: (resume.toolCount as number || 0) + (executed.ok ? 1 : 0),
...
export = { run: runAgent, resumePending, normalizeContextPolicy, applyContextPolicyToTools }
