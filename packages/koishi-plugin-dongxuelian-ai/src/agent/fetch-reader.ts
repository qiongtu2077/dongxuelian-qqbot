...
function getFetchLimits(params: FetchLimitInput = {}): FetchLimits {
  return {
...
function normalizeCharset(charset: unknown = ''): string {
...
function getCharsetFromContentType(contentType: unknown = ''): string {
  const match = /charset\s*=\s*["']?([^;"'\s]+)/i.exec(String(contentType || ''))
  return normalizeCharset(match ? match[1] : '')
}

function getCharsetFromHtml(bytes: Uint8Array = Buffer.alloc(0)): string {
...
function decodeBytes(bytes: Uint8Array = Buffer.alloc(0), contentType: unknown = ''): string {
...
function isLikelyGarbagePageText(text: unknown = ''): boolean {
...
function classifyCandidateText(text: unknown = '', page: Partial<FetchPage> = {}, options: ClassifyCandidateOptions = {}): CandidateTextQuality {
...
function defaultExtractCandidateText(body: unknown = '', maxChars: number = DEFAULT_MAX_CHARS): string {
  return String(body || '')
...
async function readCandidatePage(rawUrl: unknown, options: ReadCandidatePageOptions = {}): Promise<ReadCandidatePageResult> {
...
async function fetchWithManualRedirect(rawUrl: unknown, limits: FetchLimits = getFetchLimits()): Promise<FetchPage> {
  ...
  for (let redirectCount = 0; redirectCount <= limits.redirects; redirectCount++) {
    ...
    try {
      ...
      if (getErrorName(error) === 'AbortError') throw new Error(`读取超时（${limits.timeoutMs}ms）`)
      throw error
    } finally {
      clearTimeout(timer)
    }
...
async function fetchReadableUrl(rawUrl: unknown, params: FetchLimitInput = {}): Promise<FetchPage> {
