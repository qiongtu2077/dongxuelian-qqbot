...
interface AgentRunOptions {
  systemExtra?: Array<{ role: 'system'; content: string }>
  forceTools?: string[]
  preExecuteTools?: Array<{ name: string; args: Record<string, unknown> }>
...
function cleanExtractedUrl(url: unknown = ''): string {
  return String(url || '').replace(/[),.;:!?，。；：！？、]+$/g, '')
}

function extractHttpUrls(text: unknown = ''): string[] {
...
function extractSingleUrl(text: unknown = ''): string {
  const urls = extractHttpUrls(text)
  return urls.length === 1 ? urls[0] : ''
}

function isExplicitUrlFetchRequest(text: unknown = ''): boolean {
  const value = String(text || '')
  return !!extractSingleUrl(value) && (URL_READ_INTENT_RE.test(value) || URL_CONTENT_OBJECT_RE.test(value))
}

function getStructuredSearchContext(options: RouterOptions | SearchContext = {}): SearchContext {
  const context = options.searchContext && typeof options.searchContext === 'object' ? options.searchContext : options
  return context && typeof context === 'object' ? context as SearchContext : {}
}

function canUseStructuredSearchContext(context: SearchContext = {}): boolean {
...
function isStructuredSearchBlocked(context: SearchContext = {}): boolean {
...
function isExecutableSearchQuery(query: unknown = ''): boolean {
...
function isSelfContainedSearchIntent(text: unknown = ''): boolean {
  const value = String(text || '')
  if (!(isGeneralSearchIntent(value) || EXPLICIT_AGENT_RE.test(value))) return false
  return isExecutableSearchQuery(value)
}

function normalizeRecentUserMessages(recentUserMessages: unknown = []): string[] {
  const seen = new Set<string>()
  const result: string[] = []
...
function isGeneralSearchIntent(text: unknown = ''): boolean {
...
function isSearchFollowUpRequest(text: unknown = ''): boolean {
  const value = String(text || '').replace(/\s+/g, ' ').trim()
  if (externalToolsDenied(value)) return false
...
function isSearchRefinementRequest(text: unknown = ''): boolean {
  const value = String(text || '').replace(/\s+/g, ' ').trim()
  if (externalToolsDenied(value)) return false
...
function hasSearchableRecentContext(recentUserMessages: unknown = []): boolean {
  return normalizeRecentUserMessages(recentUserMessages).some(item => SEARCH_CONTEXT_HINT_RE.test(item))
}

function pickRecentSearchContext(current: unknown = '', recentUserMessages: unknown = []): string[] {
...
function buildContextualSearchQuery(userText: unknown = '', recentUserMessages: unknown = [], options: RouterOptions | SearchContext = {}): string {
...
function buildSearchAgentUserMessage(userText: unknown = '', recentUserMessages: unknown = [], options: RouterOptions | SearchContext = {}): string {
...
function isSearchRoutable(text: unknown = '', options: RouterOptions = {}): boolean {
  const context = getStructuredSearchContext(options)
...
function heuristicRoute(userText: unknown = '', channel: string = 'qq', options: RouterOptions = {}): HeuristicRouteResult {
...
function buildExplicitUrlFetchRunOptions(userText: unknown = ''): AgentRunOptions {
...
function buildExplicitSearchRunOptions(userText: unknown = '', options: RouterOptions = {}): AgentRunOptions {
  if (externalToolsDenied(String(userText || ''))) return {}
...
export = {
  ...
  isPreviousSearchContextQuestion: (text: unknown = '') => PREVIOUS_SEARCH_CONTEXT_RE.test(String(text || '')),
  hasSearchableRecentContext,
  pickRecentSearchContext,
  isExplicitSearchRequest: (text: unknown = '') => EXPLICIT_SEARCH_RE.test(String(text || '')) && isExecutableSearchQuery(text),
}
