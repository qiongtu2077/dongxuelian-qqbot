...
// 生成 Agent 资源任务 ID。
function createAgentResourceTaskId(kind: string, channelKey: string, userId: string): string {
...
function createAdmissionError(decision: unknown, reason: unknown): ResourceError {
...
async function runAgentWithResourceGate<T>(options: AgentResourceRunOptions<T>): Promise<T> {
  ...
  try {
    ...
    if (gateHandle) gateHandle.release('agent-finally')
  }
}
