...
const { hasAdminPermission } = require('../core/utils') as typeof import('../core/utils')
...
const agentMemory = require('../agent/memory') as typeof import('../agent/memory')
...
function getMemoryCommandErrorMessage(error: unknown, fallback: string): string {
  return error instanceof Error ? error.message : String((error as UnknownErrorLike | null)?.message || fallback)
}

async function handleMemoryCommand(session: MemorySessionLike, state: MemoryCommandState) {
  ...
  if (memoryRememberMatch) {
    ...
    try {
      const item = await agentMemory.remember({
      ...
      return handled(getMemoryCommandErrorMessage(err, '记忆写入失败。'))
    }
  }
  ...
  if (memorySearchMatch) {
    ...
    try {
      const items = await agentMemory.searchMemory({
        userId: currentUserId,
        channelKey,
        query: memorySearchMatch[1].trim(),
        limit: 8,
      })
      return handled(agentMemory.formatMemoryItems(items))
    } catch (err) {
      return handled(getMemoryCommandErrorMessage(err, '记忆检索失败。'))
    }
  }
  ...
  if (memoryListMatch) {
    ...
    try {
      return handled(agentMemory.formatMemoryItems(await agentMemory.listMemory({ userId: currentUserId, limit: memoryListMatch[1] || 20 })))
    } catch (err) {
      return handled(getMemoryCommandErrorMessage(err, '记忆列表读取失败。'))
    }
  }
  ...
  if (memoryForgetMatch) {
    ...
    try {
      const removed = await agentMemory.forgetMemory({ userId: currentUserId, memoryId: memoryForgetMatch[1] })
      return handled(removed ? '已删除这条记忆。' : '没有找到这条记忆。')
    } catch (err) {
      return handled(getMemoryCommandErrorMessage(err, '记忆删除失败。'))
    }
  }
