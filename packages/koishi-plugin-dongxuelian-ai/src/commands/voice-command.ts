...
const { sanitizeUserInput } = require('../core/utils') as typeof import('../core/utils')
...
function getVoiceLogger(runtime: VoiceRuntime = {}): VoiceLogger | null {
  try {
...
async function handleVoiceCommand(session: VoiceSessionLike, state: VoiceCommandState, runtime: VoiceRuntime = {}) {
  ...
  if (plain === '东雪莲语音列表') {
    const { getBuiltinVoices } = require('../media/voice/tts') as typeof import('../media/voice/tts')
