...
async function handleAgentCommand(session: AgentSessionLike, ctx: CommandContextLike, state: AgentCommandState, options: AgentCommandOptions = {}) {
  ...
  if (mode !== 'runtime') {
    ...
    if (toolSwitchMatch) {
      ...
      await (require('../agent/config') as typeof import('../agent/config')).setToolEnabled(channel, toolName, enabled)
    ...
    if (/^(?:东雪莲)?工具状态$/.test(plain)) {
      ...
      const stats = (require('../agent/stats') as typeof import('../agent/stats')).getStats()
      const registry = require('../agent/tools/registry') as typeof import('../agent/tools/registry')
