...
const { readJsonFile, writeJsonFile, formatPercent, hasAdminPermission, safeChannelKey } = require('../core/utils') as typeof import('../core/utils')
const { isAdminUserId } = require('../core/runtime-config') as typeof import('../core/runtime-config')
const {
...
const { resetPoliticalDetectCache, clearSensitiveRuntimeState } = require('../behavior/sensitive') as typeof import('../behavior/sensitive')
const {
...
async function handleAdminInlineCommands(session: AdminSessionLike, ctx: unknown, {
  plain,
  inGuild,
  channelKey,
  isGroupAdmin,
  ...
  getArmedEventDump,
  clearArmedEventDump,
}: AdminInlineOptions): Promise<AdminCommandResult> {
  const matched = (response: string): AdminCommandResult => ({ matched: true, response })
  ...
  if (vidBlAddG) {
    ...
    const bl = await readJsonFile<{ groups?: string[]; users?: string[] }>(VIDEO_BLACKLIST_FILE, { groups: [], users: [] })
  ...
  if (handlerAdd) {
    ...
    let list = await readJsonFile<string[]>(handlerFile, [])
  ...
  if (plain === '敏感话题处理者查看') {
    if (!inGuild) return matched('这个命令只能在群里使用。')
    const list = await readJsonFile<string[]>(handlerFile, [])
  ...
  if (plain === '敏感话题检测开') {
    if (!inGuild) return matched('这个命令只能在群里使用。')
    if (!isGroupAdmin && !hasAdminPermission(session)) return matched('只有群主、管理员或bot管理员才能操作。')
    let list = await readJsonFile<string[]>(POLITICAL_DETECT_FILE, [])
    ...
    let sw = await readJsonFile<string[]>(SUMMARY_WHITELIST_FILE, [])
  ...
  if (plain === '敏感话题检测查看') {
    const list = await readJsonFile<string[]>(POLITICAL_DETECT_FILE, [])
