...
const THINKING_LEAK_WEAK_TOOL_ACTION_RE = new RegExp(
  '(?:调用|使用|执行|检查|读取|分析).{0,24}' + TOOL_PLAN_NAME_RE + '|' + TOOL_PLAN_NAME_RE + '.{0,24}(?:调用|使用|执行|检查|读取|分析|函数|工具)',
  'i',
)

const THINKING_LEAK_WEAK_INTERNAL_PLAN_CUE_RE = new RegExp(
  '(?:用户问的是|用户询问|用户想|用户需要).{0,120}(?:调用|检查|读取|分析|工具|函数)|' +
    '(?:我(?:会|将|需要|得|要|尝试|可以先|来).{0,40}(?:调用|使用|执行|检查|读取|分析|根据结果|给出回复|回答|告诉用户))|' +
    '(?:之后我会|接下来我会|然后我会).{0,80}(?:根据结果|给出回复|回答|告诉用户|调用|检查|读取|分析)|' +
    '(?:根据结果).{0,40}(?:给出回复|回答|告诉用户)',
  'i',
)

function hasWeakThinkingLeakPlan(value: string): boolean {
  const weakHits = THINKING_LEAK_WEAK_PATTERNS.reduce((count, pattern) => count + (pattern.test(value) ? 1 : 0), 0)
  if (weakHits <= 0) return false
  if (THINKING_LEAK_WEAK_TOOL_ACTION_RE.test(value)) return true
