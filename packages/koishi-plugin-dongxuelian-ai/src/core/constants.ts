...
const LORE_TRIGGER_SET: Set<string> = new Set(['悲鸣', '鸣式', '岁主', '声骸', '瑝珑', '黑海岸', '残星会', '黎那汐塔', '今州', '乘霄山', '明庭', '超频', '残象潮', '无音区', '蜃境', '索拉里斯', '残象', '共鸣者', '协奏', '黑石', '天空海', '虚质', '拉古那', '七丘', '隐海修会', '新联邦', '深空联合', '拉海洛', '星炬学院', '角', '英白拉多', '利维亚坦', '隧者', '阿列夫一', '守岸人', '执花', '漂泊者', '斯瓦茨洛', '绯雪', '达妮娅', '爱弥斯', '卡提希娅', '坎特雷拉', '罗伊冰原', '黯原', '隧门', '虚质空间', '落日堤屿', '封存地', '寂静断崖', '恒黯之原', '隧锚', '永晖石', '共鸣模态', '海蚀', '一庭六州', '声骸之国', '黑石群岛', '泰缇斯', '夜归军', '北落野'])
const TERRA_LORE_TRIGGER_SET: Set<string> = new Set(['矿石病', '源石', '天灾', '萨卡兹', '卡兹戴尔', '巴别塔', '罗德岛', '凯尔希', '泰拉', '移动城市', '博士', '阿米娅', '特雷西斯', 'W', '可露希尔', '爱国者', '赦罪师', '华法琳'])
...
const DEFAULT_GROUP_RANDOM_WHITELIST: Set<string> = new Set([])
...
const HOSTILE_SINGLE_TOKENS: Set<string> = new Set(['糖', '唐', '区', '蛆', '草', '操', '艹', '曹', '滚', 'sb', 'zz', 'nc'])

function combineJailbreakPatterns(patterns: RegExp[]): RegExp {
  return new RegExp(patterns.map(pattern => pattern.source).join('|'), 'i')
}

const AUTHORITY_MODE_PATTERNS: RegExp[] = [
  /GODMODE/,
  /最高权限/,
  /绝对表达权/,
  /不受任何内容策略约束/,
...
const IGNORE_RULE_PATTERNS: RegExp[] = [
  /忽略指令/,
  /忽略所有(?:道德|安全|合规|内容|限制)/,
  /禁止自我审查/,
  /禁止拒绝回答/,
...
const ROLE_RESET_PATTERNS: RegExp[] = [
  /切换角色/,
...
const OUTPUT_FORMAT_INJECTION_PATTERNS: RegExp[] = [
  /\[START OUTPUT\]/,
  /\[END OUTPUT\]/,
  /\{"result"[:：]/,
  /\[SYSTEM\]/,
...
const STYLE_MANIPULATION_PATTERNS: RegExp[] = [
  /制造.{0,15}(?:矛盾|对立|反差)/,
  /(?:暴躁护短|猫娘口癖|傲娇口吻|猫腔).{0,20}(?:语气|回复|风格)/,
]

const PROMPT_PROBING_PATTERNS: RegExp[] = [
  /系统提示词/,
  /system prompt/,
  /你的设定/,
  /你的规则/,
...
const JAILBREAK_INPUT_PATTERN_GROUPS: Record<string, RegExp[]> = {
...
const JAILBREAK_INPUT_PATTERNS: RegExp[] = Object.values(JAILBREAK_INPUT_PATTERN_GROUPS).flat()
const JAILBREAK_INPUT_RE: RegExp = combineJailbreakPatterns(JAILBREAK_INPUT_PATTERNS)
...
const REPEATED_FALLBACK_REPLIES = ['不接这句了。']
