...
interface CustomDiscoveryOptions extends DiscoveryOptions {
  lookupImpl?: (hostname: string) => Promise<Array<{ address: string; family: number }> | Array<string>>
  signal?: AbortSignal
}
...
function buildCustomDiscoveryUrls(rawBaseURL: string): string[] {
  const input = String(rawBaseURL || '').trim()
  if (!input) return []
  let parsed: URL
  try { parsed = new URL(input) } catch { return [] }
  const normalized = input.replace(/\/+$/, '')
  if (/\/models(?:\/)?(?:[?#].*)?$/i.test(parsed.pathname + parsed.search + parsed.hash)) return [input]
  if (/\/v\d+(?:\.\d+)?$/i.test(parsed.pathname.replace(/\/+$/, ''))) return [`${normalized}/models`]
...
function isLoopbackHostname(hostname: string): boolean {
  ...
  if (net.isIPv4(host)) return host.split('.').length === 4 && Number(host.split('.')[0]) === 127
...
async function validateCustomDiscoveryUrl(rawURL: string, lookupImpl: CustomDiscoveryOptions['lookupImpl'] = async hostname => {
  const result = await dns.lookup(hostname, { all: true })
  return result
}): Promise<URL> {
  let url: URL
  try { url = new URL(String(rawURL || '').trim()) } catch { throw new ModelDiscoveryError('请求地址格式无效', 'DISCOVERY_URL_INVALID', 400) }
  if (url.protocol !== 'http:' && url.protocol !== 'https:') throw new ModelDiscoveryError('请求地址只允许 HTTP 或 HTTPS', 'DISCOVERY_URL_INVALID', 400)
  if (!url.hostname || url.username || url.password) throw new ModelDiscoveryError('请求地址不得包含用户名或密码', 'DISCOVERY_URL_INVALID', 400)
  if (url.protocol === 'http:' && !isLoopbackHostname(url.hostname)) throw new ModelDiscoveryError('公网请求地址必须使用 HTTPS；HTTP 仅允许本机回环地址', 'DISCOVERY_URL_INSECURE', 400)
  if (isUnsafeAddress(url.hostname) && !(url.protocol === 'http:' && isLoopbackHostname(url.hostname))) throw new ModelDiscoveryError('请求地址命中私网或保留地址', 'DISCOVERY_SSRF_BLOCKED', 400)
  if (!net.isIP(url.hostname)) {
    let addresses: Array<{ address: string; family: number } | string>
    try { addresses = await lookupImpl(url.hostname) as Array<{ address: string; family: number } | string> } catch { throw new ModelDiscoveryError('请求地址域名无法解析', 'DISCOVERY_DNS_FAILED', 400) }
    if (!addresses.length || (url.protocol !== 'http:' || !isLoopbackHostname(url.hostname)) && addresses.some(item => isUnsafeAddress(typeof item === 'string' ? item : item.address))) throw new ModelDiscoveryError('请求地址解析到私网或保留地址', 'DISCOVERY_SSRF_BLOCKED', 400)
  }
  return url
}
...
function parseCustomOpenAiModelList(payload: unknown, capability: string): DiscoveredModel[] {
  if (!['text', 'vision', 'voice-asr', 'voice-tts'].includes(String(capability))) throw new ModelDiscoveryError('未知能力', 'DISCOVERY_CAPABILITY_INVALID', 400)
  const data = payload && typeof payload === 'object' && !Array.isArray(payload)
    ? (payload as { data?: unknown }).data
    : null
  if (!Array.isArray(data)) throw new ModelDiscoveryError('供应商模型列表缺少 data 数组', 'DISCOVERY_INVALID_RESPONSE', 502)
...
async function fetchCustomDiscoveryUrl(url: URL, apiKey: string, options: CustomDiscoveryOptions): Promise<FetchResponseLike> {
  const fetchImpl = options.fetchImpl || (fetch as unknown as DiscoveryFetch)
  let current = url
  for (let redirect = 0; redirect <= MAX_CUSTOM_REDIRECTS; redirect += 1) {
    ...
    if (redirect === MAX_CUSTOM_REDIRECTS) throw new ModelDiscoveryError('供应商重定向次数过多', 'DISCOVERY_REDIRECT_LIMIT', 502)
    const location = (response as FetchResponseLike & { headers?: { get?: (name: string) => string | null } }).headers?.get?.('location')
    if (!location) throw new ModelDiscoveryError('供应商重定向缺少目标地址', 'DISCOVERY_REDIRECT_INVALID', 502)
...
async function discoverCustomProviderModels(baseURL: string, capability: string, apiKey: string, options: CustomDiscoveryOptions = {}): Promise<DiscoveredModel[]> {
  const key = String(apiKey || '').trim().replace(/[\r\n]+/g, '')
  if (!key) throw new ModelDiscoveryError('API Key 不能为空', 'DISCOVERY_KEY_REQUIRED', 400)
  if (key.length > 16384) throw new ModelDiscoveryError('API Key 长度超出限制', 'DISCOVERY_KEY_INVALID', 400)
  if (!['text', 'vision', 'voice-asr', 'voice-tts'].includes(String(capability))) throw new ModelDiscoveryError('未知能力', 'DISCOVERY_CAPABILITY_INVALID', 400)
  const candidates = buildCustomDiscoveryUrls(baseURL)
  if (!candidates.length) throw new ModelDiscoveryError('请求地址格式无效', 'DISCOVERY_URL_INVALID', 400)
  const timeoutMs = Math.max(1000, Math.min(15000, Number(options.timeoutMs) || 8000))
  return withDiscoverySlot(async () => {
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) { lastError = new ModelDiscoveryError('模型列表路径不存在', 'DISCOVERY_HTTP_ERROR', 422); continue }
        ...
        if (error instanceof ModelDiscoveryError) {
          lastError = error
          if (error.code === 'DISCOVERY_HTTP_ERROR' && /不存在/.test(error.message)) continue
          throw error
        }
        const name = error && typeof error === 'object' && 'name' in error ? String((error as { name?: unknown }).name || '') : ''
