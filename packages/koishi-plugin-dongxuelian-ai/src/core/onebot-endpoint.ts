...
const LOOPBACK_HOSTS: Set<string> = new Set(['127.0.0.1', 'localhost', '::1', '[::1]'])
const DEFAULT_ONEBOT_WS_URL = 'ws://127.0.0.1:8080/onebot/v11/ws'

function resolveOneBotWsUrl(): string {
  ...
  if (parsed.protocol !== 'ws:') throw new Error('OneBot WebSocket 只允许 ws:// loopback 地址')
  const host = parsed.hostname.toLowerCase()
  if (!LOOPBACK_HOSTS.has(host)) throw new Error('OneBot WebSocket 只允许连接本机 loopback 地址')
  parsed.username = ''
  parsed.password = ''
  parsed.hash = ''
