...
function isTerminalFileEvidence(text: string = ''): boolean {
  const evidence = String(text || '').trim()
  if (!evidence) return false
  if (/---文件内容开始---|\[用户上传文件:/.test(evidence)) return false
  return /媒体分析队列|稍后再读取|找到\d+个文件|下载失败|已过期|无法提取内容|文件解析失败|找不到|没有收到|没有可用|文件被跳过|无法确定当前会话/.test(evidence)
}

function isFileEvidence(text: string = ''): boolean {
  const content = String(text || '')
  return /---文件内容开始---|\[用户上传文件:|\[文件解析失败:/.test(content) || isTerminalFileEvidence(content)
}

function getAnalyzeFileToolCallIds(toolCalls: ToolCallLike[] = []): Set<string> | null {
  if (!Array.isArray(toolCalls) || !toolCalls.length) return null
  const ids = new Set<string>()
  for (const call of toolCalls) {
    if (call?.function?.name === 'analyze_file' && call.id) ids.add(String(call.id))
  }
  return ids
}

function isAnalyzeFileResult(item: ToolResultLike, analyzeFileCallIds: Set<string> | null): boolean {
  if (!analyzeFileCallIds) return true
  const resultCallId = String(item?.tool_call_id || '').trim()
  return !!resultCallId && analyzeFileCallIds.has(resultCallId)
}

function formatTerminalFileEvidence(evidence: string = ''): string {
...
function toolResultsIncludeFileEvidence(results: ToolResultLike[] = [], toolCalls: ToolCallLike[] = []): boolean {
  const analyzeFileCallIds = getAnalyzeFileToolCallIds(toolCalls)
  return Array.isArray(results) && results.some(item => {
    if (!isAnalyzeFileResult(item, analyzeFileCallIds)) return false
    const content = String(item?.content || '')
    return isFileEvidence(content)
  })
}

function selectFileEvidenceResult(results: ToolResultLike[] = [], toolCalls: ToolCallLike[] = []): string {
  if (!Array.isArray(results)) return ''
  const analyzeFileCallIds = getAnalyzeFileToolCallIds(toolCalls)
  const item = results.find(result => {
    if (!isAnalyzeFileResult(result, analyzeFileCallIds)) return false
