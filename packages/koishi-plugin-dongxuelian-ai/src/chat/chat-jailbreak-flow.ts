...
const { getRecentAssistantReplies } = require('../conversation') as typeof import('../conversation')
...
interface ChatModelTextResult {
  type?: string
  content?: unknown
  message?: { content?: unknown }
}

function getChatJailbreakErrorMessage(err: unknown): string {
  return err instanceof Error ? err.message : String(err || 'unknown error')
}

function normalizeChatJailbreakReply(result: unknown): string {
  if (typeof result === 'string') return result
  if (result && typeof result === 'object') {
    const data = result as ChatModelTextResult
    if (data.type === 'text' || data.content !== undefined) return String(data.content || '')
...
function isContextJailbroken(session: JailbreakSessionLike): boolean {
...
async function chatJailbreak(session: JailbreakSessionLike, userText: string, ctx?: JailbreakContextLike, options: ChatJailbreakOptions = {}): Promise<string> {
