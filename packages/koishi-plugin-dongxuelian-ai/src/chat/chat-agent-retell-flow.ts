...
const { testChatPromptRegex } = require('./chat-prompt-builder') as typeof import('./chat-prompt-builder')
const { chatJailbreak } = require('./chat-jailbreak-flow') as typeof import('./chat-jailbreak-flow')
...
type CallModelFn = (messages: AgentRetellMessage[], randomTriggered?: boolean, options?: Record<string, unknown>) => Promise<string | { content?: unknown; message?: { content?: unknown } }>
...
function getAgentReplyMaxChars(retaliationLevel: number = 0): number {
...
function getAgentOutputGuardReason(text: string = ''): string {
...
function formatRetellTime(now: Date = new Date()): string {
...
function normalizeAgentRetellText(value: unknown): string {
  if (typeof value === 'string') return value
  if (value && typeof value === 'object') {
    const data = value as { content?: unknown; message?: { content?: unknown } }
...
async function retellAgentResultForChat({
  ...
  callModel,
  now = new Date(),
}: RetellAgentResultForChatOptions): Promise<string> {
  ...
  const agentMessages: AgentRetellMessage[] = [
  ...
  let agentReply = normalizeAgentRetellText(await callModel(agentMessages, options.randomTriggered))
  ...
  if (agentReplyGuardReason) {
    ...
    agentReply = normalizeAgentRetellText(await callModel(agentMessages, options.randomTriggered))
  }
