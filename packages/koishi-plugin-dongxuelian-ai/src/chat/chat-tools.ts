...
const { readGroupContext } = require('../routing/group-scene-index') as typeof import('../routing/group-scene-index')
...
const CHAT_TOOLS_TOTAL_DEADLINE_MS: number = 5000
const SHORT_MEDIA_FOLLOWUP_MAX_CHARS: number = 12
...
function getChatToolErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : String(error || 'unknown error')
}

function getArgString(args: ChatToolArgs, key: string): string {
...
function resolveChatToolChannel(options: ChatToolOptions = {}): string {
  return String(options.channel || options.toolChannel || DEFAULT_CHAT_TOOL_CHANNEL).trim() || DEFAULT_CHAT_TOOL_CHANNEL
}

function isChatToolAllowed(channel: string, name: string): boolean {
  if (!name) return false
  return isToolEnabled(channel || DEFAULT_CHAT_TOOL_CHANNEL, name)
}

function getChatToolDefinitions(options: ChatToolOptions = {}): ChatToolDefinition[] {
...
function executeCalculate(args: ChatToolArgs = {}): string {
  const expr = getArgString(args, 'expression').trim()
  ...
  try {
    ...
    return '计算失败：' + (e instanceof Error ? e.message : '表达式格式错误')
  }
}

async function executeSearchMemory(context: ChatToolContext = {}): Promise<string> {
...
function isExplicitChatWriteActionAllowed(name: string = '', args: ChatToolArgs = {}, context: ChatToolContext = {}): boolean {
  ...
  switch (name) {
    ...
    case 'read_group_context': {
      ...
      const maxAgeMinutes = context.randomTriggered
        ? Math.min(Math.max(parsePositiveInt(args.maxAgeMinutes, 30), 1), 30)
        : Math.min(Math.max(parsePositiveInt(args.maxAgeMinutes, 60), 1), 24 * 60)
    ...
    case 'read_image_history': {
      const { getRecentImages } = require('../media/image/image-store') as typeof import('../media/image/image-store')
      const ck = context.channelKey || ''
      if (!ck) return '无法获取频道信息'
      const limit = Math.min(Math.max(parsePositiveInt(args.limit, 5), 1), 10)
    ...
    case 'analyze_file': {
      const analyzeFile = require('../agent/tools/analyze-file') as typeof import('../agent/tools/analyze-file')
    ...
    case 'list_reminders': {
      const { listRemindersTool } = require('../agent/tools/reminder-tools') as typeof import('../agent/tools/reminder-tools')
      return listRemindersTool.execute(args, context)
    }
    case 'cancel_reminder': {
      const { cancelReminderTool } = require('../agent/tools/reminder-tools') as typeof import('../agent/tools/reminder-tools')
      return cancelReminderTool.execute(args, context)
    }
    case 'create_scheduled_task': {
      const { createScheduledTaskTool } = require('../agent/tools/scheduled-task-tools') as typeof import('../agent/tools/scheduled-task-tools')
      return createScheduledTaskTool.execute(args, context)
    }
    case 'list_scheduled_tasks': {
      const { listScheduledTasksTool } = require('../agent/tools/scheduled-task-tools') as typeof import('../agent/tools/scheduled-task-tools')
      return listScheduledTasksTool.execute(args, context)
    }
    case 'get_scheduled_task': {
      const { getScheduledTaskTool } = require('../agent/tools/scheduled-task-tools') as typeof import('../agent/tools/scheduled-task-tools')
      return getScheduledTaskTool.execute(args, context)
    }
    case 'pause_scheduled_task': {
      const { pauseScheduledTaskTool } = require('../agent/tools/scheduled-task-tools') as typeof import('../agent/tools/scheduled-task-tools')
      return pauseScheduledTaskTool.execute(args, context)
    }
    case 'resume_scheduled_task': {
      const { resumeScheduledTaskTool } = require('../agent/tools/scheduled-task-tools') as typeof import('../agent/tools/scheduled-task-tools')
      return resumeScheduledTaskTool.execute(args, context)
    }
    case 'delete_scheduled_task': {
      const { deleteScheduledTaskTool } = require('../agent/tools/scheduled-task-tools') as typeof import('../agent/tools/scheduled-task-tools')
      return deleteScheduledTaskTool.execute(args, context)
    }
    case 'run_scheduled_task_now': {
      const { runScheduledTaskNowTool } = require('../agent/tools/scheduled-task-tools') as typeof import('../agent/tools/scheduled-task-tools')
      return runScheduledTaskNowTool.execute(args, context)
    }
    case 'create_uploaded_file_variant': {
      const variant = require('../agent/tools/create-uploaded-file-variant') as typeof import('../agent/tools/create-uploaded-file-variant')
...
async function handleChatToolCalls(toolCalls?: ChatToolCall[], context: ChatToolContext = {}): Promise<ChatToolCallResult> {
  if (!Array.isArray(toolCalls) || !toolCalls.length) return { results: [], heavyTools: [] }

  const results: ChatToolResultMessage[] = []
  const heavyTools: ChatToolCall[] = []
...
function getChatToolSystemHint(channelKey?: string, options: ChatToolOptions = {}): string {
  ...
  if (channelKey) {
    try {
      ...
      /* non-critical: cached file hint is best-effort and missing cache must not block chat */
    }
  }
  if (channelKey) {
    try {
      const { getRecentImagesCached } = require('../media/image/image-store') as typeof import('../media/image/image-store')
      ...
      /* non-critical: cached image hint is best-effort and missing cache must not block chat */
    }
  }
  return hint
}
