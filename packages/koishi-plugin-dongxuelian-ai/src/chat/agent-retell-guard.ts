...
const SEARCH_FAILURE_RE: RegExp = /(?:搜索失败|weak_hit|弱命中|未提取到有效搜索结果|没有拿到可靠结果|没有返回可靠来源|没有返回可用结果|没有找到可靠结果|无可靠结果|结果质量过低|搜索页结果抽取失败|API 搜索请求失败|API 搜索没有返回可靠来源|API 联网搜索不可用|轻量 HTTP 搜索未拿到可靠结果|web_fetch 失败|web_fetch 未读到可靠正文|请求太频繁|正文过短|非文本页面|拒绝访问|未打开到可用正文|未打开候选网页正文|未读到可用正文|不能作为事实依据)/i
