...
const { isProcessAlive, nowIso, sanitizeId } = require('../resource-common/files') as typeof import('../resource-common/files')
...
type ResourceGateFailureCode =
  | 'gate_permission_denied'
  | 'gate_readonly_filesystem'
  | 'gate_storage_full'
  | 'gate_quota_exceeded'
  | 'gate_path_invalid'
  | 'gate_fd_exhausted'
...
function toSafeGatePath(target: string): string {
  const relative = path.relative(GATE_ROOT, path.resolve(target)).replace(/\\/g, '/')
  if (!relative) return '.'
  if (relative === '..' || relative.startsWith('../') || path.isAbsolute(relative)) return '[outside-gate-root]'
  return relative.replace(/[^a-zA-Z0-9._/-]/g, '_').slice(0, 240)
}

// 从 Node 系统错误中提取稳定 errno，没有 errno 时使用 UNKNOWN。
function getGateErrno(error: unknown): string {
...
function classifyGateStorageError(error: unknown, stage: string, target: string, forcedCode?: ResourceGateFailureCode): ResourceGateStorageError {
  if (error instanceof ResourceGateStorageError) return error
  const errno = getGateErrno(error)
  let failureCode: ResourceGateFailureCode = forcedCode || 'gate_state_unreadable'
  if (!forcedCode) {
    if (errno === 'EACCES' || errno === 'EPERM') failureCode = 'gate_permission_denied'
    else if (errno === 'EROFS') failureCode = 'gate_readonly_filesystem'
    else if (errno === 'ENOSPC') failureCode = 'gate_storage_full'
    else if (errno === 'EDQUOT') failureCode = 'gate_quota_exceeded'
    else if (errno === 'ENOTDIR' || errno === 'EISDIR' || errno === 'EEXIST') failureCode = 'gate_path_invalid'
    else if (errno === 'EMFILE' || errno === 'ENFILE') failureCode = 'gate_fd_exhausted'
    else if (errno === 'EIO') failureCode = 'gate_io_error'
  }
  return new ResourceGateStorageError(failureCode, errno, stage, toSafeGatePath(target), error)
}

// 判断错误是否为调用方必须停止入队的资源锁存储故障。
function isResourceGateStorageError(error: unknown): error is ResourceGateStorageError {
  return error instanceof ResourceGateStorageError || !!(
    error && typeof error === 'object'
    && typeof (error as { failureCode?: unknown }).failureCode === 'string'
    && String((error as { failureCode?: unknown }).failureCode).startsWith('gate_')
  )
}
...
// 返回当天 S0 事件日志路径。
function gateEventFile(date = new Date()): string {
  const stamp = date.toISOString().slice(0, 10)
  return path.join(GATE_ROOT, `events-${stamp}.jsonl`)
}

// 精确判断路径是否存在，仅将 ENOENT 视为不存在。
function gatePathExists(target: string, stage: string): boolean {
  try {
    fs.lstatSync(target)
    return true
  } catch (error) {
    if (getGateErrno(error) === 'ENOENT') return false
    throw classifyGateStorageError(error, stage, target)
  }
}

// 创建资源锁目录，任何文件系统错误都转换为结构化故障。
function ensureGateDirectory(target: string, stage: string): void {
  try {
...
function writeGateJsonAtomic(target: string, data: unknown, stage: string): void {
  ...
  try {
    ...
    try {
      if (gatePathExists(temporary, `${stage}_temporary_exists`)) fs.rmSync(temporary, { force: true })
    } catch (cleanupError) {
...
function readGateJson<T>(target: string, stage: string, validate: (value: unknown) => value is T): T | null {
...
function removeGatePath(target: string, stage: string): boolean {
  if (!gatePathExists(target, `${stage}_exists`)) return false
  try {
...
function writeGateEvent(event: string, data: Record<string, unknown> = {}): void {
...
// 生成 ticket 文件路径。
function getTicketFile(ticketId: string): string {
  return path.join(TICKETS_DIR, `${sanitizeId(ticketId)}.json`)
}

// 验证从磁盘读取的 ticket 具备队列排序和归属所需字段。
function isResourceGateTicket(value: unknown): value is ResourceGateTicket {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false
  const ticket = value as Partial<ResourceGateTicket>
...
function isResourceGateLockMeta(value: unknown): value is ResourceGateLockMeta {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false
  const meta = value as Partial<ResourceGateLockMeta>
...
function createTicket(input: ResourceGateTicketInput): ResourceGateTicket {
  ensureGateDirs()
  const ticket: ResourceGateTicket = {
...
function listTickets(): ResourceGateTicket[] {
  ensureGateDirs()
  let names: string[]
  try {
  ...
  const tickets = names.map(name => readGateJson(path.join(TICKETS_DIR, name), 'ticket_read', isResourceGateTicket))
    .filter((ticket): ticket is ResourceGateTicket => ticket !== null)
...
// 判断当前 ticket 是否排在队头。
function isTicketHead(ticketId: string): boolean {
...
async function acquireResourceGate(input: AcquireGateOptions): Promise<ResourceGateHandle> {
      ...
      if (isTicketHead(ticket.ticketId)) {
        ...
        if (meta) {
          heartbeatTimer = setInterval(() => {
            try { updateLockMeta(ticket.ticketId) } catch (error) { heartbeatFailure = classifyGateStorageError(error, 'lock_heartbeat', LOCK_META_FILE) }
          }, 2000)
          heartbeatTimer.unref?.()
          return {
            ticketId: ticket.ticketId,
            meta,
            updateStep(step: string, memAvailableMb?: number | null) {
            ...
            release(reason = 'completed') {
              ...
              if (pendingHeartbeatFailure) throw pendingHeartbeatFailure
            },
          }
        }
      }
...
function getResourceGateStatus(staleMs = DEFAULT_STALE_MS): ResourceGateStatus {
  const meta = readLockMeta()
...
function isDailyReportRunning(): boolean {
  const meta = readLockMeta()
