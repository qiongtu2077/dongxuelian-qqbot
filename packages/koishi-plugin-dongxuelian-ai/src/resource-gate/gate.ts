...
function listTickets(): ResourceGateTicket[] {
  ...
  const tickets: ResourceGateTicket[] = []
  for (const name of names) {
    const file = path.join(TICKETS_DIR, name)
    const ticket = readJsonFile<ResourceGateTicket>(file, null)
...
function readLockMeta(): ResourceGateLockMeta | null {
  return readJsonFile<ResourceGateLockMeta>(LOCK_META_FILE, null)
}
...
function tryCreateLock(ticket: ResourceGateTicket, input: AcquireGateOptions): ResourceGateLockMeta | null {
  try {
    ...
    if (String((error as { code?: string })?.code || '') === 'EEXIST') return null
    throw error
  }
