...
function getExpiryIso(ttlMs: number): string {
...
function normalizeRecentMessages(value: unknown): AutoMemoryMessage[] {
  if (!Array.isArray(value)) return []
  return value
    .filter(item => item && typeof item === 'object')
    .map(item => {
      const record = item as AutoMemoryMessage
    ...
    .filter(Boolean)
    .slice(-AUTO_MEMORY_WINDOW * 2) as AutoMemoryMessage[]
}

// Build the daily memory file path for a user and date.
function getDailyFile(userId: unknown, date: string = new Date().toISOString().slice(0, 10)): string {
...
async function readDailyFile(userId: unknown): Promise<string> {
  try {
...
async function appendDailyFile(userId: unknown, content: string): Promise<void> {
...
function buildExtractMessages(recentMessages: AutoMemoryMessage[], existingDaily: string): ExtractMessage[] {
  ...
  const messages: ExtractMessage[] = [{ role: 'system', content: EXTRACT_PROMPT }]
  if (existingDaily) {
...
function isEmptyMemoryContent(content: string): boolean {
...
async function extractMemoryDirect(recentMessages: AutoMemoryMessage[], userId: unknown): Promise<string | null> {
  ...
  if (isEmptyMemoryContent(content) || content.length < 3) return null
  return content
}

// Execute Dream compaction inside the S2 worker.
async function runDreamDirect(userId: unknown): Promise<DreamResult> {
  ...
  if (longTerm.trim()) inputParts.push(`[long-term memory]\n${longTerm.trim()}`)
  inputParts.push(`[daily memory]\n${dailyContent}`)
  const input = inputParts.join('\n\n')

  if (input.length > 30000) return { success: false, reason: 'input-too-large' }
  ...
  if (longTerm.trim()) await fsp.writeFile(getBackupFile(userId), longTerm, 'utf8')
...
function normalizeMemorySubmission(result: SubmissionResultLike, kind: string): MemorySubmissionResult {
...
function submitAgentMemoryTask(options: MemorySubmissionOptions): MemorySubmissionResult {
  ...
  if (recentMessages.length < 2) return { accepted: false, status: 'skipped', message: 'not enough messages' }
...
async function runMemoryExtractionWorkerTask(task: MemoryWorkerTaskLike): Promise<Record<string, unknown>> {
...
async function runMemoryCompactionWorkerTask(task: MemoryWorkerTaskLike): Promise<Record<string, unknown>> {
  const userId = String(task?.payload?.userId || task?.userId || '')
...
async function runMemoryWorkerTask(task: MemoryWorkerTaskLike): Promise<Record<string, unknown>> {
  if (task?.kind === 'agent_memory') return runMemoryExtractionWorkerTask(task)
  if (task?.kind === 'agent_memory_compaction') return runMemoryCompactionWorkerTask(task)
