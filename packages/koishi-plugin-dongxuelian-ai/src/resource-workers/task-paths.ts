...
// 返回任务状态目录。
function getTaskStatusDir(status: string): string {
  return path.join(TASKS_ROOT, sanitizeId(status))
}

// 返回 pending 任务类型目录。
function getPendingKindDir(kind: string): string {
  return path.join(getTaskStatusDir('pending'), sanitizeId(kind))
}

// 返回指定任务文件路径。
function getTaskFile(status: string, kind: string, taskId: string): string {
  if (status === 'pending') return path.join(getPendingKindDir(kind), `${sanitizeId(taskId)}.json`)
  return path.join(getTaskStatusDir(status), `${sanitizeId(taskId)}.json`)
}

// 返回任务结果目录。
function getTaskResultDir(taskId: string): string {
  return path.join(RESULTS_ROOT, sanitizeId(taskId))
}

// 返回 worker 心跳状态文件。
function getWorkerStateFile(workerName: string): string {
  return path.join(WORKER_STATE_DIR, `${sanitizeId(workerName)}.json`)
}

// 返回当天 S2 事件文件。
function getWorkerEventFile(date = new Date()): string {
