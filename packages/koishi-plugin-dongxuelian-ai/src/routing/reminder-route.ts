...
type ReminderActionRequest =
  | { name: 'cancel_reminder'; args: ReminderCancelArgs }
  | { name: 'list_reminders'; args: ReminderListArgs }
  | { name: 'create_reminder'; args: ReminderCreateArgs }

interface DailyTimeParseResult {
  hour: number
  minute: number
  schedule: string
  scheduleText: string
...
function parseChineseInteger(text: string = ''): number {
  ...
  if (tenIndex >= 0) {
    ...
    const tens = left ? (CHINESE_NUMBERS[left as keyof typeof CHINESE_NUMBERS] || 0) : 1
    const ones = right ? (CHINESE_NUMBERS[right as keyof typeof CHINESE_NUMBERS] || 0) : 0
    return tens * 10 + ones
  }
  return CHINESE_NUMBERS[value as keyof typeof CHINESE_NUMBERS] || 0
}

function stripReminderNoise(text: string = ''): string {
  return String(text || '')
    .replace(/^说错了[，,、\s]*/, '')
...
function parseReminderRequest(text: string = '', now: number = Date.now()): ReminderCreateArgs | null {
  ...
  const unitMs = RELATIVE_UNITS_MS[relative[2] as keyof typeof RELATIVE_UNITS_MS]
...
function parseReminderListRequest(text: string = ''): ReminderListArgs | null {
  const value = stripReminderNoise(text)
  if (!value || !/(提醒|闹钟)/.test(value)) return null
  if (!/(查看|查一下|看看|列出|列表|还有|哪些|有什么|多少|待触发)/.test(value)) return null
  return { limit: 10 }
}

function parseReminderCancelRequest(text: string = ''): ReminderCancelArgs | null {
  const value = stripReminderNoise(text)
  if (!value || !/(提醒|闹钟|叫我|喊我)/.test(value)) return null
  if (!/(取消|删除|删掉|撤销|不用|别提醒|别叫|别喊|关掉)/.test(value)) return null
  ...
  const keyword = value
    ...
    .trim()
    .slice(0, 80)
  const result: ReminderCancelArgs = {}
  if (latest) result.latest = true
  if (keyword) result.keyword = keyword
  return result
}

function parseReminderActionRequest(text: string = '', now: number = Date.now()): ReminderActionRequest | null {
  const cancel = parseReminderCancelRequest(text)
  if (cancel) return { name: 'cancel_reminder', args: cancel }
  const list = parseReminderListRequest(text)
  if (list) return { name: 'list_reminders', args: list }
  const create = parseReminderRequest(text, now)
  if (create) return { name: 'create_reminder', args: create }
  return null
}

function isReminderToolName(name: string = ''): boolean {
...
function parseDailyTime(text: string = ''): DailyTimeParseResult | null {
  ...
  if ((dayPart === '下午' || dayPart === '晚上' || dayPart === '夜里') && hour >= 1 && hour < 12) hour += 12
  if (dayPart === '凌晨' && hour === 12) hour = 0
...
function parseScheduledTaskRequest(text: string = ''): ScheduledTaskRequest | null {
  const value = stripReminderNoise(text)
  if (!value || !/(定时|任务|每天|每日|每周|每隔|周期|早安|总结|分析|提醒|叫我|喊我)/.test(value)) return null
  ...
  let prompt = value
    ...
    .trim()
  if (!prompt) prompt = type === 'text' ? '早安' : value
  return {
    name: 'create_scheduled_task',
    args: {
      mode: 'cron',
...
function isReminderCapabilityRefusal(reply: string = ''): boolean {
...
function isReminderUnbackedPromise(reply: string = '', userText: string = ''): boolean {
  const text = String(reply || '')
  const user = String(userText || '')
  if (!text || !parseReminderRequest(user)) return false
  if (/已创建提醒|提醒已创建/.test(text)) return false
