...
function externalToolsDenied(text: string = ''): boolean {
  const value = String(text || '').replace(/\s+/g, ' ').trim()
  if (!value) return false
  return EXTERNAL_TOOL_DENY_RE.test(value)
}

function filterExternalToolDefinitions<T extends ToolDefinitionLike>(tools: T[] = [], text: string = ''): T[] {
...
function buildExternalToolPolicyHint(text: string = ''): string {
