 ...
 * 状态: 无模块级状态。
 * ========================================================================== */
const { getCachedTranscript, getVoiceEntry } = require('../media/voice/voice-store') as typeof import('../media/voice/voice-store')
...
function isVoiceQuickReadIntent(text: string = ''): boolean {
  const value = normalizeText(text)
  if (!value) return false
  return VOICE_QUICK_READ_RE.test(value)
}

function formatVoiceQueuedReply(admission: AdmissionDecisionLike): string {
...
async function resolveVoiceQuickReadReply(channelKey: string, messageId: string): Promise<string> {
  const safeChannelKey = String(channelKey || '').trim()
  const safeMessageId = String(messageId || '').trim()
  if (!safeChannelKey || !safeMessageId) return '语音记录不完整，请重新发一次语音。'
  const cached = await getCachedTranscript(safeChannelKey, safeMessageId)
  if (cached) return `[语音转文字：${cached}]`
  const entry = await getVoiceEntry(safeChannelKey, safeMessageId) as VoiceEntryLike | null
  if (!entry) return '没有找到最近可转写的语音，请重新发一次语音。'
