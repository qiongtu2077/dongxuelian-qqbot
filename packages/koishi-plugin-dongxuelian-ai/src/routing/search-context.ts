...
const { getUserMessageContent } = require('../conversation') as typeof import('../conversation')
...
function isPrivateSession(session: SearchSessionLike = {}): boolean {
  return !!(!session.guildId && (session.subtype === 'private' || session.isDirect || session.channelId || session.userId))
}

function getMessageTs(message: SearchHistoryMessage = {}, fallbackTs: number = 0): number {
...
function looksLikeMetaTurn(text: string = ''): boolean {
  const value = normalizeText(text)
  if (!value) return true
  return /^(?:切换人格|东雪莲人格|人格切换|记住|忘记|清空记忆|AI状态|状态|帮助|help)$/i.test(value)
}

function looksLikeActionOnlyFollowUp(text: string = ''): boolean {
  const value = normalizeText(text)
...
function hasConcreteSearchSubject(text: string = ''): boolean {
...
function isPotentialSearchFollowUp(text: string = ''): boolean {
  const value = normalizeText(text)
...
function classifyPrivateAge(ts: number, now: number): 'none' | 'hot' | 'warm' | 'cold' | 'expired' {
  if (!ts) return 'none'
  const age = now - ts
...
function buildPrivateSearchContext(session: SearchSessionLike, history: SearchHistoryMessage[] = [], options: PrivateSearchContextOptions = {}): PrivateSearchContext {
  ...
  const userMessages = messages
    ...
    .map(item => ({
      raw: item,
      text: normalizeText(getUserMessageContent(String(item.content || ''))),
...
function mergeSearchContext(base: Partial<PrivateSearchContext> = {}, override: Partial<PrivateSearchContext> = {}): PrivateSearchContext {
