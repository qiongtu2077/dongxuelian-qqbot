...
const { normalizeText, sanitizeUserName, safeChannelKey, formatShanghaiTime24h } = require('../core/utils') as typeof import('../core/utils')
const { redactSensitiveText } = require('../core/redactor') as typeof import('../core/redactor')

const GROUP_SCENE_DIR: string = path.join(DATA_DIR, 'group-scenes')
...
interface NormalizedSceneEntry {
  channelKey: string
  userId: string
  role: 'assistant' | 'user'
  speakerName: string
...
const sceneQueues: Map<string, Promise<unknown>> = new Map()
const sceneCache: Map<string, GroupSceneData> = new Map()

const safeSceneChannelKey = safeChannelKey

function ignoreSceneQueueFailure(error: unknown): void {
  void error
}

function getSceneFilePath(channelKey: string = ''): string {
  return path.join(GROUP_SCENE_DIR, safeSceneChannelKey(channelKey) + '.json')
}

function enqueueSceneTask<T>(channelKey: string, task: () => T | Promise<T>): Promise<T> {
  const key = safeSceneChannelKey(channelKey)
  const previous = sceneQueues.get(key) || Promise.resolve()
  const current = previous.catch(ignoreSceneQueueFailure).then(task)
  const cleanup = current.finally(() => {
    if (sceneQueues.get(key) === cleanup) sceneQueues.delete(key)
  }).catch(ignoreSceneQueueFailure)
  sceneQueues.set(key, cleanup)
  return current
}

async function waitForSceneTasks(channelKey: string): Promise<void> {
  const pending = sceneQueues.get(safeSceneChannelKey(channelKey))
  if (pending) await pending.catch(ignoreSceneQueueFailure)
}

function sanitizeSceneText(text: unknown = '', maxChars: number = 180): string {
...
function extractSceneAnchors(content: string = ''): SceneAnchor[] {
...
function extractSceneKeywords(content: string = ''): string[] {
...
function hasSceneMedia(item: SceneItemLike = {}): boolean {
  return !!(item.hasMessageRecordCue || /\[(?:文件|图片|语音|转发)/.test(item.content || ''))
}

function looksLikeShortSceneFollowUp(text: string = ''): boolean {
  const value = normalizeText(text)
  return !!(value && value.length <= SHORT_SCENE_FOLLOWUP_MAX_CHARS)
}

function normalizeSceneEntry(channelKey: string, entry: RawSceneEntry = {}): NormalizedSceneEntry | null {
...
function normalizeSceneData(data: RawSceneData | Partial<GroupSceneData> | null | undefined): GroupSceneData {
  ...
  return {
    ...
    scenes: scenes
      ...
      .map(scene => ({
        id: String(scene.id || ''),
        channelKey: safeSceneChannelKey(String(scene.channelKey || '')),
        ...
        snippets: Array.isArray(scene.snippets) ? scene.snippets.map((snippet): SceneSnippet => ({
...
async function loadGroupScenes(channelKey: string): Promise<GroupSceneData> {
  const key = safeSceneChannelKey(channelKey)
  try {
    ...
    const code = String((error as FsErrorLike)?.code || '')
    if (code === 'ENOENT') {
...
async function writeGroupScenes(channelKey: string, data: Partial<GroupSceneData>): Promise<boolean> {
  await fs.mkdir(GROUP_SCENE_DIR, { recursive: true })
...
function createSceneId(entry: NormalizedSceneEntry): string {
  const d = new Date(entry.ts)
  const padSceneDatePart = (n: number): string => String(n).padStart(2, '0')
...
function sceneContainsMessage(scene: GroupScene | null | undefined, messageId: string = ''): boolean {
  const id = String(messageId || '')
  return !!(id && Array.isArray(scene?.messageIds) && scene.messageIds.includes(id))
}

function shouldMergeScene(lastScene: GroupScene | null | undefined, entry: NormalizedSceneEntry): boolean {
...
function appendEntryToScene(scene: GroupScene, entry: NormalizedSceneEntry): GroupScene {
...
async function appendGroupSceneEntry(channelKey: string, rawEntry: RawSceneEntry = {}): Promise<boolean> {
...
function formatSceneLine(item: SceneItemLike = {}): string {
...
function classifySceneItemsForActive(items: SceneItemLike[] = [], options: ActiveSceneOptions = {}): ActiveSceneLayers {
  ...
  const idIndex = new Map<string, SceneItemLike>()
  ...
  const currentTurn: SceneItemLike[] = []
  const hotContext: SceneItemLike[] = []
  const oldBackground: SceneItemLike[] = []
...
function buildOldBackgroundSceneNote(items: SceneItemLike[] = []): string {
...
function buildActiveGroupSceneNote(channelKey: string, items: SceneItemLike[] = [], currentUserId: string = '', options: ActiveSceneOptions = {}): string {
  ...
  const inSet = (set: SceneItemLike[], item: SceneItemLike): boolean => set.includes(item)
  ...
  const formatLayer = (list: SceneItemLike[]): string[] => list.map(formatSceneLine).filter(Boolean)
...
function MAX_SCENE_MAX_INPUT_ITEMS(): number {
...
function scoreScene(scene: GroupScene | null | undefined, tokens: string[] = [], options: { sceneId?: string; anchorType?: string } = {}): number {
  if (!scene) return 0
...
function formatRetrievedScene(scene: GroupScene): string {
...
async function readGroupContext(channelKey: string, args: ReadGroupContextArgs = {}): Promise<string> {
  if (!channelKey) return '无法获取当前群聊频道。'
  const sceneId = String(args.sceneId || '').trim()
  const query = String(args.query || args.reason || '').trim()
  const maxAgeMinutes = Math.min(Math.max(parseInt(String(args.maxAgeMinutes || ''), 10) || 60, 1), 24 * 60)
  const maxScenes = Math.min(Math.max(parseInt(String(args.maxScenes || ''), 10) || 2, 1), 3)
