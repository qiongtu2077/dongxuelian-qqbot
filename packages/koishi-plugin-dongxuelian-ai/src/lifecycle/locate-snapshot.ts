...
const locateSnapshots: Map<string, LocateSnapshot> = new Map()

function buildLocateSnapshotKey(channelKey: unknown, userId: unknown): string {
...
function saveLocateSnapshot(channelKey: unknown, userId: unknown, entries: LocateSnapshotSource[] = [], now: number = Date.now()): void {
  const key = buildLocateSnapshotKey(channelKey, userId)
  if (!key) return
  const normalized: LocateSnapshotEntry[] = []
...
function getLocateSnapshot(channelKey: unknown, userId: unknown, now: number = Date.now()): LocateSnapshotEntry[] | null {
  const key = buildLocateSnapshotKey(channelKey, userId)
  if (!key) return null
...
function pruneLocateSnapshots(now: number = Date.now()): void {
  for (const [key, snapshot] of locateSnapshots.entries()) {
    if (now - snapshot.savedAt > LOCATE_SNAPSHOT_TTL_MS) locateSnapshots.delete(key)
  }
  if (locateSnapshots.size < LOCATE_SNAPSHOT_MAX_KEYS) return
  const entries: Array<[string, number]> = []
  for (const [key, snapshot] of locateSnapshots.entries()) entries.push([key, snapshot.savedAt])
  entries.sort((a, b) => a[1] - b[1])
  for (let i = 0; i <= entries.length - LOCATE_SNAPSHOT_MAX_KEYS; i += 1) locateSnapshots.delete(entries[i][0])
}

/** dispose 时由调用方显式清理，避免插件重载后残留旧快照。 */
function clearLocateSnapshots(): void {
