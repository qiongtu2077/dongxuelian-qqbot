 ...
 * 状态: 无状态。
 */
const { getGroupPersona } = require('../persona/persona') as typeof import('../persona/persona')
...
interface GroupPersonaEntry {
  persona?: string
}

function getGroupPersonaName(channelKey: string): string {
  const entry = getGroupPersona(channelKey)
  return entry && entry.persona ? String(entry.persona) : ''
}

function isPersonaSwitchRisky(personaResolution: PersonaResolution | null | undefined, groupPersonaName: string): boolean {
  return !!(
    personaResolution &&
