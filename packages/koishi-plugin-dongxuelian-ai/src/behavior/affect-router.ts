...
interface AffectDiagnostic {
  ...
  persona: { name: string; hash: string }
  policy: AffectPolicy
...
const DEFAULT_AFFECT_POLICY: Readonly<AffectPolicy> = Object.freeze({
  allowVoice: true,
  allowEmoji: false,
...
const PERSONA_AFFECT_PRESETS: Readonly<Record<string, Partial<AffectPolicy>>> = Object.freeze({
...
function hashAffectValue(value: string = '', length: number = 10): string {
  const text = String(value || '').trim()
  if (!text) return ''
  return crypto.createHash('sha256').update(text).digest('hex').slice(0, Math.max(6, Math.min(32, Number(length) || 10)))
}

function normalizeAffectText(value: unknown = '', maxLength: number = 160): string {
  return String(value || '')
    .replace(/[\r\n\t]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
...
function normalizeAffectPolicy(input: Partial<AffectPolicy> = {}): AffectPolicy {
...
function resolveAffectPolicy(plan: AffectPlan = {}, options: { personaName?: string; policy?: Partial<AffectPolicy> } = {}): AffectPolicy {
...
function classifyAffectMood(input: AffectInput = {}): string {
...
function buildAffectRouterDiagnostic(input: AffectInput = {}): AffectDiagnostic {
...
function formatAffectRouterDiagnostic(diagnostic: Partial<AffectDiagnostic> = {}): string {
