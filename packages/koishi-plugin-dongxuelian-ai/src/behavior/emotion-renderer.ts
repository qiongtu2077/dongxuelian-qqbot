...
function createEmotionRenderTaskId(channelKey: string): string {
...
function scoreTone(score: number): EmotionTone {
...
function buildHistoryHtml(history: EmotionHistoryItem[]): string {
...
function buildReasonsHtml(reasons?: string[]): string {
...
function buildKeywordsHtml(keywords?: string[]): string {
  if (!keywords || !keywords.length) return ''
  return `<div class="keywords">${keywords.map(keyword => `<span>${esc(keyword)}</span>`).join('')}</div>`
}

function renderEmotionHtml(analysis: EmotionAnalysis, stats: EmotionStats, history: EmotionHistoryItem[] = []): string {
...
async function renderEmotionImage(analysis: EmotionAnalysis, stats: EmotionStats, history: EmotionHistoryItem[] = [], channelKey = ''): Promise<unknown> {
  ...
  try {
    ...
    if (gateHandle) gateHandle.release('emotion-render-finally')
  }
}

// 只执行 HTML 到图片的纯渲染；调用方必须已经完成 S1/S0/S2 资源保护。
async function renderEmotionImageDirect(analysis: EmotionAnalysis, stats: EmotionStats, history: EmotionHistoryItem[] = []): Promise<unknown> {
  return await renderHtmlToImage(renderEmotionHtml(analysis, stats, history))
}

export = { renderEmotionHtml, renderEmotionImage, renderEmotionImageDirect }
