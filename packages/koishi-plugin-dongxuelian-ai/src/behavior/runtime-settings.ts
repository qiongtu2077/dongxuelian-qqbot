...
const {
  ...
  RANDOM_TRIGGER_RATE_BASE,
  NUMERIC_GROUP_ID_RE,
} = require('../core/constants') as typeof import('../core/constants')
const { readJsonFile, getFileFingerprint } = require('../core/utils') as typeof import('../core/utils')

const randomWhitelistCache: Set<string> = new Set(DEFAULT_GROUP_RANDOM_WHITELIST)
const randomRateCache: Map<string, number> = new Map()

let runtimeSettingsLoaded = false
let runtimeSettingsFingerprint = ''

async function getRuntimeSettingsFingerprint(): Promise<string> {
...
function refreshRandomWhitelistCache(whitelist: unknown): void {
  randomWhitelistCache.clear()
  if (Array.isArray(whitelist)) {
    for (const item of whitelist) {
      const normalized = String(item || '').trim()
      if (NUMERIC_GROUP_ID_RE.test(normalized)) randomWhitelistCache.add(normalized)
    }
  } else {
    for (const item of DEFAULT_GROUP_RANDOM_WHITELIST) randomWhitelistCache.add(item)
  }
}

function refreshRandomRateCache(rateMap: unknown): void {
...
async function loadRuntimeSettings(force: boolean = false): Promise<void> {
