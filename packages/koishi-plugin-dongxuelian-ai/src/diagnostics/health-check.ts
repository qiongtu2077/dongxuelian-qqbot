...
function getHealthCheckResultContent(resultObj: Awaited<ReturnType<typeof requestChatCompletions>>): string {
  return typeof resultObj === 'string' ? resultObj : (resultObj.type === 'text' ? resultObj.content : '')
}

async function testProvider(providerId: string, providerDef: ProviderDefinition, allKeys: HealthKeySet): Promise<HealthProviderResult> {
...
async function readHealthKeyFile(file: string): Promise<string> {
  try { return (await fsp.readFile(file, 'utf8')).trim() } catch { return '' }
}

async function runHealthCheck(force: boolean = false): Promise<HealthReport> {
  const now = Date.now()
...
function resetHealthCache(): void {
  healthCache = null
  healthCacheTs = 0
}
