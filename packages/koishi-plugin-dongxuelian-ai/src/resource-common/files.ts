...
function readJsonFile<T = unknown>(file: string, fallback: T | null = null, maxBytes = 2 * 1024 * 1024): T | null {
  try {
...
function appendJsonlEvent(file: string, event: Record<string, unknown>): void {
  ensureDir(path.dirname(file))
...
function listJsonFiles(dir: string, options: ListJsonOptions = {}): string[] {
  const result: string[] = []
  ...
  const walk = (current: string): void => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (options.recursive) walk(full)
        continue
      }
      if (entry.isFile() && entry.name.endsWith('.json')) result.push(full)
    }
  }
  walk(dir)
  return result
...
// 原子移动文件，目标目录不存在时自动创建。
function renameFileAtomic(src: string, dst: string): boolean {
  try {
    ensureDir(path.dirname(dst))
    fs.renameSync(src, dst)
    return true
  } catch (error) {
    const code = String((error as FsCodeError)?.code || '')
    if (code === 'ENOENT' || code === 'EEXIST' || code === 'EPERM') return false
    throw error
  }
}

// 只读取 JSONL 文件尾部，避免 Dashboard 状态页同步读完整大日志。
function readJsonlTailLines(file: string): string[] {
  try {
    const stat = fs.statSync(file)
...
function readRecentJsonlEvents(dir: string, prefix: string, limit = 80): unknown[] {
  ...
  return lines.slice(-max).map(line => {
    try { return JSON.parse(line) } catch { return { event: 'invalid_event_line', raw: line.slice(0, 500) } }
  })
}
...
function isProcessAlive(pid: unknown): boolean {
  ...
  try {
    ...
    return String((error as FsCodeError)?.code || '') === 'EPERM'
  }
}
