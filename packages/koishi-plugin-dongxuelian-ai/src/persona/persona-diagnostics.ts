...
const { parsePersonaDocument, createPersonaDiagnostic, parsePersonaStringList } = require('./persona-schema') as typeof import('./persona-schema')
const { resolveVoiceSampleFile } = require('../media/voice/voice-assets') as typeof import('../media/voice/voice-assets')

const PERSONA_DIAGNOSTIC_SCAN_DIRS: ReadonlyArray<readonly [string, string]> = Object.freeze([
...
function readPersonaDiagnosticText(file: string): string {
  try {
    const stat = fs.statSync(file)
    if (!stat.isFile() || stat.size > MAX_PERSONA_DIAGNOSTIC_FILE_BYTES) return ''
    return fs.readFileSync(file, 'utf8')
  } catch { /* non-critical: read-only diagnostic scan treats unreadable file as warning */
    return ''
  }
}

function listPersonaDiagnosticFiles(dir: string): string[] {
  try {
    return fs.readdirSync(dir, { withFileTypes: true })
      .filter((entry: DiagnosticDirent) => entry.isFile() && PERSONA_SKILL_FILE_RE.test(entry.name))
      .map(entry => path.join(dir, entry.name))
      .sort((a, b) => path.basename(a).localeCompare(path.basename(b)))
  } catch { /* non-critical: missing optional persona resource directory scans as empty */
    return []
  }
}

function getPersonaDocumentName(doc: Partial<PersonaDocument> = {}): string {
...
function getDiagnosticLoreRefs(meta: Record<string, unknown> = {}): string[] {
  const refs: string[] = []
...
function buildPersonaDiagnosticIndexes(documents: PersonaDocument[] = []): { loreByName: Map<string, PersonaDocument>; docsByName: Map<string, PersonaDocument[]> } {
  const loreByName = new Map<string, PersonaDocument>()
  const docsByName = new Map<string, PersonaDocument[]>()
...
function addCrossDocumentDiagnostics(documents: PersonaDocument[] = [], options: ScanPersonaDocumentsOptions = {}): void {
...
function summarizePersonaDiagnostics(documents: PersonaDocument[] = []): PersonaDiagnosticSummary {
...
function scanPersonaDocuments(options: ScanPersonaDocumentsOptions = {}) {
  ...
  for (const [type, dir] of scanDirs) {
    for (const file of listPersonaDiagnosticFiles(dir)) {
      ...
      documents.push(parsePersonaDocument(content, { type, file }) as PersonaDocument)
    }
  }
...
function formatPersonaDiagnosticReport(result: { documents?: PersonaDocument[]; summary?: PersonaDiagnosticSummary } = {}): string {
