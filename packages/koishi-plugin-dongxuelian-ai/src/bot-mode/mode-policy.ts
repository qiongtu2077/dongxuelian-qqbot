...
function decideModePolicy(commandType: BotCommandType, snapshot: BotModeSnapshotLike | null | undefined): ModePolicyDecision {
  ...
  if (mode === 'maintenance') {
    if (commandType === 'normal_chat' || commandType === 'media_event') return { action: 'silent_drop', reason: 'maintenance mode' }
  ...
  if (mode === 'report_silent') {
    ...
    if (commandType === 'agent_command') return { action: 'reject', reason: 'agent is blocked during daily report' }
