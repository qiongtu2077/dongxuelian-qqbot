...
const { VOICES_DIR, VOICE_ASSETS_FILE } = require('../../core/constants') as typeof import('../../core/constants')
...
function sanitizeVoiceAssetId(value: unknown = ''): string {
...
function getAudioExtFromMime(mimeType: unknown = ''): string {
...
function getAudioMimeFromFilename(filename: unknown, fallback: string = 'audio/mpeg'): string {
  const ext = path.extname(String(filename || '')).slice(1).toLowerCase()
  return MIME_BY_EXT[ext] || fallback
}

function createVoiceAssetId(personaName: unknown = '', existingAssets: VoiceAsset[] | null = null): string {
...
function buildVoiceAssetFilename(assetId: unknown, mimeType: unknown = ''): string {
  const id = sanitizeVoiceAssetId(assetId)
  const ext = getAudioExtFromMime(mimeType)
  return sanitizeVoiceFilename(`${id}.${ext}`)
}

function sanitizeVoiceFilename(filename: unknown = ''): string {
...
function getVoiceFilePath(filename: unknown): string | null {
  const safe = sanitizeVoiceFilename(filename)
  if (!safe) return null
  return path.join(VOICES_DIR, safe)
}

function getVoiceFileInfo(filename: unknown): VoiceFileInfo | null {
  const safe = sanitizeVoiceFilename(filename)
  ...
  } catch { /* non-critical: missing voice sample is reported in metadata */
    return { filename: safe, filePath, size: 0, mtime: 0, mimeType: getAudioMimeFromFilename(safe), missing: true }
  }
}

function readStoreAssets(): Record<string, unknown>[] {
  try {
  ...
  } catch { /* non-critical: missing or invalid voice asset store starts as empty */
    return []
  }
}

function writeStoreAssets(assets: VoiceAsset[]): void {
  ...
  try {
    ...
    try { fs.unlinkSync(VOICE_ASSETS_FILE) } catch { /* non-critical: best-effort overwrite cleanup before voice store rename */
    }
    fs.renameSync(tmp, VOICE_ASSETS_FILE)
  }
}

function normalizeVoiceAsset(raw: Record<string, unknown> = {}, fileInfo: VoiceFileInfo | null = null): VoiceAsset {
  ...
  return {
    ...
    filename,
    mimeType: String(raw.mimeType || fileInfo?.mimeType || getAudioMimeFromFilename(filename)),
...
function inferPersonaName(baseName: string, personaConfigs: PersonaVoiceConfig[] = []): string {
...
function listVoiceAssets(personaConfigs: PersonaVoiceConfig[] = []): VoiceAsset[] {
...
function findVoiceAsset(assetIdOrName: unknown, personaConfigs: PersonaVoiceConfig[] = []): VoiceAsset | null {
...
function upsertVoiceAsset(meta: Record<string, unknown> = {}): VoiceAsset {
  ...
  const previous = existing.find(item => item.id === id || item.filename === filename) || null
...
function listVoiceAssetReferences(assetOrId: VoiceAsset | unknown, personaConfigs: PersonaVoiceConfig[] = []): string[] {
  const asset = typeof assetOrId === 'object' && assetOrId ? assetOrId as VoiceAsset : findVoiceAsset(assetOrId, personaConfigs)
  if (!asset) return []
...
function deleteVoiceAsset(assetIdOrName: unknown, personaConfigs: PersonaVoiceConfig[] = []): DeleteVoiceAssetResult | null {
  const asset = findVoiceAsset(assetIdOrName, personaConfigs)
  if (!asset) return null
  const deleted: string[] = []
  const filePath = getVoiceFilePath(asset.filename)
  if (filePath) {
    try { fs.unlinkSync(filePath); deleted.push(asset.filename) } catch { /* non-critical: metadata deletion still proceeds if sample is already gone */
    }
  }
  const existing = readStoreAssets().map(item => normalizeVoiceAsset(item))
...
function resolveVoiceSampleFile(personaName: unknown, voiceAssetId: unknown = ''): (VoiceAsset & VoiceFileInfo) | null {
