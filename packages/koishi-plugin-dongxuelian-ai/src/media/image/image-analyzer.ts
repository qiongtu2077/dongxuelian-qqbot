...
function getImageAnalyzerErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : String(error)
}

function imageTaskKey(channelKey: string, messageId: string): string {
  return `${String(channelKey || '')}::${String(messageId || '')}`
}

async function enqueueAnalysis(channelKey: string, messageId: string): Promise<void> {
...
async function runAnalysis({ channelKey, messageId, url, file }: ImageTask): Promise<string | null> {
  try {
    ...
    if (!base64 && file) {
      try {
        const imgInfo = await callGetImage(file) as ImageInfo | null
    ...
    const result: unknown = await requestChatCompletions(messages as unknown as Parameters<typeof requestChatCompletions>[0], config, { max_tokens: 200, _timeoutMs: ANALYSIS_TIMEOUT_MS })
...
async function analyzeImageNow(channelKey: string, messageId: string): Promise<string | null> {
