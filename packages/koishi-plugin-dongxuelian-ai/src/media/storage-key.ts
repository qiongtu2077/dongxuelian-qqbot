...
// 生成媒体历史和缓存文件共用的安全键。
export function getSafeMediaStorageKey(channelKey: unknown): string {
  const key = String(channelKey || '')
  return key ? safeChannelKey(key) : ''
}

// 生成旧版文件和图片历史使用的未哈希键，仅供懒迁移定位旧文件。
export function getLegacyUnsafeMediaStorageKey(channelKey: unknown): string {
  return String(channelKey || '').replace(/[^a-zA-Z0-9.:_-]/g, '_')
}

// 返回指定媒体历史目录中的当前安全文件路径。
export function getMediaHistoryFilePath(historyDir: string, channelKey: unknown): string {
  return path.join(historyDir, getSafeMediaStorageKey(channelKey) + '.json')
}

// 返回需要懒迁移的旧版历史文件路径；新旧键一致时不触发迁移。
export function getLegacyMediaHistoryFilePath(historyDir: string, channelKey: unknown): string {
