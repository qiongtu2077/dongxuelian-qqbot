...
interface XlsxModule {
  read(buffer: Buffer, options: { type: string }): SpreadsheetWorkbook
  utils: {
    sheet_to_csv(sheet: SpreadsheetSheet, options: { FS: string }): string
  }
}

const queue: FileAnalysisTask[] = []
const inFlight: Map<string, Promise<string | null>> = new Map()

function getFileAnalyzerErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : String(error)
}

function taskKey(channelKey: string, messageId: string): string {
  return `${String(channelKey || '')}::${String(messageId || '')}`
}

async function enqueueFileAnalysis(channelKey: string, messageId: string): Promise<void> {
  if (!channelKey || !messageId) return
  try {
    const entry = await getFileEntry(channelKey, messageId)
    if (!entry || entry.analyzed || entry.skipped) return
    const key = taskKey(channelKey, messageId)
    if (inFlight.has(key) || queue.some(item => taskKey(item.channelKey, item.messageId) === key)) return
...
async function downloadFile(url: string, destPath: string, redirectCount: number = 0): Promise<string> {
  if (redirectCount > MAX_REDIRECTS) throw new Error('redirect 超过上限')
...
async function parseFile(filePath: string, ext: string): Promise<string | null> {
...
async function downloadWithFallback(url: string, fileId: string | null | undefined, destPath: string, messageId: string = ''): Promise<string | null> {
  if (url) {
    try {
  ...
  for (const candidate of candidates) {
    try {
      const { callGetFile } = require('../../core/api') as typeof import('../../core/api')
      const data = await callGetFile(candidate) as GetFileResult | null
      if (data && data.file) {
        try { await fs.access(data.file); return data.file } catch { /* non-critical: inaccessible OneBot file path falls back to URL */
        }
      }
      if (data && data.url) {
...
async function runAnalysis({ channelKey, messageId }: FileAnalysisTask): Promise<string | null> {
  try {
    const entry = await getFileEntry(channelKey, messageId) as FileEntryLike | null
    if (!entry || entry.analyzed || entry.skipped) return null
...
async function analyzeFileNow(channelKey: string, messageId: string): Promise<string | null> {
