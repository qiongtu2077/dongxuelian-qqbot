...
function shouldRetryRepeatedReply(session: SessionLike, reply: string = ''): boolean {
...
function buildRepeatRetryPrompt(userText: string, recentReplies: string[] = []): string {
...
function pickAbusiveFallbackReply(session: SessionLike): string {
...
function pickRepeatedFallbackReply(session: SessionLike): string {
...
function isConsecutiveUserRepeat(session: SessionLike, userText: string = ''): boolean {
...
function isUnsafeThinkingReply(reply: string = ''): boolean {
  const value = String(reply || '')
  return isThinkingLeak(value) || THINKING_OUTPUT_RE.test(value)
}

function stripStickerMarkersForGuard(reply: string = ''): string {
  return String(reply || '').replace(/\[图:[^\[\]]+\]/g, '').trim()
}

function hasInternalContextLeak(text: string = ''): boolean {
...
function extractContentTokens(text: unknown = ''): Set<string> {
...
function hasSceneItemMedia(item: SceneItemLike = {}): boolean {
...
function detectOldMediaTopicSticking({ reply, currentTurn = [], oldBackground = [], hotContext = [], hasCurrentMediaCue = false }: OldMediaStickingInput = {}): boolean {
...
function buildOldMediaStickingRetryPrompt(): string {
  return [
