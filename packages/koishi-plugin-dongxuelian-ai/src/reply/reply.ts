...
const { STICKER_DIR, THROTTLE_CONFIG_FILE } = require('../core/constants') as typeof import('../core/constants')
const { getChannelKey, getConversationKey, saveSharedChannelTurn } = require('../conversation') as typeof import('../conversation')
const { splitReplyForQQBubbles, sleep, getRandomDelayMs, sanitizeUserName } = require('../core/utils') as typeof import('../core/utils')
...
interface ReplySessionLike {
  ...
  messageId?: string
  selfId?: string
  author?: { nick?: string; name?: string; id?: string }
  bot?: { selfId?: string; internal?: InternalBotLike }
...
const lastStickerSentAt: Map<string, number> = new Map()
const throttleWindow: Map<string, number[]> = new Map()
let throttleWindowLastCleanup = 0
function getReplyErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : String((error as ErrorWithMessage)?.message || error)
}

function asReplyBasicSession(session: ReplySessionLike): BasicSessionLike {
  return session as BasicSessionLike
}

function cleanupThrottleWindow(): void {
  const now = Date.now()
...
const lastStickerFileSentAt: Map<string, number> = new Map()

let throttleCfgCache: ThrottleConfig | null = null
let throttleCfgLastRead = 0

let stickerFileIndex: Map<string, StickerMeta> = new Map()
let stickerBase64Cache: Map<string, string> = new Map()

function stripInternalParenthetical(text: string = ''): string {
...
function loadStickerCache(): void {
  ...
  } catch { /* non-critical: sticker cache is optional */ }
}
...
function getStickerImage(file: string): string {
  ...
  try {
    ...
    while (stickerBase64Cache.size > MAX_STICKER_CACHE_FILES) {
      ...
      if (oldest !== undefined) stickerBase64Cache.delete(oldest)
    }
    return image
  } catch { /* non-critical: sticker cache file read falls back to text-only reply */
    return ''
  }
}

// 表情包映射（按关键词长度降序，最长优先）
const STICKER_MAP: StickerMapEntry[] = [
  { kw: '绷不住了', file: '憋笑.jpg' },
...
async function sendStickerImage(ctx: ReplyContext, session: ReplySessionLike, sticker: PendingSticker | string): Promise<StickerSendResult> {
  ...
  if (bot?.internal && userId) {
    try {
      ...
      internalError = error
      logger.warn(`sticker internal send failed${file ? ` (${file})` : ''}: ${getReplyErrorMessage(error)}`)
    }
  } else {
  ...
  try {
    ...
    const internalMsg = internalError ? `; internal=${getReplyErrorMessage(internalError)}` : ''
    logger.error(`sticker fallback send failed${file ? ` (${file})` : ''}: ${getReplyErrorMessage(error)}${internalMsg}`)
    return { ok: false }
  }
}

function buildAssistantImageMessageId(session: ReplySessionLike, sticker: Partial<PendingSticker>): string {
...
function getAssistantSpeakerName(options: SendReplyOptions = {}): string {
  const personaName = sanitizeUserName(String(options.personaName || '')).slice(0, 40)
  return personaName || '东雪莲'
}

async function recordAssistantImageAnchor(session: ReplySessionLike, sticker: Partial<PendingSticker> = {}, sent: StickerSendResult = { ok: false }, options: SendReplyOptions = {}): Promise<void> {
  if (!sticker.file) return
  const channelKey = getChannelKey(asReplyBasicSession(session))
  ...
  try {
    const { storeAssistantImageAnchor } = require('../media/image/image-store') as typeof import('../media/image/image-store')
    await storeAssistantImageAnchor(channelKey, messageId, {
      file: sticker.file,
      conversationKey: getConversationKey(asReplyBasicSession(session)),
      userId: session.selfId || session.bot?.selfId || 'bot',
      ts: Date.now(),
    })
  } catch { /* non-critical: assistant image anchor is best-effort metadata */ }
}

function resolveNow(options: SendReplyOptions): () => number {
  if (typeof options?.now === 'function') return options.now
  if (typeof options?.time?.now === 'function') return options.time.now
  return Date.now
}

async function sendReply(ctx: ReplyContext, session: ReplySessionLike, reply: string, isRandom: boolean = false, options: SendReplyOptions = {}): Promise<number> {
  ...
  const maxPerMin = parseInt(String(cfg?.maxPerMinute || ''), 10) || 0
    if (maxPerMin > 0) {
  ...
  } catch { /* non-critical: invalid throttle config should not block replies */ } // 配置文件不存在或不合法时直接放行
  // 图片文件转 base64 CQ 码（使用缓存）
  const stickerToCQ = (file: string): string => {
    return getStickerImage(file)
  }
  // 替换 AI 主动调用的 [图:xxx] 并收集图片 base64
  const pendingStickers: PendingSticker[] = []
  const addPendingSticker = (file: string): void => {
    const image = stickerToCQ(file)
    if (image && !pendingStickers.some(sticker => sticker.file === file)) {
      pendingStickers.push({ file, image })
    }
  }
  reply = String(reply || '').replace(/\[图:(.+?)\]/g, (m, name) => {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    try {
      ...
      const enrichedError = sendError as Error & { sentParts?: number }
      enrichedError.sentParts = sentParts
      ctx.logger('dongxuelian-ai').warn(`sendReply failed: ${getReplyErrorMessage(sendError)}`)
      throw enrichedError
    }
    sentParts++
    if (i < parts.length - 1) {
  ...
  const stickerChannelKey = getChannelKey(asReplyBasicSession(session))
