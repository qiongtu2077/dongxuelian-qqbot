...
const { isUnsafeThinkingReply, hasInternalContextLeak } = require('./reply/reply-guard') as typeof import('./reply/reply-guard')
...
const { handleEmotionCommand } = require('./commands/emotion-command') as typeof import('./commands/emotion-command')
...
const forgetPendingConfirm = new Map<string, number>()
let lastForgetCleanupTs = 0

function trimForgetPendingConfirm(now = Date.now()): void {
...
function isGroupAdmin(session: HandlerSession): boolean {
  if (!session?.event?.sender?.role) return false
  return session.event.sender.role === 'owner' || session.event.sender.role === 'admin'
}

function isGroupAdminOrBotAdmin(session: HandlerSession): boolean {
...
async function handleOperationalCommandDomain(session: HandlerSession, ctx: HandlerContext, state: HandlerState): Promise<HandlerCommandResult> {
  ...
  if (/^(?:东雪莲)?测试开$/.test(plain)) {
    try { require('fs').writeFileSync(TEST_MODE_FILE, 'on') } catch (e) { ctx.logger('dongxuelian-ai').warn(`test mode enable failed: ${getHandlerErrorMessage(e)}`) }
  ...
  if (/^(?:东雪莲)?嘴臭开$/.test(plain)) {
    ...
    try { require('fs').writeFileSync(HOSTILE_MODE_FILE, 'on') } catch (e) { ctx.logger('dongxuelian-ai').warn(`hostile mode enable failed: ${getHandlerErrorMessage(e)}`) }
  ...
  if (/^谁(?:艾特|@)我$/.test(plain)) {
    ...
    let cache: HandlerTodayCache | null = null
    try { cache = JSON.parse(require('fs').readFileSync(cacheFile, 'utf8')) as HandlerTodayCache } catch { /* non-critical: missing or invalid today-cache means no @ summary yet */ }
  ...
  if (locateMatch) {
    ...
    const cacheFile = path.join(DATA_DIR, 'today-cache-' + safeKey + '.json')
    let cache: HandlerTodayCache | null = null
    try { cache = JSON.parse(require('fs').readFileSync(cacheFile, 'utf8')) as HandlerTodayCache } catch { /* non-critical: missing or invalid today-cache means locate cannot proceed */ }
    if (!cache || !Array.isArray(cache.messages) || !cache.messages.length) {
...
async function handleConversationCommandDomain(session: HandlerSession, ctx: HandlerContext, state: HandlerState): Promise<HandlerCommandResult> {
  const {
    plain, inGuild, channelKey, currentUserId,
    callOpenAI, setRepeatEnabled, repeatEnabledCache,
  } = state

  // Builds one generated-command prompt against the currently resolved persona.
  function buildPersonaCommandSystem(taskPrompt: string): string {
  ...
  function cleanGeneratedCommandReply(text: unknown = ''): string {
  ...
  if (plain.startsWith('东雪莲群记忆定时') && plain !== '东雪莲群记忆定时') {
    ...
    if (value === '关') {
      try { await safeUnlink(path.join(DATA_DIR, 'memory-timers', safeChannelKey(channelKey) + '.json')) } catch { /* non-critical: disabling an absent timer is already the desired state */ }
    ...
    const timerFile = path.join(DATA_DIR, 'memory-timers', safeChannelKey(channelKey) + '.json')
    try { require('fs').mkdirSync(path.join(DATA_DIR, 'memory-timers'), { recursive: true }) } catch { /* non-critical: writeJsonFile will report the actual failure if mkdir did not succeed */ }
...
async function handleCommand(session: HandlerSession, ctx: HandlerContext, state: HandlerState): Promise<HandlerCommandResult> {
  ...
  if (operationalResult.matched) return operationalResult

  const conversationResult = await handleConversationCommandDomain(session, ctx, state)
  if (conversationResult.matched) return conversationResult

  const {
    plain, channelKey, currentUserId, adminCommandMatched,
    loadConfig, loadRuntimeSettings, loadSkills, loadSkillsContentCache,
    resetConfigCache, getSkillsCount, getThinkingEnabled,
    getRandomTriggerBaseRate, getRandomWhitelistStatus,
    channelMissCount,
  } = state
