...
async function main() {
  ...
  check('dashboard queues agent chat API through S2 worker', dashboardAgentRoutesSrc.includes("require(path.join(AI_LIB, 'agent', 'worker-submission'))") && dashboardAgentRoutesSrc.includes("require(path.join(AI_LIB, 'resource-workers', 'agent-payload'))") && dashboardAgentRoutesSrc.includes('submitAgentWorkerTask') && dashboardAgentRoutesSrc.includes('createAgentRunWorkerPayload') && dashboardAgentRoutesSrc.includes('async: true') && dashboardAgentRoutesSrc.includes("status: submission.accepted ? 'accepted' : 'blocked'") && dashboardAgentRoutesSrc.includes('taskId: submission.taskId'))
  ...
  check('dashboard exposes async agent task polling API', dashboardAgentRoutesSrc.includes('/^\\/dashboard\\/api\\/agent\\/tasks\\/([^/]+)$/') && dashboardAgentRoutesSrc.includes("require(path.join(AI_LIB, 'resource-workers', 'task-store'))") && dashboardAgentRoutesSrc.includes('getResourceTaskById(taskId)') && dashboardAgentRoutesSrc.includes('readAgentTaskResult(taskId)') && dashboardAgentRoutesSrc.includes('sanitizeAgentTaskForDashboard(task, result)'))
