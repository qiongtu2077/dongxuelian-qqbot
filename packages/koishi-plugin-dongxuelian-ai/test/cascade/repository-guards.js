...
async function runRepositoryGuards(context) {
  ...
  const koishiServiceInstaller = read(path.join(scriptsDir, 'install-koishi-service.sh'))
  ...
  check('restart-bot delegates local koishi binary to the dedicated systemd service', restartBot.includes('systemctl restart lian-koishi') && !restartBot.includes('nohup') && !restartBot.includes('KOISHI_PID') && koishiServiceInstaller.includes('ExecStart=$NODE_BIN $APP_DIR/node_modules/koishi/bin.js start'))
