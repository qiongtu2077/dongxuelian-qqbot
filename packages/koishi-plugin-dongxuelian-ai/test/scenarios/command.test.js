...
async function run(t) {
  ...
  await withScenario({}, async ({ data, harness, makeSession, run }) => {
    ...
    t.check('scenario locate message quotes displayed first item', locateFirst.sent.length === 1 && locateFirst.sent[0] === '<quote id="at-1"/>找到啦！', JSON.stringify(locateFirst.sent))
    data.writeJson('today-cache-10001.json', {
      date: today,
      messages: [
