...
const { TEST_MODE_FILE, HOSTILE_MODE_FILE, REQUEST_TIMEOUT, MAX_OUTPUT_CHARS_FRIENDLY, MAX_OUTPUT_CHARS_ABUSIVE, USER_PROFILE_DIR, POLITICAL_DETECT_FILE, JAILBREAK_OUTPUT_RE, JAPAN_SELF_IDENTIFY_RE, GENERATION_REQUEST_RE, SENSITIVE_KEYWORDS_RE, } = require('./core/constants');
    ...
    if (rareConfirmed && !isRareProvocation(cleanInput) && !japanLinked) {
        try {
            ...
            const rareJudgeObj = await requestChatCompletions([{ role: 'system', content: '你是一个内容判断器。判断以下用户消息是否在阴阳 Bot 的国籍、稀有度或身份归属。只输出一个字：Y 或 N。不要输出任何其他文字。' },
                { role: 'user', content: cleanInput.slice(0, 200) }], cfg, { max_tokens: 5 });
            ...
            if (rawMessages) {
                ...
                try {
                    ...
                    const summaryResult = await requestChatCompletions([{ role: 'system', content: '把以下发言用 200 字以内概括其发言风格和常用话题，越精炼越好。' },
                        { role: 'user', content: rawMessages }], config, { max_tokens: 200, _timeoutMs: 8000 });
