...
const { TEST_MODE_FILE, HOSTILE_MODE_FILE, REQUEST_TIMEOUT, MAX_OUTPUT_CHARS_FRIENDLY, MAX_OUTPUT_CHARS_ABUSIVE, PROVIDERS, DASHSCOPE_KEY_FILE, GLM_KEY_FILE, USER_PROFILE_DIR, POLITICAL_DETECT_FILE, JAILBREAK_OUTPUT_RE, JAPAN_SELF_IDENTIFY_RE, GENERATION_REQUEST_RE, SENSITIVE_KEYWORDS_RE, } = require('./core/constants');
...
const { requestChatCompletions, // 通用 LLM 请求（含 fallback 链）
...
const { markAnalyzed, replaceImagePlaceholder } = require('./media/image/image-store');
const { sanitizeImageAnalysis } = require('./media/image/image-analysis-sanitizer');
const { getConversationKey, getChannelKey, // 会话/频道唯一标识生成
...
const { testChatPromptRegex, createChatPromptBaseMessages, createChatPromptNsfwMessage, resolveChatPromptPersonaLore, createChatPromptLoreMessage, createChatPromptSearchRuleMessage, createChatPromptRandomContextMessage, createChatPromptForwardSummaryMessage, createChatPromptShortFollowUpMessage, createChatPromptGenerationRequestMessage, createChatPromptRareContextMessage, createChatPromptConversationSummaryMessage, createChatPromptMemoryMessage, createChatPromptHistoryBackgroundMessage, createChatPromptSeriousQuestionMessage, createChatPromptUncertainQuestionMessage, createChatPromptPoliticalSensitiveMessage, createChatPromptHostileEvaluationMessage, createChatPromptPlainUserMessage, } = require('./chat/chat-prompt-builder'); // prompt 片段构造器（纯函数）
...
const { isRareProvocation, isWideRareProvocation, isHostileInput, // 挑衅/敌意检测
...
const { trimChatMemoryRuntime, clearGroupMemoryIfExpired, handleDirectMemoryWrite, handleMemoryConfirmation, rememberMemoryPrompt, } = require('./chat/chat-memory');
const { finalizeChatReply, } = require('./chat/chat-final-output-flow');
const { isContextJailbroken, chatJailbreak, } = require('./chat/chat-jailbreak-flow');
const { resolveTopicSwitch, } = require('./chat/chat-topic-switch');
const { retellAgentResultForChat, } = require('./chat/chat-agent-retell-flow');
const { redactSensitiveText } = require('./core/redactor');
const { loadConfig, // 加载运行时配置（API key/model/provider）
...
function asChatTools(tools) {
    return tools;
}
function asVisionMessages(messages) {
    return messages;
}
function asChatToolFlowMessages(messages) {
    return messages;
}
function asFinalizeMessages(messages) {
    return messages;
}
function getChatErrorMessage(error) {
    return error instanceof Error ? error.message : String(error || '');
}
function getChatCompletionText(result) {
...
function extractVisionObjectiveFact(reply = '') {
    ...
    if (firstSentenceMatch && firstSentenceMatch[1])
        candidates.push(String(firstSentenceMatch[1] || '').trim());
    ...
    if (!fact)
        return { visibleReply: trimmed, fact: '' };
...
async function syncVisionAnalysisArtifacts(channelKey, messageId, analysis) {
    if (!channelKey || !messageId || !analysis)
        return;
    try {
...
async function callOpenAI(messages, isRandom, extraBody = {}, tools = null) {
    ...
    if (!config.apiKey)
        throw new Error('AI key file is empty.');
...
async function prepareChatIdentityStage(session, userText, ctx, options) {
    ...
    if (personaName) {
        ...
        if (!personaSkillContent) {
            try {
                ctx.logger('dongxuelian-ai').warn(`persona bound but skill load failed: persona=${personaName} channelKey=${channelKey} source=${personaResolution.source}`);
            }
    ...
    if (directMemoryReply)
        return { earlyReply: directMemoryReply };
...
async function executeChatModelStage(input) {
    const { session, ctx, options, messages, isolatedUserMessage, cleanInput, isRandomTriggered, hostile, currentUserId, channelKey, systemPrompt, currentUserMessage, userName, retaliationLevel, rareConfirmed, } = input;
    ...
    const reply = await callOpenAI(messages, isRandomTriggered, {}, chatTools);
    ...
    if (wasVisionRequest && visionContext && isVisionBlindnessReply(replyText)) {
        if (downgradeVisionMessageToText(asVisionMessages(messages), visionContext, '[图片暂时取不到，请按当前文字上下文回复，不要假设你看到了什么图]')) {
            try {
                ctx.logger('dongxuelian-ai').warn(`vision blindness detected, downgrading. provider=${visionContext.provider} model=${visionContext.model} reply=${replyText.slice(0, 60)}`);
            }
            catch { /* non-critical: logger may be unavailable in tests */ }
            replyText = await callOpenAIForText(messages, !!options.randomTriggered);
            visionContext = null;
...
async function chat(session, userText, ctx, options = {}) {
    ...
    if (identity.earlyReply !== null)
        return identity.earlyReply;
    const { skillsContentCache, cleanInput, isRandomTriggered, rareProvocation, japanLinked, wideRareHit, channelKey, currentUserId, personaName, personaSkillContent, retaliationLevel, hostile, systemPrompt, dynamicTimePrompt, userName, safeUserName, currentUserMessage, botSelfId, now, } = identity;
    ...
    if (mentionsBot)
        mentionTagParts.push('[此条@你本人]');
    if (otherMentionIds.length)
        mentionTagParts.push('[此条还@了群友：' + otherMentionIds.slice(0, 5).join('、') + '。提到的内容针对那些群友，不是针对你；除非也@你或直接喊你的名字，否则不要把自己代入]');
    ...
    if (forwardSummaryMessage)
        messages.push(forwardSummaryMessage);
    if (cleanInput && cleanInput.length <= 6) {
        ...
        if (recentAssistant && /[?？吗呢吧嘛]\s*$/.test(recentAssistant)) {
            ...
            if (shortFollowUpMessage)
                messages.push(shortFollowUpMessage);
        }
    }
    const generationRequestMessage = createChatPromptGenerationRequestMessage(cleanInput, GENERATION_REQUEST_RE);
    if (generationRequestMessage)
        messages.push(generationRequestMessage);
    let rareConfirmed = wideRareHit;
    if (rareConfirmed && !isRareProvocation(cleanInput) && !japanLinked) {
        try {
            const cfg = await loadConfig();
            const rareJudgeObj = await requestChatCompletions([{ role: 'system', content: '你是一个内容判断器。判断以下用户消息是否在阴阳 Bot 的国籍、稀有度或身份归属。只输出一个字：Y 或 N。不要输出任何其他文字。' },
                { role: 'user', content: cleanInput.slice(0, 200) }], cfg, { max_tokens: 5, _fallbackSet: 'lightweight' });
    ...
    if (conversationSummaryMessage)
        messages.push(conversationSummaryMessage);
    ...
    if (isDebugLogEnabled('persona-profile')) {
        try {
            ...
            appendPersonaProfileShadowLog(shadowPreview)
                ...
                try {
                    logDebug(ctx, 'persona-profile', `profile_shadow_jsonl written=true file=${path.basename(String(writeResult.file || ''))} candidates=${candidates} mode=shadow_only prompt=unchanged`);
                }
                catch { /* non-critical: debug log failure must not affect chat reply */ }
            })
                .catch(error => {
                try {
                    logDebug(ctx, 'persona-profile', `profile_shadow_jsonl_failed reason=${String((error && error.message) || 'unknown').slice(0, 80)} mode=shadow_only prompt=unchanged`);
                }
                catch { /* non-critical: debug log failure must not affect chat reply */ }
            });
        }
    ...
    if (historyBackgroundMessage)
        messages.push(historyBackgroundMessage);
            ...
            if (rawMessages) {
                ...
                for (const am of summaryModels) {
                    ...
                    try {
                        ...
                        const summaryResult = await requestChatCompletions([{ role: 'system', content: '把以下发言用 200 字以内概括其发言风格和常用话题，越精炼越好。' },
                            { role: 'user', content: rawMessages }], { model: am.model, baseURL: provDef.baseURL.replace(/\/+$/, ''), apiKey, provider: am.provider }, { max_tokens: 200, signal: ac.signal, _fallbackSet: 'lightweight' });
    ...
    if (seriousQuestionMessage)
        messages.push(seriousQuestionMessage);
    // 不确定问题不要胡编
    ...
    if (uncertainQuestionMessage)
        messages.push(uncertainQuestionMessage);
    ...
    if (politicalSensitiveMessage)
        messages.push(politicalSensitiveMessage);
...
module.exports = {
    ...
    loadConfig: loadConfig, // re-export: 运行时配置加载
    ...
    getThinkingArgs: getThinkingArgs, // re-export: thinking 模式参数
