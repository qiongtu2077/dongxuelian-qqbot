...
const { STICKER_DIR, THROTTLE_CONFIG_FILE } = require('../core/constants');
const { getChannelKey, getConversationKey, saveSharedChannelTurn } = require('../conversation');
const { splitReplyForQQBubbles, sleep, getRandomDelayMs, sanitizeUserName } = require('../core/utils');
...
function loadStickerCache() {
    try {
        ...
        for (const f of files.slice(0, MAX_STICKER_INDEX_FILES)) {
            ...
            if (!stat.isFile() || stat.size > MAX_STICKER_FILE_BYTES)
                continue;
...
const STICKER_NEG_RE_MAP = new Map(STICKER_MAP.map(s => [s.kw, new RegExp('不.{0,3}' + s.kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))]));
        ...
        try {
            ...
            if (isDirect) {
                ...
                if (typeof sendPrivateMsg !== 'function')
                    throw new Error('missing sendPrivateMsg for private sticker send');
                result = await sendPrivateMsg(userId, segArr);
            }
            else {
                if (!session.guildId)
                    throw new Error('missing guildId for group sticker send');
                const sendGroupMsg = bot.internal.sendGroupMsg;
                if (typeof sendGroupMsg !== 'function')
                    throw new Error('missing sendGroupMsg for group sticker send');
...
async function recordAssistantImageAnchor(session, sticker = {}, sent = { ok: false }, options = {}) {
    if (!sticker.file)
        return;
    ...
    try {
        const { storeAssistantImageAnchor } = require('../media/image/image-store');
...
function resolveNow(options) {
    if (typeof options?.now === 'function')
        return options.now;
    if (typeof options?.time?.now === 'function')
        return options.time.now;
    return Date.now;
}
async function sendReply(ctx, session, reply, isRandom = false, options = {}) {
    ...
    try {
        ...
        if (!throttleCfgCache || now - throttleCfgLastRead > 30000) {
            try {
                throttleCfgCache = JSON.parse(String(fs.readFileSync(THROTTLE_CONFIG_FILE, 'utf8') || '').replace(/^\uFEFF/, ''));
            }
            catch {
                throttleCfgCache = null;
            }
    ...
    if (!reply.includes('[CQ:image')) {
        ...
        const matched = STICKER_MAP.find(s => !autoSkip.has(s.kw) && reply.includes(s.kw) &&
