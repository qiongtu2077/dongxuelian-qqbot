 ...
 */
const { THINKING_OUTPUT_RE, ABUSIVE_FALLBACK_REPLIES, REPEATED_FALLBACK_REPLIES, } = require('../core/constants');
const { getReplyFingerprintHistory, getRecentAssistantReplies, getRecentUserMessages, } = require('../conversation');
const { normalizeReplyFingerprint, isReplyTooSimilar, isOverusedReply, isThinkingLeak, } = require('../core/utils');
function shouldRetryRepeatedReply(session, reply = '') {
    if (!reply)
        return false;
    if (isOverusedReply(reply))
        return true;
...
function extractContentTokens(text = '') {
    ...
    for (const m of value.matchAll(/[一-龥]{3,8}/g))
        tokens.add(m[0]);
    for (const m of value.matchAll(/[A-Za-z0-9]{4,16}/g))
...
function hasSceneItemMedia(item = {}) {
    if (!item)
        return false;
    if (Array.isArray(item.anchors) && item.anchors.some(anchor => anchor && /^(image|file|voice|forward)$/.test(String(anchor.type || ''))))
        return true;
...
function detectOldMediaTopicSticking({ reply, currentTurn = [], oldBackground = [], hotContext = [], hasCurrentMediaCue = false } = {}) {
    ...
    if (!replyText)
        return false;
    if (hasCurrentMediaCue)
        return false;
    if (currentTurn.some(hasSceneItemMedia))
        return false;
    const olderMediaItems = [...oldBackground, ...hotContext].filter(hasSceneItemMedia);
    if (!olderMediaItems.length)
        return false;
    ...
    for (const item of currentTurn) {
        for (const token of extractContentTokens(item?.content || ''))
            currentTurnTokens.add(token);
    }
    const oldMediaTokens = new Set();
    for (const item of olderMediaItems) {
        for (const token of extractContentTokens(item?.content || ''))
            oldMediaTokens.add(token);
    }
    let stickingHits = 0;
    for (const token of replyTokens) {
        if (oldMediaTokens.has(token) && !currentTurnTokens.has(token))
            stickingHits += 1;
        if (stickingHits >= 1)
            return true;
    }
