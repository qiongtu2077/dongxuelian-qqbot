...
function buildReplyTimingDiagnostic(input = {}) {
    ...
    const effectiveRate = replyTimingNumber(input.effectiveRate === undefined ? baseRate * willFactor : input.effectiveRate, 0, 0, 1);
    const missCount = Math.max(0, Math.floor(replyTimingNumber(input.missCount, 0, 0, Number.MAX_SAFE_INTEGER)));
    const reasons = [];
    const blockers = [];
    if (isPrivate)
        addReplyTimingTag(reasons, 'private_must_reply');
    if (directAt)
        addReplyTimingTag(reasons, 'direct_at_must_reply');
    if (nameMentioned)
        addReplyTimingTag(reasons, 'name_mentioned_must_reply');
    if (inGuild && inRandomWhitelist)
        addReplyTimingTag(reasons, 'random_whitelist_allowed');
    if (isRandomCandidate)
        addReplyTimingTag(reasons, 'legacy_random_candidate');
    if (randomHit)
        addReplyTimingTag(reasons, 'legacy_probability_hit');
    if (randomTriggered)
        addReplyTimingTag(reasons, 'legacy_random_triggered');
    if (delayedRandomScheduled)
        addReplyTimingTag(reasons, 'delayed_for_consecutive_messages');
    if (highRisk)
        addReplyTimingTag(reasons, 'persona_switch_quote_risk');
    if (Math.abs(willFactor - 1) > 0.0001)
        addReplyTimingTag(reasons, 'will_factor_applied');
    if (missCount > 0)
        addReplyTimingTag(reasons, 'miss_count_accumulated');
    if (phase !== 'initial')
        addReplyTimingTag(reasons, phase);
    if (inGuild && !directAt && !nameMentioned && !inRandomWhitelist)
        addReplyTimingTag(blockers, 'random_whitelist_missing');
    if (otherMentions)
        addReplyTimingTag(blockers, 'other_mentions_present');
    if (replyTimingBoolean(input.skipForRandomReply))
        addReplyTimingTag(blockers, 'content_skipped_for_random');
    if (cooldownActive)
        addReplyTimingTag(blockers, 'random_cooldown_active');
    if (mutedActive)
        addReplyTimingTag(blockers, 'channel_muted_by_user_command');
    if (explicitCanceled)
        addReplyTimingTag(blockers, 'explicit_interaction_cancelled');
    if (!replyTimingBoolean(input.hasUsableText) && !replyTimingBoolean(input.hasVisual))
        addReplyTimingTag(blockers, 'no_usable_text');
    if (isRandomCandidate && effectiveRate <= 0)
        addReplyTimingTag(blockers, 'random_rate_zero');
    if (isRandomCandidate && !randomHit && effectiveRate > 0)
        addReplyTimingTag(blockers, 'legacy_probability_miss');
    let decision = 'silent';
