 ...
 */
const { getGroupMemberInfo, getGroupInfo } = require('../core/api');
...
function toSendErrorLike(error) {
...
function getCachedPlatformMuteStatus(session, now = Date.now()) {
    if (!isGroupSession(session))
        return { muted: false, skipped: true, reason: '非群聊' };
    const key = getSendChannelKey(session);
    const cached = platformMuteCache.get(key);
    if (!cached)
        return { muted: false };
    if (cached.until > now)
        return { muted: true, until: cached.until, reason: cached.reason || '平台禁言缓存', source: cached.source || 'cache' };
...
function inspectMemberMute(memberInfo, now) {
    ...
    if (until > now)
        return { muted: true, until, reason: 'Bot 当前处于群成员禁言状态', source: 'member' };
    return null;
}
function inspectGroupAllMute(groupInfo, now) {
    ...
    if (allMuted && !adminCanSpeak)
        return { muted: true, until: now + GROUP_ALL_MUTE_CACHE_MS, reason: '群处于全员禁言状态', source: 'group' };
    return null;
}
async function checkPlatformMuteStatus(session, options = {}) {
    ...
    if (!groupId || !userId)
        return { muted: false, uncertain: true, reason: '缺少群号或 Bot QQ' };
