...
async function retellToolBlockedReply(chatResult, {
  ctx,
  session,
  userText,
  randomTriggered,
...
async function retellAgentResult(agentResult, {
  ctx,
  session,
  channelKey,
  currentUserId,
...
async function handleChatResult(chatResult, {
  ctx,
  session,
  channelKey,
  currentUserId,
  ...
  if (chatResult && typeof chatResult === 'object' && chatResult.heavyToolsRequested) {
    ...
    const searchGateBlocked = searchContext && ['needs_chat_handling', 'blocked_by_cold'].includes(searchContext.searchReadiness)
