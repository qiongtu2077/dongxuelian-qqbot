...
function buildSearchQueries(rawQuery = '') {
    ...
    if (!isWuwaLatestRoleQuery(query) && !isMinecraftUpdateQuery(query) && !isHotVideoQuery(query) && !isResourceVideoQuery(query) && GENERAL_LATEST_RE.test(query)) {
        ...
        if (/[a-z]/i.test(query))
            pushSearchQuery(queries, `${query} official source latest`);
    }
    if (query)
        pushSearchQuery(queries, query);
    const seen = new Set();
...
function scoreSearchResult(item = {}, query = '') {
    ...
    if (WUWA_RE.test(query) && WUWA_RE.test(haystack))
        score += 20;
    if (MINECRAFT_RE.test(query) && MINECRAFT_RE.test(haystack))
        score += 20;
    if (LATEST_ROLE_RE.test(query) && /(?:角色|共鸣者|卡池|公告|版本|前瞻|更新|新(?:角色|共鸣者))/.test(haystack))
        score += 18;
    if (LATEST_VERSION_RE.test(query) && /(?:版本|更新|release|released|update|snapshot|pre-release|changelog|patch notes)/i.test(haystack))
        score += 18;
    if (/官方|公告|新闻|资讯|版本|更新|前瞻|共鸣者/.test(haystack))
        score += 12;
    if (LOW_QUALITY_DOMAIN_RE.test(host) || LOW_QUALITY_DOMAIN_RE.test(title))
        score -= 120;
    if (/素材|模板|图片|壁纸|免抠|海报|设计|下载/.test(haystack))
        score -= 60;
    return score;
}
