...
function normalizeConfig(raw = {}) {
    const source = isConfigRecord(raw) ? raw : {};
    ...
    const dangerousPolicy = ['auto', 'confirm', 'block'].includes(String(source.dangerousPolicy || '')) ? String(source.dangerousPolicy || '') : defaults.dangerousPolicy;
...
function readConfigFile() {
    try {
        ...
        if (!stat.isFile() || stat.size > MAX_TOOL_CONFIG_BYTES)
            return null;
