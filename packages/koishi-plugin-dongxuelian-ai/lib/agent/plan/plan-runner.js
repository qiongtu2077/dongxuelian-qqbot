...
function isRunnerPlan(value) {
    return !!value && typeof value === 'object' && typeof value.id === 'string';
}
function isRunnerPlanStatus(value) {
    return !!value && typeof value === 'object' && Array.isArray(value.active);
}
async function resolvePlan(planId = '', filters = {}) {
    ...
    const matched = active.find((plan) => {
        if (filters.userId && plan.userId !== filters.userId)
            return false;
        if (filters.channelKey && plan.channelKey !== filters.channelKey)
            return false;
        return true;
    });
    return matched || active[0] || null;
}
async function resumePlan({ planId = '', channelKey, userId, userName = '', channel = '', bot, isAdmin = false } = {}) {
