...
function asPlanResult(plan) {
    return plan;
}
function isPlanStatusList(plan) {
    return Array.isArray(plan.active);
}
...
async function updateTaskStatus({ planId, taskId, state, outcome = null, toolCallCount } = {}) {
    ...
    const nextState = ['todo', 'in_progress', 'done', 'abandoned', 'failed'].includes(String(state)) ? state : '';
    if (!nextState)
    ...
    if (toolCallCount !== undefined)
        task.toolCallCount = Math.max(0, parseInt(String(toolCallCount), 10) || 0);
...
async function checkPlanStatus(planId, { userId, channelKey, isAdmin } = {}) {
    ...
    if (!userId && !channelKey)
        return { active: active.map(asPlanResult), recent: recent.map(asPlanResult) };
