...
function assertNotWriteBlockedBasename(target, label = '路径') {
    ...
    if (WRITE_BLOCKED_BASENAMES.has(basename))
        throw new Error(`${label}禁止写入安全配置文件：${basename}`);
}
...
function pushUniqueRoot(result, root) {
    ...
    if (!result.some(item => normalizeAgentPathCase(item) === key))
        result.push(value);
}
...
function mergeConfiguredAndDefaultRoots(roots = []) {
    ...
    for (const root of roots)
        pushUniqueRoot(result, root);
    for (const root of getAgentPathDefaultRoots())
        pushUniqueRoot(result, root);
    return result;
}
async function realpathOrResolvedAgentPath(target) {
    ...
    catch { /* non-critical: non-existing root uses resolved path for later existence checks */
        return resolved;
    }
}
...
async function assertExistingAgentPathInsideRoots(target, label = '路径') {
    ...
    if (!roots.some(root => isAgentPathInside(real, root)))
        throw new Error(`${label}超出允许范围：${abs}`);
...
async function assertNewAgentPathInsideRoots(target, label = '路径', createDirectories = false) {
    ...
    if (!realParent)
        throw new Error(`父目录不存在：${path.dirname(abs)}`);
    if (!roots.some(root => isAgentPathInside(realParent, root)))
        throw new Error(`${label}超出允许范围：${abs}`);
