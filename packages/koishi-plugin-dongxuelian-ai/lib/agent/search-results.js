...
function normalizeResultUrl(url = '') {
    ...
    try {
        ...
        if (parsed.hostname.endsWith('baidu.com') && parsed.pathname.startsWith('/link'))
            return raw;
        parsed.hash = '';
        for (const key of [...parsed.searchParams.keys()]) {
            if (/^(utm_.*|spm|from|fr|tn|ie|wd|oq|rsv_.*|sa|ved|usg|form)$/i.test(key))
...
function getResultKey(item = {}) {
    ...
    if (url)
        return url.replace(/[?&](?:rut|sid|share_token)=[^&]+/gi, '');
...
function hasQuerySignal(item = {}, query = '') {
    ...
    if (terms.some(term => haystack.includes(term.toLowerCase())))
        return true;
    if (hasTrustedSearchSignal(item, query))
        return true;
    const host = getSearchHostname(item.url);
    if (TRUSTED_DOMAIN_FOR_PASS_RE.test(host) && getResultDomainSignal(item))
        return true;
    return false;
}
function getResultDomainSignal(item = {}) {
    try {
        ...
        if (/\.(gov|edu)$/i.test(host))
            return true;
        if (/(?:official|news|support|developer|docs|blog|help|changelog|release|minecraft|mojang|kurogames|wutheringwaves|bilibili|weibo|taptap|gamekee)/i.test(host))
            return true;
    }
...
function isUsefulSearchResult(item = {}, query = '') {
    ...
    if (!normalized.title || !normalized.url)
        return false;
    if (SEARCH_NAV_TITLE_RE.test(normalized.title))
        return false;
    if (SEARCH_NOISE_RE.test(normalized.title) || SEARCH_NOISE_RE.test(normalized.snippet))
        return false;
    if (SEARCH_INTERNAL_URL_RE.test(normalized.url))
        return false;
    if (!/^https?:\/\//i.test(normalized.url))
        return false;
    if (isLowQualitySearchResult(normalized))
        return false;
    if (DICTIONARY_RESULT_RE.test(normalized.title))
        return false;
    if (DICTIONARY_RESULT_RE.test(normalized.url))
        return false;
...
function rankSearchCandidates(candidates = [], query = '', limit = 8) {
    ...
    for (const raw of Array.isArray(candidates) ? candidates : []) {
        ...
        if (!isUsefulSearchResult(item, query))
            continue;
...
function classifySearchResult(ranked = [], pages = []) {
    ...
    if (hasPageContent && hasHighScore)
        return 'usable_hit';
    if (hasVeryHighScore && pages.some(p => p.text && p.text.length >= 40))
        return 'usable_hit';
    if (hasTrustedDirect && pages.some(p => p.text && p.text.length >= 20))
        return 'usable_hit';
    if (ranked.length > 0)
        return 'weak_hit';
    return 'hard_fail';
...
function detectFailurePattern(ranked = [], pages = [], allCandidates = []) {
    if (!ranked.length && !allCandidates.length)
        return 'no_results';
    ...
    for (const item of items) {
        ...
        if (DICTIONARY_RESULT_RE.test(text))
            dictCount++;
        if (HOMEPAGE_RESULT_RE.test(text))
            homeCount++;
    }
    if (dictCount >= total * 0.4)
        return 'dictionary_ambiguity';
    if (homeCount >= total * 0.3)
        return 'homepage_only';
    if (ranked.length && pages.every(p => !p.text || p.text.length < 80))
...
function buildStrategyQueries(failurePattern, originalQuery, usedQueries) {
    ...
    const push = (q) => {
        ...
        if (trimmed && !usedQueries.has(trimmed.toLowerCase()))
            result.push(trimmed);
    };
