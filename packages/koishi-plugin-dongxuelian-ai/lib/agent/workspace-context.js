...
function pushUniqueCandidate(out, item) {
    ...
    if (out.some(candidate => candidate.key === key))
        return;
...
function resolveAgentPathInput(value = '', roots = [], options = {}) {
    ...
    if (requested && path.isAbsolute(requested)) {
        ...
        for (const item of getAliasPathCandidates(requested, roots))
            pushUniqueCandidate(candidates, item);
        if (requested) {
            for (const root of roots) {
        ...
        for (const root of roots) {
            for (const aliasPath of alias.paths) {
                ...
                if (pathExists(candidate))
                    pushUniqueCandidate(result, { path: candidate, reason: alias.name });
            }
        }
    }
