...
async function downloadSkill({ source, url, owner, repo, skillPath, branch }) {
    ...
    if (!type)
        return { ok: false, error: 'Cannot detect source type from URL' };
    const adapter = ADAPTERS[type];
    if (!adapter)
        return { ok: false, error: `No adapter for source type: ${type}` };
    await cleanTempDir();
