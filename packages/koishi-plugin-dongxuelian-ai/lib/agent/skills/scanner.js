...
function getMaxSeverity(findings) {
    ...
    for (const f of findings) {
        if ((SEVERITY_ORDER[f.severity] || 0) > (SEVERITY_ORDER[max] || 0))
            max = f.severity;
    }
    return max;
}
...
function readWhitelistSync() {
    try {
        if (!fs.existsSync(SKILL_SCANNER_WHITELIST_FILE))
            return {};
...
function isWhitelistedAndUnchanged(skillDir) {
    ...
    if (!entry || typeof entry !== 'object' || typeof entry.sha256 !== 'string')
        return false;
    let current;
    try {
