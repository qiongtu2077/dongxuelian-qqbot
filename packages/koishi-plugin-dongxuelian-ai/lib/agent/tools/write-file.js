...
module.exports = {
    ...
    async execute(params = {}) {
        ...
        if (contentBytes > MAX_CONTENT_BYTES)
            throw new Error(`内容过大：${contentBytes} bytes，最大 ${MAX_CONTENT_BYTES} bytes`);
        ...
        if (linkStat && linkStat.isSymbolicLink())
            throw new Error(`目标是符号链接，拒绝写入：${filePath}`);
        let existing = null;
        try {
            existing = await fs.stat(abs);
        }
        catch {
            /* non-critical: absence means this is a new file */
        }
        if (existing) {
        ...
        if (existing && existing.isDirectory())
            throw new Error(`目标是目录：${filePath}`);
        if (existing && existing.size > MAX_OVERWRITE_TARGET_BYTES)
            throw new Error(`目标文件过大，拒绝覆盖：${existing.size} bytes`);
        if (existing && !params.overwrite)
            throw new Error('文件已存在，如需覆盖请设置 overwrite=true');
