    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            if (!SKIP_DIRS.has(entry.name))
                await collectFiles(full, matcher, out, maxFiles);
        }
...
module.exports = {
    ...
    async execute(params = {}) {
        ...
        const limit = Math.max(1, Math.min(MAX_MATCHES, parseInt(String(params.limit), 10) || 50));
        let regex;
        ...
        try {
            ...
            /* non-critical: invalid or unsafe regex falls back to literal search */
            regex = new RegExp(escaped, params.ignoreCase === false ? 'u' : 'iu');
        }
        ...
        const files = [];
        if (stat.isFile())
            files.push(checked.abs);
        else if (stat.isDirectory())
            await collectFiles(checked.abs, grepWildcardToRegExp(params.glob || '*'), files);
        else
            throw new Error(`不支持的搜索路径：${target}`);
        const matches = [];
        for (const file of files) {
            if (!isAgentPathInside(file, checked.real) && stat.isDirectory())
                continue;
            const st = await fs.stat(file).catch(() => null);
            if (!st || !st.isFile() || st.size > MAX_FILE_BYTES)
                continue;
            ...
            for (let i = 0; i < lines.length; i++) {
                if (regex.test(lines[i])) {
                    ...
                    if (joined.length > MAX_TOTAL_OUTPUT_CHARS)
                        return `匹配 ${matches.length} 条（输出已截断）：\n${joined.slice(0, MAX_TOTAL_OUTPUT_CHARS)}`;
                    if (matches.length >= limit)
                        return `匹配 ${matches.length} 条（已达上限）：\n${matches.join('\n')}`;
                }
                regex.lastIndex = 0;
            }
        }
