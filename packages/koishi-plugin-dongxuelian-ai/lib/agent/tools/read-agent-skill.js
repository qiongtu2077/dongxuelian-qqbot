...
function isSkillEnabled(name, context = {}) {
    if (context.channel === 'dashboard' && context.autoRelevantSkill !== false)
        return true;
    const target = String(name || '').trim().toLowerCase();
    if (!target)
        return false;
    if (getEnabledSkills().some(item => String(item || '').trim().toLowerCase() === target))
        return true;
...
module.exports = {
    ...
    async execute(params = {}, context = {}) {
        if (!isSkillEnabled(params.name, context))
            throw new Error(`Agent Skill 未启用：${String(params.name || '').trim()}`);
        ...
        if (result.references.length)
            lines.push(`可读取参考文件：${result.references.join(', ')}`);
