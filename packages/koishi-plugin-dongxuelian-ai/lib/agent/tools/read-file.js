...
module.exports = {
    ...
    async execute(params = {}) {
        ...
        if (buffer.includes(0))
            throw new Error('疑似二进制文件，拒绝读取');
        const content = buffer.toString('utf8');
        const lines = content.split('\n');
        const start = Math.max(0, (parseInt(String(params.offset), 10) || 1) - 1);
        const end = Math.min(lines.length, start + Math.min(500, parseInt(String(params.limit), 10) || 200));
