...
module.exports = {
    ...
    async execute(params = {}) {
        ...
        if (contentBytes > MAX_APPEND_BYTES)
            throw new Error(`内容过大：${contentBytes} bytes，最大 ${MAX_APPEND_BYTES} bytes`);
        ...
        if (linkStat && linkStat.isSymbolicLink())
            throw new Error(`目标是符号链接，拒绝追加：${filePath}`);
        ...
        if (existing && existing.size + contentBytes > MAX_APPEND_TARGET_BYTES)
            throw new Error(`追加后文件过大：${existing.size + contentBytes} bytes，最大 ${MAX_APPEND_TARGET_BYTES} bytes`);
