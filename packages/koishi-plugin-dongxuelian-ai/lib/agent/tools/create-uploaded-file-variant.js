...
function resolveTargetFileName(params = {}, entry = {}) {
    ...
    if (!path.extname(nextName) && originalExt)
        nextName += `.${originalExt}`;
    return nextName;
}
...
async function pickRecentFile(channelKey, params = {}) {
    ...
    const matched = candidates.find(item => String(item.fileName || '').toLowerCase().includes(keyword) ||
...
function hasTrustedCrossUserFileEvidence(context = {}, picked = {}) {
    ...
    if (evidenceMessageId && evidenceMessageId === normalizeId(picked.messageId))
        return true;
...
function assertFileVariantTargetAllowed(picked, context = {}) {
    if (!picked || context.isDirect)
        return;
    if (picked.reason === 'messageId' || picked.reason === 'keyword')
        return;
    if (hasTrustedCrossUserFileEvidence(context, picked))
        return;
...
function applyTextReplacement(buffer, params = {}, ext = '') {
    ...
    if (!replacement || !TEXT_EXTENSIONS.has(ext))
        return buffer;
...
async function createVariant(params = {}, context = {}) {
    ...
    if (stat.size > MAX_VARIANT_FILE_BYTES)
        throw new Error(`文件过大，拒绝创建副本：${stat.size} bytes`);
    ...
    if (!isPathInside(destPath, OUTPUT_DIR))
