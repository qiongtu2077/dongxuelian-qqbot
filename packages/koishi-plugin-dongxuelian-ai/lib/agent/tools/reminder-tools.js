"use strict";
const { registerOnceTask, loadCrons, unregisterCron } = require('../cron');
...
function normalizeReminderPrompt(params = {}) {
...
async function executeCreateReminder(params = {}, context = {}) {
    ...
    if (delay > MAX_DELAY_MS)
        return '提醒时间太远，最多支持 30 天内的一次性提醒。';
...
function isReminderVisibleToContext(cron = { id: '' }, context = {}) {
    const targetChannel = resolveReminderTarget(context);
    const userId = String(context.userId || '').trim();
    if (targetChannel && String(cron.targetChannel || '') !== targetChannel)
        return false;
...
async function executeCancelReminder(params = {}, context = {}) {
    ...
    if (!hasExplicitTarget)
        return '请说明要取消哪一条提醒，例如提供提醒 id、关键词，或说“取消最近一条提醒”。';
    let target = requestedId ? visible.find(cron => cron.id === requestedId) : null;
    if (!target && keyword)
        target = visible.find(cron => String(cron.prompt || '').includes(keyword));
    if (!target && params.latest === true && visible.length)
        target = visible[0];
    if (!target && visible.length > 1)
        return '有多条待触发提醒，请说明要取消哪一条，或说“取消最近一条提醒”。';
    if (!target)
        return '没找到可取消的提醒。';
