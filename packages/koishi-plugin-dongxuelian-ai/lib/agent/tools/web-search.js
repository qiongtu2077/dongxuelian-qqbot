...
function hasSearchSourceSignal(text = '') {
    ...
    if (/https?:\/\/[^\s)）]+/i.test(value))
        return true;
    if (/(?:来源|参考|出处|链接)[:：\s]|(?:kurogames|wutheringwaves|bilibili|weibo|TapTap|GameKee)|(?:库洛|鸣潮官网)|官方(?:公告|新闻|资讯|微博|B站|bilibili)/i.test(value))
        return true;
    return false;
}
function apiSearchLooksUnreliable(text = '') {
    ...
    if (value.length < 30)
        return true;
    if (!hasSearchSourceSignal(value))
        return true;
    if (/未搜索到|没有找到|无法确认|无可靠结果|不能确定|搜索失败|素材|模板|图片下载|免费下载|图库|设计素材/.test(value))
        return true;
    const urls = value.match(/https?:\/\/[^\s)）]+/gi) || [];
    if (urls.length > 0 && urls.every(url => isLowQualitySearchResult({ url, title: getSearchHostname(url) })))
        return true;
    return false;
}
...
function extractUrlsFromSearchText(text = '') {
    ...
    for (const raw of matches) {
        ...
        seen.add(url);
        urls.push(url);
        if (urls.length >= API_SEARCH_URL_LIMIT)
            break;
    }
    return urls;
}
...
async function readApiSearchCandidatePages(candidates = []) {
    ...
    for (const item of candidates.slice(0, API_SEARCH_URL_LIMIT)) {
        if (pages.length >= API_SEARCH_FETCH_PAGE_LIMIT)
            break;
...
async function verifyApiSearchWithFetch(apiText = '', queries = []) {
    ...
    const candidates = rankSearchCandidates(mergeHttpSearchCandidates(...queries.map(item => buildApiSearchCandidates(apiText, item))), query, API_SEARCH_URL_LIMIT);
...
function getWebSearchErrorMessage(error) {
    ...
    try {
        for (const query of queries.slice(0, BROWSER_SEARCH_QUERY_LIMIT)) {
            try {
                ...
                results.push(result);
                if (!/未提取到有效搜索结果|搜索结果质量较低|素材|模板/.test(String(result || '')))
                    break;
            }
            catch (error) {
    ...
    async execute(params = {}) {
        ...
        try {
            ...
            if (result && typeof result === 'string' && !apiSearchLooksUnreliable(result)) {
                ...
                if (verified.ok)
                    return `API 搜索返回了候选来源，已用 web_fetch 验证正文。\n${verified.text}`;
