...
const { registerCron, registerOnceTask, loadCrons, getCron, pauseCron, resumeCron, unregisterCron, runCronNow, createCronId, } = require('../cron');
...
function normalizeTaskType(params = {}) {
...
function normalizeTaskKind(params = {}, type = 'text', mode = 'once') {
    if (['reminder', 'scheduled_text', 'scheduled_agent', 'file_agent'].includes(String(params.taskKind)))
        return String(params.taskKind);
    if (mode === 'once' && type === 'text')
        return 'reminder';
    return type === 'text' ? 'scheduled_text' : 'scheduled_agent';
}
function normalizeContextPolicy(params = {}) {
...
async function executeCreateScheduledTask(params = {}, context = {}) {
    if (context.randomTriggered)
        return '随机主动回复不能创建定时任务。';
    ...
    if (mode === 'once') {
        ...
        if (!Number.isFinite(runAt) || delay < MIN_DELAY_MS)
            return '定时任务时间太近或无效。';
...
async function executeListScheduledTasks(params = {}, context = {}) {
    ...
    const items = (data.crons || [])
        ...
        if (status === 'all')
            return true;
        if (status === 'active')
            return cron.enabled !== false && !['done', 'cancelled', 'paused'].includes(cron.status);
...
async function executeGetScheduledTask(params = {}, context = {}) {
    const id = String(params.id || params.taskId || '').trim();
    if (!id)
        return '需要提供定时任务 id。';
    const cron = await getCron(id);
    if (!cron || !isTaskVisibleToContext(cron, context))
        return '没找到可查看的定时任务。';
...
async function executePauseScheduledTask(params = {}, context = {}) {
    const id = String(params.id || params.taskId || '').trim();
    if (!id)
        return '需要提供定时任务 id。';
    const cron = await getCron(id);
...
async function executeResumeScheduledTask(params = {}, context = {}) {
    const id = String(params.id || params.taskId || '').trim();
    if (!id)
        return '需要提供定时任务 id。';
    const cron = await getCron(id);
...
async function executeDeleteScheduledTask(params = {}, context = {}) {
    const id = String(params.id || params.taskId || '').trim();
    if (!id)
        return '需要提供定时任务 id。';
    const cron = await getCron(id);
...
async function executeRunScheduledTaskNow(params = {}, context = {}) {
    const id = String(params.id || params.taskId || '').trim();
    if (!id)
        return '需要提供定时任务 id。';
    const cron = await getCron(id);
    ...
    return {
        definition: {
            ...
            parameters: {
                ...
                required: name === 'list_scheduled_tasks' ? [] : ['id'],
            },
        },
        execute,
