...
function normalizeTaskKind(params = {}, type = 'text', mode = 'once') {
  if (['reminder', 'scheduled_text', 'scheduled_agent', 'file_agent'].includes(params.taskKind)) return params.taskKind
...
function isTaskVisibleToContext(cron = {}, context = {}) {
...
async function executeListScheduledTasks(params = {}, context = {}) {
  const limit = Math.max(1, Math.min(parseInt(params.limit, 10) || 10, 30))
