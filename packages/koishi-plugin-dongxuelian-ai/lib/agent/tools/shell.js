...
module.exports = {
    ...
    async execute(params = {}) {
        ...
        return new Promise(resolve => {
            const child = execFile(isWin ? 'cmd.exe' : '/bin/sh', isWin ? ['/d', '/s', '/c', command] : ['-c', command], { cwd, timeout: 25000, maxBuffer: 500 * 1024, windowsHide: true, env }, (err, stdout, stderr) => {
                const parts = [];
                if (stdout)
                ...
                if (err && err.killed)
                    parts.push('(命令执行超时，已取消)');
                else if (err)
                    parts.push(`[exit code: ${err.code || -1}]`);
                if (parts.length === 0)
                    parts.push('(执行成功，无输出)');
                resolve(parts.join('\n'));
            });
