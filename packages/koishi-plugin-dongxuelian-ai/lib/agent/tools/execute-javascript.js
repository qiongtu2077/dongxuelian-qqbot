...
function safeStringifyResult(result) {
    ...
    return JSON.stringify(result === undefined ? null : result, (_key, value) => {
        if (typeof value === 'string' && value.length > MAX_RESULT_CHARS)
            return value.slice(0, MAX_RESULT_CHARS) + `...(字符串截断，共 ${value.length} 字符)`;
...
module.exports = {
    ...
    async execute(params = {}) {
        ...
        if (code.length > MAX_CODE_CHARS)
            throw new Error(`代码过长：${code.length} 字符，最大 ${MAX_CODE_CHARS}`);
        if (BLOCKED.test(code))
            throw new Error('代码包含被禁止的 Node/进程/模块能力');
    ...
    `, context);
        ...
        if (text && text.length > MAX_JSONIFY_CHARS)
