...
async function collectLogFiles() {
    ...
    try {
        ...
        if (st.isFile() && !files.includes(FALLBACK_LOG_FILE))
            files.push(FALLBACK_LOG_FILE);
    }
    catch {
...
function parseSince(value = '') {
    ...
    if (hours)
        return Date.now() - Number(hours[1]) * 3600000;
    ...
    async execute(params = {}) {
        ...
        if (tailMode) {
            for (const file of files) {
                ...
                if (!stat || stat.size > MAX_FILE_BYTES || (since && stat.mtimeMs < since))
                    continue;
        ...
        for (const file of files) {
            ...
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];
                if (level && !line.toLowerCase().includes(level))
                    continue;
                if (regex.test(line)) {
                    matches.push(`${path.basename(file)}:${i + 1}: ${redact(line).slice(0, 260)}`);
                    if (matches.length >= limit)
                        return `找到 ${matches.length} 条日志（已达上限）：\n${matches.join('\n')}`;
                }
                regex.lastIndex = 0;
            }
        }
