...
module.exports = {
    ...
    async execute(params = {}) {
        ...
        if (typeof params.newString !== 'string')
            throw new Error('newString 必须是字符串');
        if (params.oldString === params.newString)
            throw new Error('newString 必须不同于 oldString');
        const replacementBytes = Buffer.byteLength(params.newString, 'utf8');
        if (replacementBytes > MAX_REPLACEMENT_BYTES)
            throw new Error(`替换内容过大：${replacementBytes} bytes，最大 ${MAX_REPLACEMENT_BYTES} bytes`);
        ...
        if (stat.size > MAX_FILE_BYTES)
            throw new Error(`文件过大：${stat.size} bytes，最大 ${MAX_FILE_BYTES} bytes`);
        const buffer = await fs.readFile(abs);
        if (buffer.includes(0))
            throw new Error('疑似二进制文件，拒绝编辑');
        const content = buffer.toString('utf8');
        const occurrences = countOccurrences(content, params.oldString);
        if (occurrences === 0)
            throw new Error('未找到 oldString');
        if (occurrences > 1 && !params.replaceAll)
            throw new Error('oldString 匹配多处，请设置 replaceAll=true 或提供更长上下文');
        ...
        if (nextBytes > MAX_EDIT_RESULT_BYTES)
            throw new Error(`编辑后文件过大：${nextBytes} bytes，最大 ${MAX_EDIT_RESULT_BYTES} bytes`);
