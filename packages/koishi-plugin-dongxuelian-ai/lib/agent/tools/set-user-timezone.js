...
async function readMap() {
    try {
        ...
        if (!stat.isFile() || stat.size > MAX_TZ_FILE_BYTES)
            return {};
        const data = JSON.parse(await fs.readFile(TZ_FILE, 'utf8'));
        const result = {};
...
module.exports = {
    ...
    async execute(params = {}) {
        ...
        if (!TZ_RE.test(timezone))
            throw new Error('时区格式无效，请使用 IANA 时区名，例如 Asia/Shanghai');
        try {
            new Date().toLocaleString('zh-CN', { timeZone: timezone });
        }
        catch {
