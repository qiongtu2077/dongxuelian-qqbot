...
const { SKILLS_CORE_DIR, SKILLS_MODES_DIR, SKILLS_PERSONAS_DIR, } = require('../core/constants');
const { resolvePersona, loadPersonalSkill, parsePersonaFrontmatter, } = require('../persona/persona');
...
function loadAgentPromptFile(file) {
    try {
        ...
        if (!stat.isFile() || stat.size > MAX_AGENT_PERSONA_FILE_BYTES)
            return '';
...
function buildAgentPersonaContext(options = {}) {
    ...
    const messages = [];
    if (agentMode) {
        messages.push({ role: 'system', content: '【Agent 直连模式】当前处于 Agent 工具调用模式，不使用角色人格。' }, { role: 'system', content: AGENT_DIRECT_MODE_PROMPT });
        return messages;
    }
    messages.push({ role: 'system', content: buildAgentPersonaSystemMessage({ personaName, personaContent, source, channel, plan: personaPlan }) }, { role: 'system', content: AGENT_GUARD_PROMPT });
...
function mergeAgentSystemExtra(...groups) {
    ...
    for (const group of groups) {
        ...
        for (const item of group) {
            if (item && typeof item === 'object' && item.role === 'system' && typeof item.content === 'string' && item.content.trim()) {
                result.push({ role: 'system', content: item.content });
            }
        }
    }
    return result;
