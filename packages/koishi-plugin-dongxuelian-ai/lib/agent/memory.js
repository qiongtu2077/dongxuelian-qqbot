...
function withUserLock(userId, fn) {
    ...
    next.finally(() => { if (writeLocks.get(key) === next)
...
function getMemoryFileCandidates(userId) {
    ...
    if (legacy !== current)
        files.push(path.join(MEMORY_DIR, legacy + '.json'));
    return files;
}
function getMemoryFile(userId) {
    return getMemoryFileCandidates(userId)[0];
}
function normalizeMemoryFileData(data) {
...
async function readMemoryFile(userId) {
    for (const file of getMemoryFileCandidates(userId)) {
        try {
            ...
            if (!stat.isFile() || stat.size > MAX_MEMORY_FILE_BYTES)
                continue;
...
function scoreMemory(item, queryTokens, channelKey = '') {
    ...
    if (channelKey && item.channelKey === channelKey)
        score += 1;
