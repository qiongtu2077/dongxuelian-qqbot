...
function isContextMessage(value) {
    return !!value;
}
...
function compactMessages(messages = [], maxMessages = 24) {
    ...
    if (users.length)
        summaryParts.push('较早用户目标：' + users.join(' / '));
    if (tools.length)
        summaryParts.push('较早工具结果摘要：' + tools.join(' / '));
...
function getSummaryContent(rawSummary) {
    if (typeof rawSummary === 'string')
        return rawSummary;
    if (rawSummary && typeof rawSummary === 'object' && typeof rawSummary.content === 'string') {
        return rawSummary.content;
    }
    return '';
}
function isToolCallSummary(rawSummary) {
    return !!rawSummary
        && typeof rawSummary === 'object'
        && rawSummary.type === 'tool_calls';
}
async function compactWithLLM(messages = [], config = {}, requestFn) {
    ...
    try {
        ...
        if (!/##\s*目标/.test(summary) || !/##\s*下一步/.test(summary))
            throw new Error('bad summary shape');
...
function estimateCacheHitRate(systemMessage = '', previousSystemMessage = '') {
    ...
    let same = 0;
    while (same < max && current.charCodeAt(same) === previous.charCodeAt(same))
        same++;
