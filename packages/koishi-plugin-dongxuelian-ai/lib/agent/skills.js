...
function skillReadText(file) {
    try {
        ...
        if (!stat.isFile() || stat.size > MAX_SKILL_INDEX_FILE_BYTES)
            return '';
        return fs.readFileSync(file, 'utf8');
    }
    catch { /* non-critical: unreadable skill index file is skipped */
        return '';
    }
}
...
function skillListReferences(rootDir, primaryFile) {
    ...
    function walk(dir, depth) {
        if (references.length >= MAX_REFERENCE_FILES || depth > 4)
            return;
        let entries = [];
        try {
            entries = fs.readdirSync(dir, { withFileTypes: true });
        }
        catch { /* non-critical: unreadable reference directory is skipped */
            return;
        }
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (!SKIP_REFERENCE_DIRS.has(entry.name))
                    walk(full, depth + 1);
                continue;
            }
            if (!entry.isFile())
                continue;
            if (path.resolve(full) === primary)
                continue;
            if (!TEXT_FILE_RE.test(entry.name))
                continue;
            const stat = fs.statSync(full, { throwIfNoEntry: false });
            if (!stat || !stat.isFile() || stat.size > MAX_SKILL_REFERENCE_BYTES)
                continue;
...
function skillCollectEntry(skills, group, entry, baseDir) {
    ...
    if (entry.isDirectory()) {
        ...
        catch { /* non-critical: unreadable nested skill directory is skipped */
            return;
        }
        for (const item of nested) {
            if (item.name === DIRECTORY_SKILL_FILE)
                continue;
            skillCollectEntry(skills, group, item, full);
        }
        return;
    }
    if (!entry.isFile() || !LEGACY_SKILL_RE.test(entry.name))
        return;
...
function listAgentSkills() {
    ...
    for (const group of SKILL_DIRS) {
        ...
        catch { /* non-critical: optional skill group directory may be absent */
            continue;
        }
        for (const entry of entries)
            skillCollectEntry(skills, group, entry, group.dir);
    }
    ...
    try {
        ...
        for (const entry of poolEntries) {
            ...
            const legacyFiles = (() => { try {
                return fs.readdirSync(path.join(SKILL_POOL_DIR, entry.name)).filter((f) => LEGACY_SKILL_RE.test(f));
            }
            catch { /* non-critical: unreadable pool skill directory is skipped */
                return [];
            } })();
            if (fs.existsSync(directorySkill)) {
...
function skillResolveRequestedFile(skill, requestedFile = '') {
    ...
    if (!skill.dir) {
        ...
        if (value === ownName || value === skill.path)
            return skill.file;
        throw new Error('该 Skill 不允许读取额外参考文件');
    }
    if (path.isAbsolute(value) || value.includes('\0'))
        throw new Error('Skill 参考文件必须使用相对路径');
    ...
    if (path.basename(normalized) === DIRECTORY_SKILL_FILE || normalized === skill.path)
        return skill.file;
    ...
    if (!skillPathInside(resolvedTarget, root))
        throw new Error('Skill 参考文件超出技能目录');
    let target = '';
    try {
    ...
    if (!skillPathInside(target, root))
        throw new Error('Skill 参考文件超出技能目录');
    return target;
}
function readAgentSkill(name, options = {}) {
    ...
    if (!skill)
        throw new Error(`未知 Agent Skill：${skillNormalizeName(name)}`);
    const file = skillResolveRequestedFile(skill, options.file);
    const stat = fs.statSync(file);
    if (!stat.isFile())
        throw new Error(`不是 Skill 文件：${skillRelativePath(file)}`);
    if (stat.size > MAX_SKILL_REFERENCE_BYTES)
        throw new Error(`Skill 文件过大：${stat.size} bytes`);
    const buffer = fs.readFileSync(file);
    if (buffer.includes(0))
        throw new Error('Skill 文件疑似二进制，拒绝读取');
...
function findRelevantAgentSkills(query = '', options = {}) {
    ...
    return listAgentSkills()
        ...
        for (const term of terms) {
            if (skillNameKey(skill.name).includes(term))
                score += 8;
            if (String(skill.description || '').toLowerCase().includes(term))
                score += 5;
            if (text.includes(term))
                score += 2;
        }
        if (/前端|dashboard|后台|控制台|源码|代码|文件|目录|agent|skill|技能|浏览器|browser|pdf|ppt|docx/i.test(queryText) && skill.name === 'QA_source_index')
            score += 6;
        if (/浏览器|网页|页面|截图|browser|cdp/i.test(queryText) && /^browser_/i.test(skill.name))
            score += 8;
        if (/联网|搜索|搜|查(一下|找)?|最新|新闻|资讯|官网|官方|来源|网页|资料|web[_-]?search|search/i.test(queryText) && skill.name === 'web_search_strategy')
            score += 12;
        if (/ppt|pptx|演示|幻灯片/i.test(queryText) && /^pptx$/i.test(skill.name))
            score += 8;
        if (/pdf/i.test(queryText) && /^pdf$/i.test(skill.name))
            score += 8;
        if (/docx|word|文档/i.test(queryText) && /^docx$/i.test(skill.name))
            score += 8;
        if (/读文件|读日志|看日志|查日志|查看文件|查看日志|log|文件内容|tail/i.test(queryText) && skill.name === 'file_reader')
            score += 10;
        if (/计划|规划|分步骤|怎么做|步骤|方案|遇阻|放弃|换方法/i.test(queryText) && skill.name === 'make_plan')
            score += 10;
        return { skill, score };
    })
...
function buildAgentSkillSummary(enabledNames = [], options = {}) {
    ...
    for (const skill of listAgentSkills()) {
        if (enabled.has(skillNameKey(skill.name)))
