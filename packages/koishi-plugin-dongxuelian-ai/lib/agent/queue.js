...
function startTask(task) {
    ...
    withTimeout(task.fn, task.timeoutMs)
        ...
        .catch((error) => {
        const queueError = error instanceof Error ? error : null;
        if (queueError && queueError.code === 'AGENT_QUEUE_TIMEOUT')
            timeoutCount++;
        ...
        if (queue && queue[0] === task)
            queue.shift();
        if (queue && queue.length === 0)
            taskQueues.delete(task.key);
        activeKeys.delete(task.key);
        drainAgentQueues();
    });
}
function drainAgentQueues() {
    if (activeCount >= options.maxGlobal)
        return;
    ...
    for (const queue of queues) {
        if (activeCount >= options.maxGlobal)
            return;
        const task = queue[0];
        if (getChannelDepth(task.channelKey) >= options.maxPerChannel)
            continue;
        startTask(task);
    }
}
function enqueueAgentTask({ channelKey = 'unknown', userId = 'unknown', fn, timeoutMs, maxDepth } = {}) {
    if (typeof fn !== 'function')
        return Promise.reject(new Error('Agent 任务缺少 fn'));
...
function getAgentQueueStats() {
    ...
    for (const queue of taskQueues.values()) {
        for (const task of queue)
            byChannel[task.channelKey] = (byChannel[task.channelKey] || 0) + 1;
    }
    return {
        options: { ...options },
        activeCount,
