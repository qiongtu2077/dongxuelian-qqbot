...
const { truncateText: coreTruncateText } = require('../core/utils');
...
function cleanupStaleQuotaCache(day) {
    for (const [k] of quotaCache) {
        if (!k.startsWith(day + ':'))
            quotaCache.delete(k);
    }
}
function ignorePushQueueFailure(error) {
    void error;
}
function enqueueQuotaOperation(key, fn) {
    const previous = quotaOperationChains.get(key) || Promise.resolve();
    const run = previous.catch(ignorePushQueueFailure).then(fn);
    quotaOperationChains.set(key, run);
    run.finally(() => {
        if (quotaOperationChains.get(key) === run)
            quotaOperationChains.delete(key);
    }).catch(ignorePushQueueFailure);
    return run;
}
function parsePushLogLine(line) {
    try {
        return JSON.parse(line);
    }
    catch { /* non-critical: corrupt push log line is ignored during quota scan */
        return null;
    }
}
async function countLoggedQuota(channelKey, now = Date.now()) {
    ...
    try {
        ...
        for (const line of lines) {
            ...
            if (!entry || !entry.ok || entry.bypassEnabled || String(entry.channelKey || '') !== target)
                continue;
            if (todayKey(Number(entry.at || 0)) === day)
                count++;
        }
        return count;
    }
...
async function getQuota(channelKey, now = Date.now()) {
    ...
    if (!quotaCache.has(key))
        quotaCache.set(key, await countLoggedQuota(channelKey, now));
...
async function sendBotMessage(bot, target, content) {
    ...
    if (typeof bot.sendMessage === 'function')
        return bot.sendMessage(targetText, content);
...
function listPushLog(limit = 50) {
    try {
        ...
        return lines.slice(-Math.max(1, Math.min(200, parseInt(String(limit), 10) || 50))).reverse().map(line => {
            ...
            catch { /* non-critical: expose corrupt log line as raw text for diagnostics */
                return { raw: line };
            }
        });
    }
