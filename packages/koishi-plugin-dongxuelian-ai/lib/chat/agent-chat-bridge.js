...
const { sanitizeUserName, normalizeText } = require('../core/utils');
...
function trimRecentAgentContextCache(now = Date.now()) {
    ...
    if (recentAgentContextCache.size <= MAX_RECENT_AGENT_CONTEXT_ENTRIES)
        return;
...
function extractSearchSummary(text = '') {
    ...
    for (const line of lines) {
        ...
        if (kept.length > 0 && kept.length < 12 && line.length <= 220)
            kept.push(line);
        if (kept.join('\n').length >= MAX_TOOL_SUMMARY_CHARS)
            break;
    }
...
function recordAgentChatResult({ session, userMessage = '', userName = '用户', userId = '', channelKey = '', agentResult = {} } = {}) {
    ...
    if (!reply || /^\(Agent 未获取到有效回复\)/.test(reply))
        return null;
    if (agentResult && agentResult.pendingId)
        return null;
    ...
    if (session && cleanUserMessage) {
        saveConversationTurn(session, `<user>\n昵称：${safeUserName}\n发言：${cleanUserMessage}\n</user>`, reply);
    }
    const now = Date.now();
    const entry = {
        ts: now,
...
function getRecentAgentContextNote({ channelKey = '', userId = '', userMessage = '' } = {}) {
    ...
    if (!isAgentFollowUp(userMessage))
        return '';
    const entry = recentAgentContextCache.get(buildAgentContextKey(channelKey, userId));
    if (!entry)
        return '';
    if (Date.now() - Number(entry.ts || 0) > RECENT_AGENT_CONTEXT_TTL_MS)
        return '';
    const lines = [
        '[最近 Agent 工具上下文-内部参考]',
    ...
    if (entry.toolCalls)
        lines.push(`工具调用数：${entry.toolCalls}`);
