...
function getArgString(args, key) {
...
function isAnalysisTimeoutTool(name) {
...
function buildChatToolFailureContent(name, error) {
...
function guardDangerousChatTool(name = '', args = {}, context = {}, channel = DEFAULT_CHAT_TOOL_CHANNEL) {
    ...
    if (safeResult.action === 'confirm') {
        ...
        if (!channelKey || !userId)
            return `工具 ${name} 需要确认，但当前缺少用户或频道信息，未执行。`;
...
async function executeChatTool(toolCall, context = {}) {
    ...
    if (!isChatToolAllowed(channel, name))
        return `工具 ${name || 'unknown'} 当前渠道未启用。`;
    let args = {};
    try {
    ...
    switch (name) {
        ...
        case 'analyze_historical_image': {
            ...
            if (!ck || !msgId)
                return '需要提供 messageId（从 read_image_history 获取）。';
            ...
            if (!shouldQueue)
                return `当前资源状态为 ${admission.resourceState}，暂时不能加入图片分析队列，原因：${reason}。请稍后再试。`;
...
async function handleChatToolCalls(toolCalls, context = {}) {
    ...
    for (const tc of toolCalls) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('tool timeout')), timeout));
...
function getChatToolSystemHint(channelKey, options = {}) {
    ...
    if (can('web_search') || can('web_fetch')) {
        if (can('web_search'))
            hintParts.push('遇到会随时间变化的问题，例如最新、最近、当前、现在、热门、比较火、趋势、排行、推荐、版本更新、新角色、新闻、视频等，不要凭记忆编答案，应先调用 web_search。');
        if (can('web_fetch'))
            hintParts.push('用户给出具体公开 URL 并要求查看、总结、核对网页内容时，应调用 web_fetch 读取正文。');
        if (can('web_search') && can('web_fetch'))
            hintParts.push('web_search 负责找候选来源并尽量打开正文，web_fetch 负责读取指定 URL；如果没有“正文质量：usable”的可靠正文，要直接说明没有拿到可靠结果，并给出可继续搜索或换链接的方向。');
    }
    if (can('read_group_context'))
        hintParts.push('read_group_context 只能查当前群公开旧片段，适合当前短句或追问接不上时理解“刚才/之前/那个/这张图/那个文件”等指代；工具结果是旧背景，不代表当前话题，不能主动翻旧账。');
    if (can('read_image_history'))
        hintParts.push('read_image_history 返回的图片分析结果只能作为聊天背景知识，绝对不能主动提起图片内容，只有用户明确问到图片时才可以引用。');
    if (can('create_reminder'))
        hintParts.push('用户明确要求“几分钟后提醒我/明天提醒/到点叫我”时必须调用 create_reminder。');
    if (can('create_scheduled_task'))
        hintParts.push('用户要求周期性执行、每天/每周/每隔一段时间说话、总结、分析、运行 agent 时，调用 create_scheduled_task。');
    if (can('list_scheduled_tasks') || can('pause_scheduled_task') || can('resume_scheduled_task') || can('delete_scheduled_task') || can('run_scheduled_task_now'))
        hintParts.push('用户要求查看/暂停/恢复/取消/删除/试跑定时任务时调用对应 scheduled task 工具。');
    if (can('create_reminder') || can('create_scheduled_task'))
        hintParts.push('只有工具结果表示创建成功后，才能说提醒或定时任务已设置。');
    if (can('create_uploaded_file_variant'))
        hintParts.push('用户明确要求把近期上传文件改名、另存副本、发回时可以调用 create_uploaded_file_variant。');
    ...
    if (channelKey) {
        try {
            ...
            if (files.length > 0) {
                ...
                const fileHints = [];
                if (can('analyze_file'))
                    fileHints.push('如果用户明确问"读文件"、"文件里面说了什么"、"刚才那个文件"等，可用 analyze_file');
                if (can('create_uploaded_file_variant'))
                    fileHints.push('如果用户明确要求"把刚才文件改名/重命名/另存为/发给我"，可用 create_uploaded_file_variant');
                if (fileHints.length)
    ...
    if (channelKey) {
        try {
            const { getRecentImagesCached } = require('../media/image/image-store');
            ...
            if (recent.length > 0) {
                ...
                if (can('read_image_history')) {
                    ...
                    if (shortFollowUp)
                        hint += ' 当前消息很短，可能是在接最近图片；先判断 active scene 锚点，再决定读图、澄清或不接。';
                }
            }
        }
        catch {
