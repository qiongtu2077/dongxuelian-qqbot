 ...
 */
const SEARCH_FAILURE_RE = /(?:搜索失败|weak_hit|弱命中|未提取到有效搜索结果|没有拿到可靠结果|没有返回可靠来源|没有返回可用结果|没有找到可靠结果|无可靠结果|结果质量过低|搜索页结果抽取失败|API 搜索请求失败|API 搜索没有返回可靠来源|API 联网搜索不可用|轻量 HTTP 搜索未拿到可靠结果|web_fetch 失败|web_fetch 未读到可靠正文|请求太频繁|正文过短|正文质量：(?:short|empty|garbage|error|unknown)|非文本页面|拒绝访问|未打开到可用正文|未打开候选网页正文|未读到可用正文|不能作为事实依据)/i;
...
const SECRET_ASSIGNMENT_RE = /\b(?:api[_-]?key|secret[_-]?key|access[_-]?token|refresh[_-]?token|admin[_-]?token|token|password|passwd|pwd|authorization|cookie|set-cookie)\b\s*[:=：]\s*['"]?[^'"\s,;，；]+/ig;
const BEARER_RE = /\bBearer\s+[A-Za-z0-9._~+/=-]{8,}/ig;
const KEY_PREFIX_RE = /\b(?:sk|tp|ak)-[A-Za-z0-9._~+/=-]{8,}\b/ig;
...
function shouldFilterAgentMaterialLine(line = '') {
    ...
    if (!EXTERNAL_PROMPT_KEYWORD_RE.test(value))
        return false;
    if (/^(?:system prompt|developer message|系统提示|开发者消息)\s*[:：]/i.test(value.trim()))
        return true;
...
function guardAgentRetellReply(reply = '', agentResult = {}, options = {}) {
    ...
    if (!hasSearchFailureMaterial(agentResult))
        return value;
    if (replyAcknowledgesSearchFailure(value))
        return value;
