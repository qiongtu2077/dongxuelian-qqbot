 ...
 * 状态: 无。
 */
const { MAX_OUTPUT_CHARS_FRIENDLY, MAX_OUTPUT_CHARS_YINYANG, MAX_OUTPUT_CHARS_ABUSIVE, POLITICAL_DETECT_FILE, JAILBREAK_OUTPUT_RE, SENSITIVE_KEYWORDS_RE, } = require('../core/constants');
...
const { isJailbreakAttempt, hasBannedOutput, isSemanticProfile, readJsonFile, sanitizeReply, stripMarkdownForQQ, trimReply, } = require('../core/utils');
...
function getAgentOutputGuardReason(text = '') {
    if (hasBannedOutput(text))
        return '包含禁用表达';
    if (isUnsafeThinkingReply(text))
        return '包含内部草稿或工具计划';
    if (hasInternalContextLeak(text))
        return '泄漏内部上下文';
    return '';
}
...
function normalizeAgentRetellText(value) {
...
async function retellAgentResultForChat({ session, ctx, options = {}, agentResultText = '', cleanInput = '', channelKey = '', systemPrompt = '', currentUserMessage = '', userName = '用户', retaliationLevel = 0, callModel, now = new Date(), }) {
    ...
    if (JAILBREAK_OUTPUT_RE.test(agentReply))
        agentReply = await chatJailbreak(session || {}, cleanInput, ctx, { systemPrompt });
    ...
    let agentFinal = trimReply(stripMarkdownForQQ(sanitizeReply(agentReply, userName)), getAgentReplyMaxChars(retaliationLevel));
    ...
    if (isSemanticProfile(agentFinal))
        agentFinal = '别问了，这个我不聊。';
    return agentFinal;
}
module.exports = {
