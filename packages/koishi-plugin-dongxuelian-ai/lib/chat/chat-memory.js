...
const { getConversationHistory, writeMemory, deleteMemory, clearGroupMemory, checkMemoryTimerExpired, readMemoryTimer, } = require('../conversation');
const { writeJsonFile, safeChannelKey } = require('../core/utils');
const lastMemoryPromptTs = new Map();
let lastCleanupTs = 0;
function trimChatMemoryRuntime(now = Date.now()) {
    if (now - lastCleanupTs < 300000)
        return;
    lastCleanupTs = now;
    for (const [key, ts] of lastMemoryPromptTs.entries()) {
        if (now - ts > 300000)
            lastMemoryPromptTs.delete(key);
    }
}
...
async function handleDirectMemoryWrite({ cleanInput, currentUserId, channelKey, inGuild }) {
    if (!currentUserId || !inGuild || !/^(?:记住|记下)\s+/.test(cleanInput))
        return null;
...
async function handleMemoryConfirmation({ session, cleanInput, currentUserId, channelKey, inGuild }) {
    ...
    if (/^(?:嗯|好|可以|是|记住|记下|行|对)/.test(cleanInput) && /需要.{0,10}记住/.test(lastReply)) {
        ...
        if (Date.now() - promptTs < 60000) {
            ...
            if (matchResult)
                await writeMemory(currentUserId, '', channelKey, matchResult[1].trim());
        }
    }
    if (/^(?:不是|记错了|没说过|记错|不对)/.test(cleanInput)) {
        ...
        if (recentMemory.length > 0) {
            ...
            for (const item of memoryItems)
                await deleteMemory(currentUserId, channelKey, item.trim());
        }
    }
}
function rememberMemoryPrompt(currentUserId, channelKey, reply) {
    if (!/需要.{0,10}记住/.test(String(reply || '')))
        return;
