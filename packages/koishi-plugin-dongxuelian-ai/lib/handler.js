...
// 找出今天缓存里 @ 过指定用户的消息。
function findMentionedMessages(cache, userId) {
    return cache.messages.filter(m => {
        if (Array.isArray(m.mentionUserIds) && m.mentionUserIds.includes(userId))
            return true;
        if (m.content && extractAtIds(m.content).includes(userId))
            return true;
        return false;
    });
}
...
async function handleOperationalCommandDomain(session, ctx, state) {
    ...
    if (/^(?:东雪莲)?测试开$/.test(plain)) {
        try {
            ...
            ctx.logger('dongxuelian-ai').warn(`test mode enable failed: ${getHandlerErrorMessage(e)}`);
        }
    ...
    if (/^(?:东雪莲)?嘴臭开$/.test(plain)) {
        if (!hasAdminPermission(session))
            return handled('只有管理员能操作这个命令。');
        try {
            require('fs').writeFileSync(HOSTILE_MODE_FILE, 'on');
        }
        catch (e) {
            ctx.logger('dongxuelian-ai').warn(`hostile mode enable failed: ${getHandlerErrorMessage(e)}`);
        }
    ...
    if (/^谁(?:艾特|@)我$/.test(plain)) {
        ...
        if (typeof internalBot?.sendGroupForwardMsg === 'function') {
            try {
                forwardOk = !!(await internalBot.sendGroupForwardMsg.call(internalBot, groupId, nodes));
            }
            catch {
                forwardOk = false;
            }
    ...
    if (voiceRateViewMatch) {
        ...
        if (voiceRateViewMatch[1] && !hasAdminPermission(session))
            return handled('只有bot管理员才能查看指定群语音概率。');
        if (!targetGroup || (!inGuild && !voiceRateViewMatch[1]))
            return handled('请在群里使用，或指定群号：东雪莲群聊语音概率查看 <群号>');
...
async function handleConversationCommandDomain(session, ctx, state) {
    ...
    if (plain === '确认忘记我') {
        ...
        if (!ts || Date.now() - ts > 60000)
            return handled('确认超时，请重新发送「东雪莲忘记我」。');
    ...
    if (plain === '东雪莲清空群记忆') {
        if (!inGuild)
            return handled('这个命令只能在群里用。');
        if (!isGroupAdminOrBotAdmin(session))
            return handled('只有群管理员/群主才能清空群记忆。');
        clearConversationHistory();
    ...
    if (plain.startsWith('东雪莲群记忆定时') && plain !== '东雪莲群记忆定时') {
        if (!isGroupAdminOrBotAdmin(session))
            return handled('只有群管理员/群主才能设置。');
        if (!inGuild)
            return handled('这个命令只能在群里用。');
        const value = plain.slice(8).trim();
        if (value === '关') {
            try {
                await safeUnlink(path.join(DATA_DIR, 'memory-timers', safeChannelKey(channelKey) + '.json'));
            }
            catch { /* non-critical: disabling an absent timer is already the desired state */ }
            return handled('群记忆定时清空已关闭。');
        }
        const hours = parseFloat(value);
        if (!isFinite(hours) || hours < 0.5 || hours > 168)
            return handled('请设置 0.5-168 小时。例如：东雪莲群记忆定时 3');
        ...
        try {
            require('fs').mkdirSync(path.join(DATA_DIR, 'memory-timers'), { recursive: true });
        }
        catch { /* non-critical: writeJsonFile will report the actual failure if mkdir did not succeed */ }
    ...
    if (plain.startsWith('东雪莲人格切换 ') && plain.length > 7) {
        ...
        if (!found)
            return handled(`未找到人格"${targetName}"。可用：${personas.map(p => p.name).join('、')}`);
    ...
    if (plain === '东雪莲群人格重置') {
        if (!isGroupAdminOrBotAdmin(session))
            return handled('只有群管理员/群主才能重置群级人格。');
        if (!inGuild)
            return handled('群级人格重置只能在群里用。');
        resetGroupPersona(channelKey);
        return handled('已重置群级人格。所有未切换个人人格的用户将使用默认东雪莲。');
    }
    if (plain === '东雪莲复读开') {
        if (!hasAdminPermission(session))
            return handled('只有管理员才能开启复读。');
        if (!inGuild)
            return handled('复读开关只能在群里用。');
        setRepeatEnabled(channelKey, true);
        return handled('本群连续复读已开启。');
    }
    if (plain === '东雪莲复读关') {
        if (!hasAdminPermission(session))
            return handled('只有管理员才能关闭复读。');
        if (!inGuild)
            return handled('复读开关只能在群里用。');
...
async function handleCommand(session, ctx, state) {
    ...
    if (plain === 'AI诊断') {
        if (!hasAdminPermission(session))
            return handled('只有 bot 管理员才能使用 AI 诊断。');
    ...
    if (agentManagementResult.matched)
        return agentManagementResult;
    const planResult = await handlePlanCommand(session, ctx, state);
    if (planResult.matched)
        return planResult;
