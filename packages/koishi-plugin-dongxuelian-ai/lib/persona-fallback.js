 ...
 * 状态: 无。
 */
const { sanitizeReply, stripMarkdownForQQ, trimReply } = require('./utils')
const { isUnsafeThinkingReply, hasInternalContextLeak, stripStickerMarkersForGuard, shouldRetryRepeatedReply } = require('./reply-guard')
const { hasBannedOutput } = require('./utils')

function normalizeModelText(result) {
  if (typeof result === 'string') return result
  if (result && typeof result === 'object') return result.content || result.message?.content || ''
  return ''
}
