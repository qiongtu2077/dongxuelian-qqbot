...
const { checkFile, getExtension, sanitizeFileName, TEXT_EXTENSIONS, BINARY_DOC_EXTENSIONS, wrapFileContent } = require('./file-safety')
...
async function runAnalysis({ channelKey, messageId }) {
  try {
    ...
    console.warn('[file-analyzer] analysis failed:', e.message || e)
    try {
      const entry = await getFileEntry(channelKey, messageId)
      if (entry && !entry.analyzed) {
        await markFileAnalyzed(channelKey, messageId, `[文件解析失败: ${e.message || 'unknown'}]`)
      }
    } catch {}
    return null
  }
