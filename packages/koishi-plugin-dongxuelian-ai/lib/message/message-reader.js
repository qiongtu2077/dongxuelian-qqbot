...
function isReaderRecord(value) {
    return !!value && typeof value === 'object';
}
function asReaderSegment(value) {
...
function sanitizeDisplayName(name = '', sanitizeUserName) {
    if (typeof sanitizeUserName === 'function')
        return sanitizeUserName(String(name || '群友'));
    return normalizeText(name || '群友') || '群友';
}
function getReaderSegmentData(segment = {}) {
    return Object.assign({}, isReaderRecord(segment.attributes) ? segment.attributes : {}, isReaderRecord(segment.attrs) ? segment.attrs : {}, isReaderRecord(segment.data) ? segment.data : {});
}
...
function collectFallbackFeatures(content = '') {
    ...
    if (!value)
        return features;
    if (normalizeText(stripUrls(value)))
        features.hasText = true;
    if (URL_TEST_RE.test(value) || /\bBV[0-9A-Za-z]{10}\b/i.test(value))
        features.hasLink = true;
    if (/\[CQ:(?:json|xml|share),/i.test(value) || /<(?:json|xml)[^>]*>/i.test(value) || /appid=|appId=|miniapp|小程序/i.test(value))
        features.hasEmbed = true;
    if (/\[CQ:(?:forward|longmsg),/i.test(value) || /<forward\b[^>]*\/?>/i.test(value) || MESSAGE_RECORD_CUE_RE.test(value))
        features.hasForward = true;
    if (/\[CQ:(?:image|img|mface|face|video),/i.test(value) || /<(?:img|image|video|mface)[^>]*\/?>/i.test(value))
        features.hasVisual = true;
    if (/\[CQ:record,/i.test(value) || /<(?:audio|record)[^>]*\/?>/i.test(value))
        features.hasAudio = true;
    if (/\[CQ:file,/i.test(value) || /<file\b[^>]*\/?>/i.test(value))
        features.hasFile = true;
    return features;
}
// 汇总转发节点文本，尽量把聊天记录转成可读句子。
function summarizeForwardNodes(nodes, depth = 0, sanitizeUserName) {
    if (!Array.isArray(nodes) || depth > MAX_FORWARD_DEPTH)
        return '';
    ...
    const items = nodes
        ...
        if (type === 'forward')
            return summarizeForwardNodes(getSegmentChildren(node), depth + 1, sanitizeUserName);
        if (type !== 'node')
            return '';
...
function extractSegmentText(segments, options = {}) {
    const { includeFace = true, includeForward = true, includeMediaLabel = false, depth = 0, sanitizeUserName, } = options;
    if (!Array.isArray(segments) || depth > 4)
        return '';
    const parts = [];
    for (const item of segments) {
...
function appendReaderSegments(target, segments) {
    ...
    for (const segment of segments) {
        if (segment && !target.includes(segment))
            target.push(segment);
    }
}
