...
const { extractImageRefFromContent, getMessageSegments, normalizeSegmentData, getFileSegmentData, } = require('./message-segment');
const { storeImageUrl } = require('../media/image/image-store');
const { enqueueAnalysis } = require('../media/image/image-analyzer');
...
const { cacheSmallFileBackground } = require('../media/file/incoming-file');
...
async function resolveIncomingAudioPlain({ ctx, session, analyzed, plain, directAt }) {
