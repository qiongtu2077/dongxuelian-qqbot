...
const { CONVERSATIONS_DIR, MEMORY_HISTORY_LIMIT, MAX_HISTORY_MESSAGES, CONVERSATION_EXPIRE_MS, CONVERSATION_SUMMARY_INTERVAL, MAX_REPEAT_CHECK_HISTORY, MAX_CHANNEL_SHARED_MESSAGES, MAX_REPLY_FINGERPRINT_HISTORY, MAX_CHANNEL_PROMPT_MESSAGES, MAX_THREAD_CONTEXT_MESSAGES, MAX_REPLY_CHAIN_DEPTH, GLM_KEY_FILE, DASHSCOPE_KEY_FILE, PROVIDERS, SENSITIVE_CACHE_PREFIX, USER_PROFILE_DIR, TODAY_CACHE_PREFIX, SUMMARY_WHITELIST_FILE, DATA_DIR, } = require('./core/constants');
const { readTextFile, readJsonFile, writeJsonFile, sanitizeUserName, safeChannelKey, todayCst, todayCstMinusDays, formatShanghaiTime24h, normalizeText } = require('./core/utils');
...
function getChatResultContent(resultObj) {
...
async function analyzeChannelSensitive(channelKey) {
    ...
    try {
        ...
        if (/SENSITIVE/i.test(result)) {
            pendingSensitiveAlert.set(channelKey, { flagged: true, ts: Date.now() });
        }
        try {
            require('fs').unlinkSync(file);
        }
        catch { /* non-critical: best-effort sensitive cache cleanup after analysis */
        }
    }
    catch (error) {
