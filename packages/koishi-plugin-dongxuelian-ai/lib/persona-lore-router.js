 ...
 * 状态: 无。
 */
const { LORE_TRIGGER_SET, TERRA_LORE_TRIGGER_SET } = require('./constants')
...
function truncateLoreText(text = '', maxChars = DEFAULT_LORE_MAX_CHARS) {
  ...
  const suffix = '\n...[已按世界观预算截断]'
  return {
    text: source.slice(0, Math.max(0, limit - suffix.length)).trimEnd() + suffix,
    truncated: true,
  }
}
...
function routePersonaLore(options = {}) {
  ...
  for (const entry of entries) {
    ...
    const maxChars = Math.min(entry.maxChars, remaining)
  ...
  return {
    ...
    omitted,
    totalBudget,
    usedChars: included.reduce((sum, item) => sum + item.chars, 0),
