...
function getMemoryCommandErrorMessage(error, fallback) {
...
async function handleMemoryCommand(session, state) {
    ...
    if (memoryRememberMatch) {
        ...
        if (!agentConfig.memory?.enabled)
            return handled('Agent 记忆当前未开启。');
        if (agentConfig.memory?.adminOnly && !hasAdminPermission(session))
            return handled('只有管理员能写入 Agent 长期记忆。');
        try {
    ...
    if (memorySearchMatch) {
        ...
        if (!agentConfig.memory?.enabled)
            return handled('Agent 记忆当前未开启。');
        if (agentConfig.memory?.adminOnly && !hasAdminPermission(session))
            return handled('只有管理员能检索 Agent 长期记忆。');
        try {
    ...
    if (memoryListMatch) {
        ...
        if (!agentConfig.memory?.enabled)
            return handled('Agent 记忆当前未开启。');
        if (agentConfig.memory?.adminOnly && !hasAdminPermission(session))
            return handled('只有管理员能查看 Agent 长期记忆。');
        try {
    ...
    if (memoryForgetMatch) {
        ...
        if (!agentConfig.memory?.enabled)
            return handled('Agent 记忆当前未开启。');
        if (agentConfig.memory?.adminOnly && !hasAdminPermission(session))
            return handled('只有管理员能删除 Agent 长期记忆。');
        try {
