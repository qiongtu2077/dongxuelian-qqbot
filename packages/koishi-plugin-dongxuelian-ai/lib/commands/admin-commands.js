...
const { RANDOM_WHITELIST_FILE, RANDOM_RATE_FILE, USER_BLACKLIST_FILE, VIDEO_BLACKLIST_FILE, SUMMARY_WHITELIST_FILE, EVENT_DUMP_DIR, EVENT_DUMP_ARM_EXPIRE_MS, POLITICAL_HANDLER_DIR, POLITICAL_DETECT_FILE, RANDOM_TRIGGER_RATE_BASE, RANDOM_TRIGGER_RAMP, } = require('../core/constants');
const { readJsonFile, writeJsonFile, formatPercent, hasAdminPermission, safeChannelKey } = require('../core/utils');
const { isAdminUserId } = require('../core/runtime-config');
const { randomWhitelistCache, randomRateCache, getFileFingerprint, getRandomWhitelistStatus, } = require('../behavior/runtime-settings');
const { loadUserBlacklist, setBlacklistFingerprint, } = require('../core/user-blacklist');
const { resetPoliticalDetectCache, clearSensitiveRuntimeState } = require('../behavior/sensitive');
const { DEFAULT_RANDOM_VOICE_RATE, setRandomVoiceRate, resetRandomVoiceRate, } = require('../behavior/random-voice-rate');
async function handleAdminInlineCommands(session, ctx, { plain, inGuild, channelKey, isGroupAdmin, armEventDump, getArmedEventDump, clearArmedEventDump, }) {
    ...
    if (vidBlAddG) {
        if (!inGuild)
            return matched('这个命令只能在群里使用。');
        if (!isGroupAdmin && !hasAdminPermission(session))
            return matched('只有群主、管理员或bot管理员才能操作。');
        const bl = await readJsonFile(VIDEO_BLACKLIST_FILE, { groups: [], users: [] });
        if (!Array.isArray(bl.groups))
            bl.groups = [];
        if (!bl.groups.includes(vidBlAddG[1]))
            bl.groups.push(vidBlAddG[1]);
    ...
    if (vidBlDelG) {
        ...
        const bl = await readJsonFile(VIDEO_BLACKLIST_FILE, { groups: [], users: [] });
        if (Array.isArray(bl.groups))
            bl.groups = bl.groups.filter(g => g !== vidBlDelG[1]);
        await writeJsonFile(VIDEO_BLACKLIST_FILE, bl);
        return matched(`视频解析已移出群黑名单：${vidBlDelG[1]}`);
    }
    if (plain === '视频黑名单查看') {
        const bl = await readJsonFile(VIDEO_BLACKLIST_FILE, { groups: [], users: [] });
        if (Array.isArray(bl.groups) && bl.groups.length)
            return matched(`视频黑名单群：\n${bl.groups.join('\n')}`);
        return matched('视频群黑名单为空。');
    }
    // 敏感话题处理者
    const safeChannelKeyStr = safeChannelKey(channelKey);
    ...
    if (handlerAdd) {
        if (!inGuild)
            return matched('这个命令只能在群里使用。');
        if (!isGroupAdmin && !hasAdminPermission(session))
            return matched('只有群主、管理员或bot管理员才能设置处理者。');
        let list = await readJsonFile(handlerFile, []);
        if (!Array.isArray(list)) {
            await writeJsonFile(handlerFile, [handlerAdd[1]]);
            return matched(`已添加敏感话题处理者：${handlerAdd[1]}`);
        }
        if (!list.includes(handlerAdd[1])) {
            list.push(handlerAdd[1]);
            await writeJsonFile(handlerFile, list);
        }
        return matched(`已添加敏感话题处理者：${handlerAdd[1]}`);
    }
    ...
    if (handlerDel) {
        ...
        let list = await readJsonFile(handlerFile, []);
        if (Array.isArray(list)) {
            list = list.filter(id => id !== handlerDel[1]);
            await writeJsonFile(handlerFile, list);
        }
    ...
    if (plain === '敏感话题检测开') {
        ...
        if (!list.includes(channelKey)) {
            ...
            await writeJsonFile(POLITICAL_DETECT_FILE, list);
        }
        ...
        if (!Array.isArray(sw))
            sw = [];
        if (!sw.includes(channelKey)) {
            sw.push(channelKey);
            await writeJsonFile(SUMMARY_WHITELIST_FILE, sw);
        }
    ...
    if (plain === '敏感话题检测关') {
        ...
        let list = await readJsonFile(POLITICAL_DETECT_FILE, []);
        if (Array.isArray(list)) {
            list = list.filter(k => k !== channelKey);
            await writeJsonFile(POLITICAL_DETECT_FILE, list);
        }
        resetPoliticalDetectCache();
        clearSensitiveRuntimeState(channelKey);
    ...
    if (swAdd) {
        ...
        if (!Array.isArray(sw)) {
            await writeJsonFile(SUMMARY_WHITELIST_FILE, [swAdd[1]]);
            return matched(`已添加解除上限群白名单：${swAdd[1]}`);
        }
        if (!sw.includes(swAdd[1])) {
            sw.push(swAdd[1]);
            await writeJsonFile(SUMMARY_WHITELIST_FILE, sw);
        }
        return matched(`已添加解除上限群白名单：${swAdd[1]}`);
    ...
    if (rateSetMatch) {
        if (!canManageCurrentGroupRandomRate())
            return rejectCurrentGroupRandomRateManage();
        const rate = Number(rateSetMatch[1]) / 100;
        if (!Number.isFinite(rate) || rate < 0 || rate > 1)
            return matched('概率范围只能是 0% 到 100% 之间。');
    ...
    if (voiceRateSetTargetMatch || voiceRateSetCurrentMatch) {
        ...
        if (!targetGroup || (!inGuild && !voiceRateSetTargetMatch))
            return matched('请在群里使用，或指定群号：东雪莲群聊语音概率设置 <群号> 10%');
        if (voiceRateSetTargetMatch && !hasAdminPermission(session))
            return matched('只有bot管理员才能设置指定群语音概率。');
        if (!voiceRateSetTargetMatch && !isGroupAdmin && !hasAdminPermission(session))
            return matched('只有群主、群管理员或bot管理员才能设置语音概率。');
        const rate = Number(voiceRateSetTargetMatch ? voiceRateSetTargetMatch[2] : voiceRateSetCurrentMatch?.[1]) / 100;
        if (!Number.isFinite(rate) || rate < 0 || rate > 1)
            return matched('语音概率范围只能是 0% 到 100% 之间。');
    ...
    if (voiceRateResetMatch) {
        ...
        if (!targetGroup || (!inGuild && !voiceRateResetMatch[1]))
            return matched('请在群里使用，或指定群号：东雪莲群聊语音概率重置 <群号>');
        if (voiceRateResetMatch[1] && !hasAdminPermission(session))
            return matched('只有bot管理员才能重置指定群语音概率。');
        if (!voiceRateResetMatch[1] && !isGroupAdmin && !hasAdminPermission(session))
            return matched('只有群主、群管理员或bot管理员才能重置语音概率。');
