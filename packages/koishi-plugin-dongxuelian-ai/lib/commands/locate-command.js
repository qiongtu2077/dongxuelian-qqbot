 ...
 * ========================================================================== */
const { getLocateSnapshot } = require('../lifecycle/locate-snapshot');
...
async function callWithTimeout(task, timeoutMs) {
    ...
    try {
        ...
        const run = Promise.resolve().then(task).then((value) => ({ ok: true, value }), () => ({ ok: false, reason: 'error' }));
...
function extractReplyTargetId(message) {
...
function normalizeRealSeq(value) {
...
function formatLocateEntry(line, target) {
...
function collectLocateContextEntries(cache, cacheIdx) {
    ...
    for (let i = start; i < end; i += 1)
        entries.push(formatLocateEntry(messages[i] || {}, i === cacheIdx));
    return entries;
}
/**
 ...
 */
function buildLocateContextNodes(cache, cacheIdx, botId) {
...
function buildLocateTargetLine(cache, cacheIdx) {
    const messages = Array.isArray(cache.messages) ? cache.messages : [];
...
function findCacheIndexOfSnapshotEntry(cache, entry) {
    const messages = Array.isArray(cache.messages) ? cache.messages : [];
    if (entry.messageId) {
    ...
    if (entry.ts)
        return messages.findIndex(m => m.ts === entry.ts);
    return -1;
}
/**
 ...
 */
function createLocateTransport(session) {
    const internal = session?.bot?.internal;
    if (!internal)
        return null;
    if (typeof internal.getMsg !== 'function' || typeof internal.sendGroupMsg !== 'function')
        return null;
    ...
    return {
        readMessage(messageId, timeoutMs) {
            return callWithTimeout(async () => {
                const data = toRecord(await getMsg(messageId));
                if (!data)
                    throw new Error('locate get_msg returned no data');
                return data;
            }, timeoutMs);
        },
        sendQuote(groupId, target, timeoutMs) {
            return callWithTimeout(async () => {
        ...
        sendForwardCard(groupId, nodes, timeoutMs) {
            if (!sendGroupForwardMsg)
...
function logLocate(check, level, message) {
    try {
...
async function handleLocateCommand(input) {
    ...
    const finishWithContext = async ({ cacheIdx, reasonText, reason }) => {
        logLocate(ctx, 'warn', `locate degraded: ${context} targetCacheIdx=${cacheIdx} reason=${reason}`);
        try {
            await session.send(reasonText);
        }
        catch { /* non-critical: 文案发送失败仍继续尝试卡片 */ }
        ...
        try {
            await session.send(LOCATE_TEXT_CARD_FAILED);
        }
        catch { /* non-critical: 兜底文案发送失败不再重试 */ }
        try {
            await session.send(buildLocateTargetLine(cache, cacheIdx));
        }
        catch { /* non-critical: 单条兜底发送失败不再重试 */ }
        return handled();
    };
    ...
    if (targetIdx < 0 || targetIdx >= snapshot.length)
