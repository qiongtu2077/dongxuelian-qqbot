...
function classifyAffectMood(input = {}) {
    ...
    if (risk === 'sensitive' || risk === 'refusal' || SENSITIVE_REFUSAL_RE.test(reply) || SENSITIVE_REFUSAL_RE.test(userText))
        return 'refuse';
    if (risk === 'comfort' || COMFORT_RE.test(userText))
        return 'comfort';
    if (risk === 'serious' || SERIOUS_RE.test(userText))
        return 'serious';
    if (PLAYFUL_RE.test(reply) || PLAYFUL_RE.test(userText))
        return 'playful';
    return 'neutral';
}
function buildAffectRouterDiagnostic(input = {}) {
    ...
    if (personaName)
        reasons.push('persona_policy_resolved');
    if (input.randomTriggered)
        reasons.push('random_reply_context');
    if (input.agentRetell)
        reasons.push('agent_retell_context');
