...
function isUsableSource(filePath) {
    try {
...
/** 判断 WAV 缓存是否可直接复用。 */
function isCacheFresh(sourcePath, cachePath) {
    try {
...
function convertMp4ToWav(sourcePath, cachePath) {
...
/** 准备可发送的固定语音 WAV 缓存路径。 */
async function prepareRareVoiceWav() {
    const sourcePath = resolveRareVoiceSource();
    if (!sourcePath)
        return null;
    if (isCacheFresh(sourcePath, RARE_VOICE_WAV_CACHE))
        return RARE_VOICE_WAV_CACHE;
...
async function readRareVoiceAudioBuffer() {
    try {
        const wavPath = await prepareRareVoiceWav();
        if (!wavPath)
            return null;
        const stat = fs.statSync(wavPath);
        if (!stat.isFile() || stat.size <= 44 || stat.size > MAX_RARE_VOICE_WAV_BYTES)
            return null;
