...
const { DATA_DIR, STICKER_DIR } = require('../core/constants');
...
function ignoreStickerShadowRotationFailure(error) {
    void error;
}
...
function stickerShadowFlattenSegments(segments = [], out = [], depth = 0) {
    if (!Array.isArray(segments) || depth > 4)
        return out;
...
function stickerShadowGuessExt(...values) {
    for (const value of values) {
        ...
        if (STICKER_SHADOW_ALLOWED_EXTS.has(ext))
            return ext.slice(1);
    }
    return '';
}
function stickerShadowInferVisual(input = {}) {
    ...
    for (const segment of segments) {
        ...
        if (!['image', 'img', 'mface', 'face', 'video'].includes(type))
            continue;
        const data = stickerShadowSegmentData(segment);
        refs.push({
            type,
    ...
    if (hasMface)
        kind = 'qq_mface';
    else if (hasFace && !hasImage && !hasVideo)
        kind = 'qq_face';
    else if (hasVideo)
        kind = 'video_visual';
    else if (isGif)
...
function formatStickerShadowIngestDiagnostic(plan = {}) {
    const visual = (plan.visual && typeof plan.visual === 'object' ? plan.visual : {});
    const simulated = (plan.simulated && typeof plan.simulated === 'object' ? plan.simulated : {});
...
function stickerShadowDeriveQueryAtoms(replyText = '', affectDiagnostic = {}) {
    ...
    for (const item of STICKER_SHADOW_REPLY_HINTS) {
        if (item.re.test(text))
            atoms.push(...item.tags);
    }
    const mood = stickerShadowNormalizeAtom(affectDiagnostic?.mood || '', 20);
    if (mood && STICKER_SHADOW_MOOD_HINTS[mood])
        atoms.push(...STICKER_SHADOW_MOOD_HINTS[mood]);
...
function formatStickerShadowSendDiagnostic(plan = {}) {
    ...
    const seedIndex = (plan.seedIndex && typeof plan.seedIndex === 'object' ? plan.seedIndex : {});
    const affect = (plan.affect && typeof plan.affect === 'object' ? plan.affect : {});
    const probabilityGate = (plan.probabilityGate && typeof plan.probabilityGate === 'object' ? plan.probabilityGate : null);
    const simulated = (plan.simulated && typeof plan.simulated === 'object' ? plan.simulated : {});
    const reasons = Array.isArray(plan.reasons) ? plan.reasons : [];
    return [
        'sticker_shadow_send',
...
function sanitizeStickerShadowLogPlan(plan = {}) {
    ...
    if (clone.contentSample)
        clone.contentSample = stickerShadowSanitizeSample(clone.contentSample, 180);
    if (clone.replySample)
        clone.replySample = stickerShadowSanitizeSample(clone.replySample, 220);
    if (clone.personaSample)
        clone.personaSample = stickerShadowNormalizeAtom(clone.personaSample, 60);
    if (Array.isArray(clone.markers))
        clone.markers = clone.markers.slice(0, 8);
    if (Array.isArray(clone.queryAtoms))
        clone.queryAtoms = clone.queryAtoms.slice(0, 12);
    if (Array.isArray(clone.candidates))
        clone.candidates = clone.candidates.slice(0, STICKER_SHADOW_MAX_CANDIDATES);
    return clone;
}
