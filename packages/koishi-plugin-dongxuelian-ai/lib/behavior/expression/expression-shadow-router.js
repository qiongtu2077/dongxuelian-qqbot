...
const { loadExpressionPool, EXPRESSION_POOL_MIN_USE_COUNT, EXPRESSION_POOL_STATUS } = require('./expression-pool-store');
const { EXPRESSION_LEARNER_SENSITIVE_TOPIC_KEYWORDS, EXPRESSION_LEARNER_SENSITIVE_TOPIC_WINDOW_MS } = require('./expression-learner');
...
function shadowText(value, maxLen = 80) {
...
function shadowEmptySkipped() {
    const out = {};
    for (const reason of Object.values(EXPRESSION_SHADOW_SKIP_REASONS))
        out[reason] = 0;
    return out;
}
function resolveExpressionInjectionMode(personaName) {
...
function detectExpressionSensitiveTopicActive(messages = [], now = Date.now(), windowMs = EXPRESSION_LEARNER_SENSITIVE_TOPIC_WINDOW_MS) {
...
function shadowSelectPicks(candidates, maxPicks) {
...
function buildExpressionShadowPlan(input = {}, options = {}) {
    ...
    const recentSpeakerSet = new Set(Array.isArray(input.recentSpeakerIds) ? input.recentSpeakerIds.map((id) => String(id || '').trim()).filter(Boolean) : []);
    ...
    for (const entry of entries) {
        ...
        if (recentSpeakerSet.size > 0 && Array.isArray(entry.contributors)) {
            ...
            for (const id of entry.contributors) {
                if (recentSpeakerSet.has(String(id))) {
    ...
    if (!picks.length && !reasons.length)
        reasons.push('all_candidates_filtered');
    if (picks.length)
        reasons.push('would_inject_when_v24');
    return baseDiagnostic;
}
function formatExpressionShadowDiagnostic(diagnostic = {}) {
