 ...
 * 状态: 无。
 */
const { SENSITIVE_KEYWORDS_RE } = require('../../core/constants');
...
const SELF_USER_TOKENS = new Set(['bot', 'self', 'assistant']);
const SENSITIVE_TOPIC_KEYWORDS = Object.freeze(['住院', '去世', '过世', '死了', '分手', '离婚', '抑郁', '自杀', '癌症', '重病', '葬礼']);
...
function expressionLearnerString(value, maxLength = MAX_LEARN_CONTENT_CHARS) {
    return String(value == null ? '' : value).replace(/\s+/g, ' ').trim().slice(0, maxLength);
}
function expressionLearnerScanText(entry) {
    return expressionLearnerString(entry && entry.content, MAX_SCAN_CONTENT_CHARS);
}
function expressionLearnerNumber(value, fallback) {
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : fallback;
}
function expressionLearnerTimestamp(entry) {
    const ts = expressionLearnerNumber(entry && entry.ts, NaN);
    if (Number.isFinite(ts) && ts > 0)
        return ts;
    return NaN;
}
function expressionLearnerBotNamePatterns(botName) {
...
function expressionLearnerHasBotMention(content, botName, botUserIds, mentionUserIds) {
    if (Array.isArray(mentionUserIds) && Array.isArray(botUserIds)) {
        for (const uid of mentionUserIds) {
            if (botUserIds.some((b) => String(b) === String(uid)))
                return true;
        }
    }
    if (!content)
...
function expressionLearnerIsSelfMessage(entry, selfUserIds) {
    const role = String(entry && entry.role || '').toLowerCase();
    if (role === 'assistant' || role === 'bot' || role === 'self')
        return true;
    const rawUserId = String(entry && entry.userId || '');
    if (!rawUserId)
        return false;
    if (SELF_USER_TOKENS.has(rawUserId.toLowerCase()))
        return true;
    if (Array.isArray(selfUserIds)) {
        for (const sid of selfUserIds) {
            if (String(sid) === rawUserId)
                return true;
        }
    }
    return false;
}
function expressionLearnerSortByTime(messages) {
...
function expressionLearnerBuildRepeatSkipSet(indexed, options) {
    ...
    for (let i = 0; i < indexed.length; i += 1) {
        ...
        const speakers = new Set([String(baseEntry.userId || '')]);
        for (let j = i + 1; j < indexed.length; j += 1) {
            ...
            if (nextText !== baseText)
                continue;
            if (baseTs != null && Number.isFinite(indexed[j].ts) && (indexed[j].ts - baseTs) > windowMs)
                break;
            if (baseTs == null && (j - i) > 50)
                break;
        ...
        if (window.length >= minUsers && speakers.size >= minUsers) {
            for (const item of window)
                skipIndexes.add(item.index);
        }
    }
    return skipIndexes;
}
function expressionLearnerBuildSensitiveTopicSkipSet(indexed, options) {
    ...
    for (const item of indexed) {
        for (const trigger of triggers) {
            if (trigger.ts != null && Number.isFinite(item.ts)) {
                if (Math.abs(item.ts - trigger.ts) <= windowMs) {
                    skipIndexes.add(item.index);
                    break;
                }
            }
...
function expressionLearnerEmptySkipped() {
    const skipped = {};
    for (const reason of Object.values(SKIP_REASONS))
        skipped[reason] = 0;
    return skipped;
}
function filterExpressionLearningMessages(messages = [], options = {}) {
    ...
    for (const item of indexed) {
        ...
        if (!entry || typeof entry !== 'object')
            continue;
        if (expressionLearnerIsSelfMessage(entry, selfUserIds)) {
            skipped[SKIP_REASONS.selfBot] += 1;
            continue;
        }
        const rawContent = String(entry.content == null ? '' : entry.content);
        if (IMAGE_OR_EMOJI_RE.test(rawContent)) {
            skipped[SKIP_REASONS.hasImageOrEmoji] += 1;
            continue;
        }
        const scanContent = expressionLearnerString(rawContent, MAX_SCAN_CONTENT_CHARS);
        if (!scanContent) {
            skipped[SKIP_REASONS.emptyText] += 1;
            continue;
        }
        if (expressionLearnerHasBotMention(scanContent, botName, botUserIds, entry.mentionUserIds)) {
            skipped[SKIP_REASONS.mentionsBot] += 1;
            continue;
        }
        if (SENSITIVE_KEYWORDS_RE.test(scanContent)) {
            skipped[SKIP_REASONS.sensitiveKeyword] += 1;
            continue;
        }
        if (repeatSkip.has(item.index)) {
            skipped[SKIP_REASONS.repeatWindow] += 1;
            continue;
        }
        if (sensitiveTopicSkip.has(item.index)) {
            skipped[SKIP_REASONS.sensitiveTopicWindow] += 1;
            continue;
        }
        kept.push({
