...
const { EXPRESSION_POOL_DIR } = require('../../core/constants');
const { writeJsonFile, safeChannelKey, truncateText } = require('../../core/utils');
...
function expressionPoolReadFileSafe(file) {
    try {
        ...
        if (stat.size > EXPRESSION_POOL_MAX_FILE_BYTES) {
            ...
            catch { /* non-critical: oversized expression pool cleanup best effort */ }
            return null;
        }
...
function expressionPoolNormalizeEntry(raw, channelKey) {
    ...
    for (const c of contributorsSource) {
        ...
        if (contributors.length >= EXPRESSION_POOL_MAX_CONTRIBUTORS)
            break;
    }
    return {
        id,
...
function expressionPoolNormalizeFileShape(raw, channelKey) {
    const safeChannelKey = expressionPoolSafeChannelKey(channelKey);
    if (!raw || typeof raw !== 'object')
        return { entries: [], updatedAt: 0, channelKey: safeChannelKey };
    const rawRecord = raw;
...
async function expressionPoolPersist(channelKey, pool) {
    ...
    catch { /* non-critical: writeJsonFile will surface mkdir/write failures */ }
...
function expressionPoolMergeContributors(target, incoming) {
    ...
    for (const id of incoming) {
        ...
        seen.add(value);
        target.push(value);
        if (target.length >= EXPRESSION_POOL_MAX_CONTRIBUTORS)
            break;
    }
}
function expressionPoolEvictWeakest(entries) {
    if (entries.length <= EXPRESSION_POOL_MAX_ENTRIES_PER_CHANNEL)
        return;
    const indexed = entries.map((entry, index) => ({ entry, index }));
    indexed.sort((a, b) => {
        if (a.entry.status === POOL_STATUS.archived && b.entry.status !== POOL_STATUS.archived)
            return -1;
        if (b.entry.status === POOL_STATUS.archived && a.entry.status !== POOL_STATUS.archived)
            return 1;
...
function appendExpressionCandidate(channelKey, candidate, options = {}) {
    ...
    return expressionPoolEnqueueWrite(safeChannelKey, async () => {
        ...
        for (let i = 0; i < entries.length; i += 1) {
            ...
            if (entry.status === POOL_STATUS.archived)
                continue;
        ...
        if (bestIndex >= 0) {
            ...
            if (!created)
                return { mode: APPEND_MODES.rejected, reason: 'normalize_failed', entry: null };
...
function archiveByContributor(channelKey, userId) {
    ...
    return expressionPoolEnqueueWrite(safeChannelKey, async () => {
        ...
        for (const entry of pool.entries) {
            if (entry.status === POOL_STATUS.archived)
                continue;
            if (!entry.contributors.includes(target))
                continue;
