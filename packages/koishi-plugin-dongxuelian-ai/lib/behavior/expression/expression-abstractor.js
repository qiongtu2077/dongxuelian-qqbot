...
const { DATA_DIR, TODAY_CACHE_PREFIX, SUMMARY_WHITELIST_FILE,
  PROVIDERS, GLM_KEY_FILE, DASHSCOPE_KEY_FILE } = require('../../core/constants')
const { readTextFile, readJsonFile } = require('../../core/utils')
...
const lastHarvestAt = new Map()

const ABSTRACTOR_FALLBACK_MODELS = Object.freeze([
...
function abstractorClampText(value, maxLen) {
  return String(value == null ? '' : value).replace(/\s+/g, ' ').trim().slice(0, Math.max(1, maxLen))
}

function abstractorBuildSystemPrompt() {
  return [
    '你是一个群聊语料的"句式骨架"提取器。',
...
function abstractorBuildUserPayload(messages = []) {
  const lines = []
...
function abstractorParseModelOutput(raw) {
  if (raw == null) return []
...
async function abstractorWithTimeout(promise, ms) {
  let timer
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => reject(new Error('abstractor_timeout')), ms)
...
async function abstractorCallModel(messages, options = {}) {
  const cfg = await loadConfig().catch(() => ({}))
  const requestChatCompletions = options.requestChatCompletions
    || require('../../core/api').requestChatCompletions
  ...
  for (const am of fallback) {
    ...
    let apiKey = ''
    try {
      apiKey = am.keyFile ? (await readTextFile(am.keyFile).catch(() => '') || cfg.apiKey || '') : (cfg.apiKey || '')
...
async function runExpressionHarvestForChannel(ctx, channelKey, options = {}) {
  ...
  try {
    ...
    let raw = ''
    try {
      raw = await abstractorWithTimeout(Promise.resolve().then(() => callModel(promptMessages, options)), EXPRESSION_ABSTRACTOR_TIMEOUT_MS)
    } catch {
      raw = ''
    }
...
async function abstractorListEligibleChannels() {
  ...
  let files = []
  try { files = await fsp.readdir(DATA_DIR) } catch { files = [] }
  const out = []
  const seen = new Set()
...
async function runExpressionHarvestForAllChannels(ctx, options = {}) {
...
function buildExpressionHarvestDiagnostic(summary = {}) {
  return {
...
function formatExpressionHarvestDiagnostic(diagnostic = {}) {
