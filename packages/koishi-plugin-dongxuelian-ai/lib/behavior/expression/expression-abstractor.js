...
function abstractorBuildUserPayload(messages = []) {
    ...
    for (const entry of messages) {
        if (lines.length >= EXPRESSION_ABSTRACTOR_MAX_INPUT_LINES)
            break;
        const text = String(entry && entry.content ? entry.content : '').replace(/\s+/g, ' ').trim();
        if (!text)
            continue;
        const piece = `- ${text}`;
        if (piece.length > charBudget)
            break;
        lines.push(piece);
...
function abstractorParseModelOutput(raw) {
    ...
    try {
        ...
        catch { /* non-critical: invalid model JSON means no candidates */
            return [];
        }
    }
    ...
    for (const item of parsed) {
        ...
        if (out.length >= EXPRESSION_ABSTRACTOR_MAX_BATCH)
            break;
    }
    return out;
}
function abstractorWithTimeout(promise, ms) {
...
async function runExpressionHarvestForChannel(ctx, channelKey, options = {}) {
    ...
    try {
        ...
        const messages = data && typeof data === 'object' && Array.isArray(data.messages) ? data.messages : [];
        ...
        for (const cand of candidates) {
            try {
                ...
                if (result && result.mode === 'created')
                    summary.created += 1;
                else if (result && result.mode === 'merged')
                    summary.merged += 1;
                else
                    summary.rejected += 1;
            }
...
async function abstractorListEligibleChannels() {
    ...
    catch { /* non-critical: no data dir means no eligible channels */
        files = [];
    }
    const out = [];
    const seen = new Set();
