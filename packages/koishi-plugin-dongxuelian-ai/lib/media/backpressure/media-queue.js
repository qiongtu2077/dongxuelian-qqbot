...
// 返回当天 S6 事件日志路径。
function mediaEventFile(date = new Date()) {
...
function mediaKindDir(kind) {
    if (/voice/i.test(kind))
        return path.join(MEDIA_QUEUE_ROOT, 'voice');
    if (/file/i.test(kind))
        return path.join(MEDIA_QUEUE_ROOT, 'file');
    return path.join(MEDIA_QUEUE_ROOT, 'image');
}
// 将内部媒体任务 kind 归一为资源中心使用的三类队列键。
function mediaKindKey(kind) {
    if (/voice/i.test(kind))
        return 'voice';
    if (/file/i.test(kind))
        return 'file';
...
// 返回媒体队列键对应的既有容量上限。
function mediaQueueLimit(kind) {
    if (kind === 'voice')
        return MAX_VOICE_QUEUE;
    if (kind === 'file')
        return MAX_FILE_QUEUE;
    return MAX_IMAGE_QUEUE;
}
// 返回指定 kind 需要观测的队列目录，用于轻量判断队列是否发生过变化。
function getMediaQueueWatchDirs(kind = '') {
    if (kind)
        return [mediaKindDir(kind)];
    return [
        path.join(MEDIA_QUEUE_ROOT, 'image'),
        path.join(MEDIA_QUEUE_ROOT, 'file'),
        path.join(MEDIA_QUEUE_ROOT, 'voice'),
...
function getPendingMediaProbeCacheKey(kind = '') {
    return kind ? String(kind) : '*';
}
// 读取 deferred 冷却期的轻量短路缓存；目录没变且还没到期时可直接返回空。
function shouldShortCircuitPendingMediaScan(kind = '', now = Date.now()) {
...
function rememberPendingMediaProbe(kind = '', nextScanAtMs = 0) {
    const key = getPendingMediaProbeCacheKey(kind);
...
function invalidatePendingMediaProbeCache() {
    pendingMediaProbeCache.clear();
}
// 读取 cache-index。
function readCacheIndex() {
    return readJsonFile(MEDIA_CACHE_INDEX_FILE, {}) || {};
}
// 写 cache-index。
...
function trimCacheIndex(index) {
    const entries = Object.entries(index || {});
    if (entries.length <= MEDIA_CACHE_INDEX_MAX_ENTRIES)
        return index;
    entries.sort((a, b) => {
        const aUpdated = String(a[1]?.updatedAt || '');
        const bUpdated = String(b[1]?.updatedAt || '');
        return bUpdated.localeCompare(aUpdated);
    });
    return Object.fromEntries(entries.slice(0, MEDIA_CACHE_INDEX_MAX_ENTRIES));
}
// 清理过期 pending 媒体任务。
function cleanupExpiredMediaTasks(kind = '') {
    ensureMediaDirs();
    const now = Date.now();
    let removed = 0;
...
function findExistingMediaTask(hash, kind = '') {
    const files = [
...
function listPendingMediaTasks(kind = '', limit = 500) {
    ensureMediaDirs();
    if (shouldShortCircuitPendingMediaScan(kind))
        return [];
    ...
    const tasks = listJsonFiles(getMediaQueueScanRoot(kind), { recursive: true, maxFiles: Math.max(1, Math.min(20000, Number(limit || 500))) })
        ...
        .filter((task) => task !== null && (!kind || task.kind === kind))
        .filter(task => {
        const deferredUntil = Date.parse(String(task.deferredUntil || task.notBefore || ''));
...
function getFlatMediaTaskFile(root, taskId) {
    return path.join(root, `${sanitizeId(taskId)}.json`);
}
// 原子领取一个媒体任务，rename 成功才算被当前 worker 抢到。
function claimNextMediaTask(workerName = 'media-worker', kind = '') {
    ensureMediaDirs();
...
function requeueMediaTask(task, reason = 'resource_defer', delayMs = DEFAULT_MEDIA_REQUEUE_DELAY_MS) {
    ensureMediaDirs();
...
function completeMediaTask(task, result = {}) {
    ensureMediaDirs();
    const src = getFlatMediaTaskFile(MEDIA_RUNNING_ROOT, task.id);
    const dst = getFlatMediaTaskFile(MEDIA_DONE_ROOT, task.id);
    const next = { ...task, status: 'done', finishedAt: nowIso(), updatedAt: nowIso(), result };
    try {
        fs.renameSync(src, dst);
    }
    catch {
...
function failMediaTask(task, error, reason = 'failed') {
    ensureMediaDirs();
    const src = getFlatMediaTaskFile(MEDIA_RUNNING_ROOT, task.id);
...
function enforceMediaQueueLimit(kind) {
    ...
    for (const item of drop) {
        ...
        try {
            fs.renameSync(item.file, dst);
        }
        catch {
            removePath(item.file);
        }
...
function enqueueMediaTask(input) {
    ensureMediaDirs();
...
function listStoredMediaTasks(root) {
    return listJsonFiles(root, { maxFiles: 20000 })
        .map(readMediaTaskFile)
        .filter((task) => task !== null);
}
// 归一媒体未完成记录的结束原因；旧记录无法判断时明确归入历史未知。
function normalizeMediaFinishReason(task) {
    if (task.finishReason === 'queue_limit')
        return 'queue_limit';
    if (task.finishReason === 'processing_failed')
        return 'processing_failed';
    if (task.finishReason === 'restart_interrupted')
        return 'restart_interrupted';
    return 'legacy_unknown';
}
// 列出资源中心诊断区块需要的未完成媒体任务摘要与已脱敏报错。
function listUnfinishedMediaTasksForDiagnostics() {
    ensureMediaDirs();
...
function buildMediaQueueKindStatus(kind, pending, running, done, cacheValues, now = Date.now()) {
...
function sanitizeRunningMediaTask(task) {
    return {
        id: task.id,
        kind: task.kind,
        status: task.status,
...
function getMediaBackpressureStatus() {
    ensureMediaDirs();
