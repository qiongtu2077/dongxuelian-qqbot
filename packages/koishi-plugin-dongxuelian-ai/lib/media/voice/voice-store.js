...
const { getSafeMediaStorageKey: getSafeKey, getMediaHistoryFilePath } = require('../storage-key');
const VOICE_HISTORY_DIR = path.join(DATA_DIR, 'voice-history');
...
const voiceHistoryCache = new Map();
const voiceStoreQueues = new Map();
// 返回队列 key，保证同一频道写入串行。
function getVoiceQueueKey(channelKey) {
    return getSafeKey(channelKey) || 'unknown';
}
// 忽略前序写入失败，避免队列永久卡死。
function ignoreVoiceStoreQueueFailure() {
    // non-critical: keep per-channel voice store queue alive after previous failure
}
// 串行执行同一频道的 voice-store 操作。
function enqueueVoiceStoreTask(channelKey, task) {
    ...
    const cleanup = current.finally(() => {
        if (voiceStoreQueues.get(key) === cleanup)
            voiceStoreQueues.delete(key);
...
// 归一语音历史 JSON 结构。
function normalizeVoiceHistoryData(data) {
    const voices = {};
...
async function readVoiceHistory(channelKey) {
    ...
    try {
        ...
        if (!stat.isFile() || stat.size > MAX_VOICE_HISTORY_BYTES)
            return { voices: {} };
...
// 写入频道语音历史。
async function writeVoiceHistory(channelKey, data) {
    try {
        await fs.mkdir(VOICE_HISTORY_DIR, { recursive: true });
...
// 保存入站语音元信息；不执行 ASR。
async function storeVoice(channelKey, messageId, meta = {}) {
...
// 读取单条语音记录。
async function getVoiceEntry(channelKey, messageId) {
    if (!channelKey || !messageId)
        return null;
    return enqueueVoiceStoreTask(channelKey, async () => {
        const data = cleanExpiredVoices(await readVoiceHistory(channelKey));
...
// 读取已缓存的转写文本。
async function getCachedTranscript(channelKey, messageId) {
...
// 标记语音已转写。
async function markVoiceTranscribed(channelKey, messageId, transcript) {
    if (!channelKey || !messageId)
        return false;
    return enqueueVoiceStoreTask(channelKey, async () => {
...
// 标记语音转写不可用或失败。
async function markVoiceTranscriptionUnavailable(channelKey, messageId, status = 'unavailable') {
    if (!channelKey || !messageId)
        return false;
    return enqueueVoiceStoreTask(channelKey, async () => {
        const data = await readVoiceHistory(channelKey);
