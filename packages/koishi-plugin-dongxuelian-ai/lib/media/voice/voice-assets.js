...
const { VOICES_DIR, VOICE_ASSETS_FILE } = require('../../core/constants');
...
function writeStoreAssets(assets) {
    ...
    try {
        ...
        try {
            fs.unlinkSync(VOICE_ASSETS_FILE);
        }
        catch { /* non-critical: best-effort overwrite cleanup before voice store rename */
        }
        fs.renameSync(tmp, VOICE_ASSETS_FILE);
    }
}
...
function findVoiceAsset(assetIdOrName, personaConfigs = []) {
    ...
    return listVoiceAssets(personaConfigs).find(asset => asset.id === raw ||
        asset.id === safe ||
        asset.filename === raw ||
        asset.personaName === raw ||
        path.basename(asset.filename, path.extname(asset.filename)) === safe) || null;
}
function upsertVoiceAsset(meta = {}) {
    const filename = sanitizeVoiceFilename(meta.filename);
    const fileInfo = getVoiceFileInfo(filename);
    if (!filename || !fileInfo || fileInfo.missing)
        throw new Error('音色样本文件不存在');
...
function listVoiceAssetReferences(assetOrId, personaConfigs = []) {
    ...
    return personaConfigs.filter(config => {
        if (!config || config.voice !== '__cloned__')
            return false;
...
function deleteVoiceAsset(assetIdOrName, personaConfigs = []) {
    ...
    if (filePath) {
        ...
        catch { /* non-critical: metadata deletion still proceeds if sample is already gone */
        }
    }
    const existing = readStoreAssets().map(item => normalizeVoiceAsset(item));
...
function resolveVoiceSampleFile(personaName, voiceAssetId = '') {
    ...
    if (!info || info.missing)
        return null;
    if (info.size < MIN_VOICE_SAMPLE_BYTES || info.size > MAX_VOICE_SAMPLE_BYTES)
        return null;
    return { ...asset, ...info };
}
module.exports = {
