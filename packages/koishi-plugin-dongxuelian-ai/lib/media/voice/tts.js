...
function recordTtsFailure(options, code, message, details = {}) {
    ...
    if (diagnostics && typeof diagnostics === 'object')
        diagnostics.lastError = failure;
    if (typeof options?.onDiagnostic === 'function') {
        try {
            options.onDiagnostic(failure);
        }
        catch { /* non-critical: diagnostic callback must not break TTS fallback */
        }
    }
    ...
    if (logger && typeof logger.warn === 'function') {
        ...
        try {
            logger.warn(`tts failed: ${fields}`);
        }
        catch { /* non-critical: logging failure should not break TTS fallback */
        }
    }
    return null;
}
function decodeTtsAudioData(audioData) {
    ...
    if (uriMatch) {
        ...
        if (!uriMatch[2])
            return { error: 'audio data URI is not base64 encoded', declaredMime };
        base64 = uriMatch[3] || '';
    }
    const normalized = String(base64)
        ...
        .replace(/_/g, '/');
    if (!normalized)
        return { error: 'empty base64 audio payload', declaredMime };
    ...
    if (!buffer.length)
        return { error: 'decoded audio payload is empty', declaredMime };
...
function hasUsableWavData(buffer) {
    ...
    while (offset + 8 <= buffer.length) {
        ...
        if (chunkId === 'data')
            return chunkSize > 0 && dataEnd <= buffer.length;
...
function detectAudioMime(buffer) {
    if (!Buffer.isBuffer(buffer) || buffer.length < 4)
        return '';
    if (hasUsableWavData(buffer))
        return 'audio/wav';
    if (buffer.length > 16 && buffer.toString('ascii', 0, 4) === 'OggS')
        return 'audio/ogg';
    if (buffer.length > 16 && buffer.toString('ascii', 0, 4) === 'fLaC')
        return 'audio/flac';
    if (buffer.length > 16 && buffer.toString('ascii', 4, 8) === 'ftyp')
        return 'audio/mp4';
    if (buffer.length > 32 && buffer.toString('ascii', 0, 3) === 'ID3')
        return 'audio/mpeg';
    if (buffer.length > 32 && buffer[0] === 0xff && (buffer[1] & 0xe0) === 0xe0)
        return 'audio/mpeg';
    return '';
}
...
function cleanupOldTtsSendFiles(now = Date.now()) {
    try {
        ...
        for (const name of fs.readdirSync(TTS_TEMP_DIR)) {
            if (!name.startsWith(TTS_SEND_TEMP_PREFIX))
                continue;
            const filePath = path.join(TTS_TEMP_DIR, name);
            const stat = fs.statSync(filePath);
            if (now - stat.mtimeMs > TTS_SEND_FILE_TTL_MS)
...
function scheduleTtsSendFileCleanup(filePath, ttlMs = TTS_SEND_FILE_TTL_MS) {
    const timer = setTimeout(() => {
        ...
        catch { /* non-critical: delayed TTS temp cleanup is best-effort */
        }
...
async function sendVoiceMessage(session, audioBuf, options = {}) {
    ...
    try {
        ...
        if (tempFile) {
            ...
            catch { /* non-critical: best-effort TTS temp cleanup after send failure */
            }
        }
        return false;
    }
...
function resolvePersonaVoice(personaName, options = {}) {
    const plan = options.plan || resolvePersonaRuntimePlan({ personaName: String(personaName || '') });
    if (!plan || !plan.name)
        return { voice: DEFAULT_VOICE, style: NEUTRAL_TTS_STYLE };
...
function loadClonedVoiceUri(personaName, voiceAssetId = '') {
    try {
        ...
        if (cached && cached.mtime === sample.mtime)
            return cached.uri;
...
function markChannelCooldown(channelKey) {
    ...
    if (channelCooldowns.size > 200) {
        ...
        for (const [k, v] of channelCooldowns) {
            if (now - v > CHANNEL_COOLDOWN_MS)
                channelCooldowns.delete(k);
        }
    }
}
function shouldTriggerRandomVoice(channelKey, randomFn = Math.random) {
    if (isChannelOnCooldown(channelKey))
        return false;
