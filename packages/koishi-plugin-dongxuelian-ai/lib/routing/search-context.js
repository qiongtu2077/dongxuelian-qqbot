...
function hasConcreteSearchSubject(text = '') {
    ...
    if (value.length < 4)
        return false;
    if (looksLikeActionOnlyFollowUp(value))
        return false;
    if (/^(?:这个|那个|这些|那些|刚才|之前|上次|还有|再来|找找|搜搜|看看|怎么说|真的吗|为什么|为啥|然后呢)[？?。.!！]*$/.test(value))
        return false;
...
function isPotentialSearchFollowUp(text = '') {
    ...
    if (looksLikeActionOnlyFollowUp(value))
        return true;
    if (/^(?:那|这个|那个|这些|那些|还有|再|继续|换|顺手|具体|详细)?[^，。！？!?]{0,12}(?:今天|明天|后天|周末|下周|未来(?:\d+|[一二三四五六七两])天)[^，。！？!?]{0,8}[？?。.!！]*$/.test(value))
        return true;
    if (hasConcreteSearchSubject(value))
        return false;
...
function classifyPrivateAge(ts, now) {
    if (!ts)
        return 'none';
    const age = now - ts;
    if (age <= PRIVATE_ACTIVE_HOT_MS)
        return 'hot';
    if (age <= PRIVATE_ACTIVE_WARM_MS)
        return 'warm';
    if (age <= PRIVATE_ACTIVE_COLD_MS)
        return 'cold';
    return 'expired';
}
function buildPrivateSearchContext(session, history = [], options = {}) {
    ...
    for (let index = userMessages.length - 1; index >= 0 && hints.length < 4; index -= 1) {
        ...
        if (!hasConcreteSearchSubject(item.text) || metaTurn)
            continue;
        hints.unshift({
