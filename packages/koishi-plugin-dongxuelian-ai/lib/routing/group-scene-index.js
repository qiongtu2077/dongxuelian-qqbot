...
const { normalizeText, sanitizeUserName, safeChannelKey, formatShanghaiTime24h } = require('../core/utils');
...
function ignoreSceneQueueFailure(error) {
    void error;
}
...
function enqueueSceneTask(channelKey, task) {
    ...
    const cleanup = current.finally(() => {
        if (sceneQueues.get(key) === cleanup)
            sceneQueues.delete(key);
...
async function waitForSceneTasks(channelKey) {
    ...
    if (pending)
        await pending.catch(ignoreSceneQueueFailure);
}
...
function extractSceneAnchors(content = '') {
    ...
    const anchors = [];
    if (/\[图片/.test(text))
        anchors.push({ type: 'image', label: '图片' });
    if (/\[语音/.test(text))
        anchors.push({ type: 'voice', label: '语音' });
    if (/\[转发|合并转发/.test(text))
...
function extractSceneKeywords(content = '') {
    ...
    for (const m of text.matchAll(/[A-Za-z0-9_.-]+\.(?:txt|md|json|pdf|docx?|xlsx?|pptx?|zip|rar|png|jpe?g|gif|mp3|wav|m4a)/gi)) {
        if (m[0])
            found.add(m[0].slice(0, 40));
    }
    const chinese = text.match(/[一-龥]{2,8}/g) || [];
    for (const word of chinese.slice(0, 8))
        found.add(word);
...
async function loadGroupScenes(channelKey) {
    ...
    try {
        ...
        if (!stat.isFile() || stat.size > GROUP_SCENE_MAX_FILE_BYTES)
...
function shouldMergeScene(lastScene, entry) {
    ...
    if (entry.replyToId && sceneContainsMessage(lastScene, entry.replyToId))
        return true;
    if (gap >= 0 && gap <= GROUP_SCENE_HOT_GAP_MS)
        return true;
    if (gap >= 0 && gap <= GROUP_SCENE_COLD_GAP_MS && entry.anchors.length && (lastScene.anchors || []).length)
        return true;
    return false;
}
function appendEntryToScene(scene, entry) {
    scene.endTs = Math.max(Number(scene.endTs || 0), entry.ts);
    if (entry.messageId && !scene.messageIds.includes(entry.messageId))
        scene.messageIds.push(entry.messageId);
    if (!entry.hasMessageRecordCue && !scene.speakers.includes(entry.speakerName))
        scene.speakers.push(entry.speakerName);
    ...
    for (const keyword of entry.keywords) {
        if (keyword && !scene.keywords.includes(keyword))
            scene.keywords.push(keyword);
    }
    if (entry.content && !scene.samples.includes(entry.content))
        scene.samples.push(entry.content);
    scene.snippets.push({
...
async function appendGroupSceneEntry(channelKey, rawEntry = {}) {
    ...
    return enqueueSceneTask(channelKey, async () => {
        ...
        while (scenes.length > GROUP_SCENE_MAX_SCENES)
            scenes.shift();
        data.scenes = scenes;
...
function classifySceneItemsForActive(items = [], options = {}) {
    ...
    if (!list.length)
        return { currentTurn: [], hotContext: [], oldBackground: [] };
    ...
    if (currentMessageId)
        currentTurnIds.add(currentMessageId);
    ...
    for (const item of list) {
        ...
        const inCurrentTurn = ((id && currentTurnIds.has(id)) ||
...
function tokenizeQuery(text = '') {
    ...
    if (value)
        tokens.add(value);
    for (const m of value.matchAll(/[0-9]+(?:\.[0-9]+)?\s*(?:kb|mb|gb|KB|MB|GB|秒|分钟|小时|%|％)?/g))
        tokens.add(m[0].replace(/\s+/g, ''));
    for (const m of value.matchAll(/[A-Za-z0-9_.-]{2,40}/g))
        tokens.add(m[0]);
    for (const m of value.matchAll(/[一-龥]{2,8}/g))
        tokens.add(m[0]);
...
function scoreScene(scene, tokens = [], options = {}) {
    if (!scene)
        return 0;
    if (options.sceneId && String(scene.id) === String(options.sceneId))
        return 10000;
    const haystack = [
        scene.id,
        ...(scene.keywords || []),
    ...
    if (options.anchorType && options.anchorType !== 'any') {
        if ((scene.anchors || []).some(anchor => anchor.type === options.anchorType))
            score += 6;
    }
