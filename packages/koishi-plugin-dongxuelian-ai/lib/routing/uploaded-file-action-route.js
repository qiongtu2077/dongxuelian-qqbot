...
function stripActionNoise(text = '') {
    return String(text || '')
        .replace(/^帮我[，,、\s]*/, '')
        .replace(/[。.!！~～\s]+$/g, '')
        .trim();
...
function parseUploadedFileVariantRequest(text = '') {
    const value = stripActionNoise(text);
    if (!value || !ACTION_RE.test(value))
        return null;
...
function isUploadedFileVariantCapabilityRefusal(reply = '', userText = '') {
    const text = String(reply || '');
    const user = String(userText || '');
    if (!text || !ACTION_RE.test(user))
        return false;
    return REFUSAL_RE.test(text) && /(?:文件|文档|附件|重命名|改名|另存|发送|发给你|发回)/.test(text + user);
}
function formatUploadedFileVariantFallback(result = '') {
    const text = String(result || '');
    if (/已发送文件/.test(text))
        return '已经重命名并发回去了。';
    if (/没有可处理的近期文件|当前会话没有可处理|还没有可用本地副本|可能已过期/.test(text))
        return '没找到最近可处理的文件，重新发一下我再改名发回去。';
    if (/缺少|无法确定发送目标/.test(text))
        return '文件副本建好了，但我没法确定要发到哪里。';
    if (/未发送|不可用|失败/.test(text))
        return '文件副本建好了，但发送失败了。';
    if (/已创建文件副本/.test(text))
        return '已经创建好改名后的文件副本。';
