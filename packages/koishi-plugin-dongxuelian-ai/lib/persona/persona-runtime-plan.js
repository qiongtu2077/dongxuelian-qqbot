 ...
 * 状态: 无。
 */
const { parsePersonaDocument, parsePersonaNumber, parsePersonaStringList, createPersonaDiagnostic } = require('./persona-schema');
...
const LEGACY_PERSONA_WILL = Object.freeze({
    '长离': 0.8,
    '椿': 1.3,
    '特蕾西娅': 0.9,
});
const KNOWN_NSFW_POLICIES = new Set(['none', 'off', 'soft', 'adult', 'strict', 'reply']);
function clampPersonaRuntimeNumber(value, fallback, min, max) {
    const parsed = typeof value === 'number' ? value : parsePersonaNumber(value);
    if (!Number.isFinite(parsed))
        return fallback;
    return Math.max(min, Math.min(max, parsed));
}
function normalizePersonaRuntimeText(value = '', maxLength = 240) {
    return String(value || '')
        .replace(/[\r\n\t]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim()
...
function normalizePersonaRuntimeNsfw(value = '') {
    const text = String(value || '').trim().toLowerCase();
    if (!text)
        return 'none';
    return KNOWN_NSFW_POLICIES.has(text) ? text : 'none';
}
function buildPersonaRuntimeDiagnostics(doc = { meta: {} }, options = {}) {
...
function compilePersonaRuntimePlan(options = {}) {
    ...
    if (primaryLore && primaryLore !== 'none' && !loreRefs.includes(primaryLore))
        loreRefs.unshift(primaryLore);
...
function resolvePersonaRuntimePlan(options = {}) {
...
function getPersonaRuntimePlanLegacySnapshot(plan = {}) {
    return {
