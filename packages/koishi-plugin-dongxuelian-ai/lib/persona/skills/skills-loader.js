...
const { SKILLS_CORE_DIR, SKILLS_MODES_DIR, SKILLS_LORE_DIR, LORE_TRIGGER_SET, TERRA_LORE_TRIGGER_SET, } = require('../../core/constants');
...
function getSkillErrorMessage(error) {
    return error instanceof Error ? error.message : String(error);
}
function warnSkillCacheLoadFailed(scope, error) {
...
async function readSkillTextIfSmall(file) {
    ...
    if (!stat || !stat.isFile() || stat.size > MAX_SKILL_FILE_BYTES)
        return '';
    ...
    async function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                if (isDebugLogEnabled('skills'))
                    console.warn(`[dongxuelian-ai] skill load failed: ${path.basename(fullPath)} ${getSkillErrorMessage(e)}`);
            }
        }
    }
...
async function loadSkillsContentCache() {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (content)
                cache['core:' + name] = stripSkillFrontmatter(content);
        }
    }
    catch (error) {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (content)
                cache['mode:' + name] = stripSkillFrontmatter(content);
        }
    }
    catch (error) {
...
async function refreshSkillsContentCacheIfChanged() {
    ...
    if (fingerprint === skillsContentCacheFingerprint && Object.keys(skillsContentCache).length > 0)
        return false;
