...
let synced = false;
function getSkillSeedErrorMessage(error) {
...
function copyMissingTree(source, target) {
    ...
    catch { /* non-critical: optional packaged seed subdirectory may be absent */
        return 0;
    }
...
function migrateMissingLoreFrontmatter(sourceDir, targetDir) {
    ...
    catch { /* non-critical: lore seed directory may be absent */
        return 0;
    }
    let migrated = 0;
    for (const entry of entries) {
        ...
        if (!frontmatter || hasFrontmatter(targetText))
            continue;
        try {
...
function ensureRuntimeSkillSeeds() {
    if (synced)
        return;
    synced = true;
    if (!fs.existsSync(PACKAGE_SKILLS_SEED_DIR))
        return;
    if (samePath(PACKAGE_SKILLS_SEED_DIR, SKILLS_DIR))
        return;
    for (const part of SKILL_SEED_PARTS) {
        copyMissingTree(path.join(PACKAGE_SKILLS_SEED_DIR, part), path.join(SKILLS_DIR, part));
    }
    migrateMissingLoreFrontmatter(path.join(PACKAGE_SKILLS_SEED_DIR, 'lore'), path.join(SKILLS_DIR, 'lore'));
}
