...
const { SKILLS_CORE_DIR, SKILLS_MODES_DIR, SKILLS_PERSONAS_DIR, SKILLS_LORE_DIR, } = require('../core/constants');
const { parsePersonaDocument, createPersonaDiagnostic, parsePersonaStringList } = require('./persona-schema');
const { resolveVoiceSampleFile } = require('../media/voice/voice-assets');
...
function readPersonaDiagnosticText(file) {
    try {
        ...
        if (!stat.isFile() || stat.size > MAX_PERSONA_DIAGNOSTIC_FILE_BYTES)
            return '';
