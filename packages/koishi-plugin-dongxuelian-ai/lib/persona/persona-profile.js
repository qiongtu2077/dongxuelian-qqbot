...
const { sanitizePersonaProfileKey, safePersonaProfileFile, readLegacyPersonaProfileData, } = require('./persona-profile-sources');
...
function getPersonaProfileShadowTraitsForAnalysis(analysis = {}) {
    ...
    if (length > 0 && length <= 12)
        traits.push('short_bursty');
    if (length >= 80)
        traits.push('long_explainer');
    if (analysis.hasCorrection)
        traits.push('direct_correction');
    if (analysis.hasQuestion)
        traits.push('questioning');
    if (analysis.hasTechnical)
        traits.push('technical_debug');
    if (analysis.isMedia)
        traits.push('media_followup');
    if (analysis.hasMeme)
        traits.push('meme_casual');
    if (analysis.hasCoordination)
        traits.push('coordination');
    if (analysis.isCommandLike)
        traits.push('command_like');
    return traits;
}
...
function summarizePersonaProfileShadowTraits(analyses = []) {
    ...
    for (const item of analyses) {
        ...
        if ((Number(item.length) || 0) <= 12)
            counts.short += 1;
        if ((Number(item.length) || 0) >= 80)
            counts.long += 1;
        if (item.isMedia)
            counts.media += 1;
        if (item.isMention)
            counts.mention += 1;
        if (item.isCommandLike)
            counts.command += 1;
        if (item.isSensitive)
            counts.sensitive += 1;
        if (item.hasTechnical)
            counts.technical += 1;
        ...
        if (item.hasMeme)
            counts.meme += 1;
        if (item.hasCoordination)
            counts.coordination += 1;
    }
    const ratio = (key) => total ? counts[key] / total : 0;
    const traits = [];
    if (total && ratio('short') >= 0.6)
        traits.push('short_bursty');
    if (total && ratio('long') >= 0.3)
        traits.push('long_explainer');
    if (total && ratio('correction') >= 0.25)
        traits.push('direct_correction');
    if (total && ratio('question') >= 0.25)
        traits.push('questioning');
    if (total && ratio('technical') >= 0.25)
        traits.push('technical_debug');
    if (total && ratio('media') >= 0.25)
        traits.push('media_followup');
    if (total && ratio('meme') >= 0.25)
        traits.push('meme_casual');
    if (total && ratio('coordination') >= 0.25)
        traits.push('coordination');
    if (total && ratio('command') >= 0.25)
        traits.push('command_like');
    const blockers = [];
    if (total < 2)
        blockers.push('too_few_candidates');
    if (counts.sensitive > 0)
        blockers.push('sensitive_like_text');
    if (ratio('command') >= 0.4)
        blockers.push('command_like_noise');
    if (ratio('media') >= 0.6)
        blockers.push('media_markup_dominant');
    if (ratio('mention') >= 0.5)
        blockers.push('mention_or_broadcast_dominant');
    if (!traits.length)
        blockers.push('no_stable_trait');
    const warnings = [];
    if (ratio('meme') >= 0.25)
        warnings.push('meme_may_ooc');
    if (ratio('correction') >= 0.25)
        warnings.push('do_not_copy_harshness');
    if (ratio('media') > 0)
        warnings.push('ignore_media_placeholders');
    if (ratio('command') > 0)
        warnings.push('ignore_commands_as_style');
    return {
        total,
...
function buildPersonaProfileShadowPromptPreview(traits = [], warnings = []) {
    ...
    if (has('short_bursty'))
        parts.push('短句直给');
    if (has('long_explainer'))
        parts.push('可承接较长解释');
    if (has('direct_correction'))
        parts.push('对纠错先认问题再回答');
    if (has('questioning'))
        parts.push('优先回答追问');
    if (has('technical_debug'))
        parts.push('技术/测试语境少寒暄');
    if (has('media_followup'))
        parts.push('图片文件追问先说明能否读到');
    if (has('meme_casual'))
        parts.push('可轻接梗但不复读口癖');
    if (has('coordination'))
        parts.push('群活动消息简短确认');
    if (has('command_like'))
        parts.push('命令内容只作意图不学语气');
    if (!parts.length)
        parts.push('暂无稳定风格');
    if (warnings.includes('do_not_copy_harshness'))
        parts.push('不模仿攻击性');
...
function getPersonaProfileShadowSelectionBlockers(block = {}, options = {}) {
    ...
    const blockers = [];
    if (!allowedStatuses.has(String(block.status || 'candidate')))
    ...
    if (!includeSensitive && String(block.sensitivity || '') === 'sensitive')
        blockers.push('sensitive');
    const effectiveConfidence = computePersonaProfileEffectiveConfidence(block, { ...options, now });
    if (effectiveConfidence < minEffectiveConfidence)
        blockers.push('lowConfidence');
...
function buildPersonaProfileShadowCandidate(block = {}, options = {}) {
    ...
    if (!selectedRank)
        learningBlockers.push('not_selected_topn');
    if (!isPersonaProfileShadowStyleBlock(normalized))
        learningBlockers.push('not_style_source');
    for (const blocker of selectionState.blockers)
        learningBlockers.push(`selection_${blocker}`);
    if (analysis.isSensitive)
        learningBlockers.push('sensitive_like_text');
    if (analysis.isCommandLike)
        learningBlockers.push('command_like_noise');
    if (analysis.isMedia)
        learningBlockers.push('media_markup');
    if (analysis.isMention)
        learningBlockers.push('mention_or_broadcast');
...
function reinforcePersonaProfileBlock(existing = {}, incoming = {}, options = {}) {
    ...
    if (!reason)
        return { matched: false, reason: 'no_match', block: existingBlock };
...
function computePersonaProfileEffectiveConfidence(block = {}, options = {}) {
    ...
    if (status === 'disputed' || status === 'archived')
        return 0;
    const now = normalizePersonaProfileTs(options.now, Date.now());
    const expiresAt = Number(block.expiresAt || 0);
    if (Number.isFinite(expiresAt) && expiresAt > 0 && expiresAt <= now)
...
function selectPersonaProfileBlocksByEffectiveConfidence(blocks = [], options = {}) {
    ...
    for (const raw of Array.isArray(blocks) ? blocks : []) {
        ...
        if (!includeSensitive && block.sensitivity === 'sensitive') {
            skipped.sensitive += 1;
            continue;
        }
        const effectiveConfidence = computePersonaProfileEffectiveConfidence(block, { ...options, now });
...
function buildPersonaProfileShadowPreview(profile = {}, options = {}) {
    ...
    (Array.isArray(selection.selected) ? selection.selected : []).forEach((block, index) => {
        if (block && block.id)
            selectedRanks.set(String(block.id), index + 1);
    });
    const selected = (Array.isArray(selection.selected) ? selection.selected : [])
    ...
    for (const candidate of candidates) {
        ...
        if (!blockersForCandidate.length)
            continue;
        for (const reason of blockersForCandidate)
            skippedLearning[reason] = (skippedLearning[reason] || 0) + 1;
    }
    return {
...
function ignorePersonaProfileRotationFailure(error) {
    void error;
}
