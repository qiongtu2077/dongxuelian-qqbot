...
function createPersonaDiagnostic(level, code, message, details = {}) {
    return { level, code, message, ...details };
}
function parsePersonaNumber(value) {
    if (typeof value === 'number')
        return Number.isFinite(value) ? value : NaN;
    const parsed = Number(String(value ?? '').trim());
    return Number.isFinite(parsed) ? parsed : NaN;
}
function parsePersonaStringList(value) {
...
function getPersonaSchemaKnownFields() {
    return PERSONA_SCHEMA_KNOWN_FIELDS.slice();
}
function validatePersonaMeta(meta = {}, context = {}) {
    const diagnostics = [];
...
function parsePersonaDocument(content = '', context = {}) {
