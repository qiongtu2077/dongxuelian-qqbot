 ...
 */
const fs = require('fs');
const { CONVERSATION_SUMMARY_INTERVAL, MEMORY_HISTORY_LIMIT, SENSITIVE_CACHE_PREFIX, } = require('../core/constants');
...
function formatConversationSummaryMessage(message) {
...
async function runConversationSummaryWorkerTask(task) {
    ...
    if (!key)
        return { mode: 'conversation_summary', summarized: false, reason: 'empty-key' };
    ...
    if (!text.trim())
        return { mode: 'conversation_summary', summarized: false, reason: 'empty-summary-input' };
    ...
    if (!summary)
        return { mode: 'conversation_summary', summarized: false, reason: 'empty-model-result' };
    const freshData = readConversationDisk(key);
    if (!freshData)
        return { mode: 'conversation_summary', summarized: false, reason: 'conversation-disk-missing-after-llm' };
...
// 读取敏感缓存文件，过大时清理并返回空。
function readSensitiveCache(channelKey) {
    ...
    try {
        ...
        if (!stat.isFile() || stat.size > MAX_SENSITIVE_CACHE_FILE_BYTES) {
            ...
            catch { /* non-critical: oversized sensitive cache cleanup */ }
            return null;
        }
...
// 清理已经分析过的敏感缓存文件。
function unlinkSensitiveCache(channelKey) {
    const file = SENSITIVE_CACHE_PREFIX + safeChannelKey(channelKey) + '.json';
    try {
        fs.unlinkSync(file);
    }
    catch { /* non-critical: best-effort sensitive cache cleanup */ }
}
// 构造敏感缓存分析提示词。
function buildSensitiveAnalysisPrompt() {
...
// 使用统一文字能力链判断敏感缓存，失败降级由运行时优先级负责。
async function callSensitiveAnalysisModel(text) {
    const messages = [{ role: 'system', content: buildSensitiveAnalysisPrompt() }, { role: 'user', content: text }];
    try {
        const cfg = await loadConfig();
        const result = await requestChatCompletions(messages, cfg, { max_tokens: 20 });
        return getBackgroundChatResultContent(result);
    }
    catch {
        return '';
    }
...
async function runSensitiveCacheAnalysisWorkerTask(task) {
    const channelKey = String(task?.payload?.channelKey || task?.channelKey || '');
    if (!channelKey)
        return { mode: 'sensitive_cache_analysis', analyzed: false, reason: 'empty-channel-key' };
    ...
    if (sensitive)
        writePendingSensitiveAlert(channelKey, { sourceTaskId: String(task?.id || ''), result: result.slice(0, 80) });
...
async function runBackgroundLlmWorkerTask(task) {
    ...
    if (task?.kind === 'sensitive_cache_analysis')
        return runSensitiveCacheAnalysisWorkerTask(task);
