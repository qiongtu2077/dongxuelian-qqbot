...
function trimProcessMetricsFile(file, cutoff) {
    let lines = [];
    try {
...
function cleanupOldProcessMetricsFiles(now = Date.now()) {
    try {
        ...
        for (const entry of entries) {
            ...
            if (fileDay >= cutoff)
                continue;
            if (trimProcessMetricsFile(file, cutoff))
                changed += 1;
        }
        return changed;
    }
...
function shouldWriteWorkerMemoryAlert(workerName, pid, limitMb, now = Date.now()) {
    ...
    if (now - lastAt < WORKER_MEMORY_ALERT_COOLDOWN_MS)
        return false;
...
// 读取当前进程 RSS，单位 MB。
function readCurrentProcessRssMb() {
    try {
...
function getWorkerRssLimitMb(workerName) {
...
function getUnsafePidReason(pid) {
    if (pid <= 1)
        return 'pid_le_1';
    if (pid === process.pid)
...
function collectLinuxProcessTree(rootPid) {
...
function terminateRecordedProcessPids(options = {}) {
    ...
    for (const rawEvent of events) {
        ...
        if (String(event.taskId || '') !== taskId)
            continue;
        if (!eventNames.has(String(event.event || '')))
            continue;
...
function checkWorkerMemoryLimit(workerName, limitMb) {
    ensureDir(RESOURCE_SYSTEM_ROOT);
