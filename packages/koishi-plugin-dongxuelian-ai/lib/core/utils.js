 ...
 */
const { AT_ID_PATTERN_XML, AT_ID_PATTERN_CQ, RESERVED_PREFIXES, JAILBREAK_INPUT_RE, JAILBREAK_FALLBACK_REPLIES, OVERUSED_REPLY_PATTERNS, BANNED_ACTION_OUTPUT_RE, EVALUATION_REQUEST_RE, RARE_PROVOCATION_RE, WIDE_RARE_PROVOKE_RE, HOSTILE_INPUT_RE, HOSTILE_SINGLE_TOKENS, PROVIDERS, MAX_OUTPUT_CHARS_FRIENDLY, } = require('./constants');
...
function isHostileInput(text = '') {
    ...
    if (!value)
        return false;
    if (HOSTILE_INPUT_RE.test(value))
        return true;
    if (isRareProvocation(value))
        return true;
    if (value.length <= 3 && HOSTILE_SINGLE_TOKENS.has(value.toLowerCase()))
        return true;
    return false;
}
...
function isReservedCommand(plain = '') {
    ...
    if (!value)
        return false;
    if (value.startsWith('昵称') && value !== '昵称')
        return true;
    if (/^at\s*\S+/i.test(value))
        return true;
...
async function readTextFile(file, options = {}) {
    ...
    catch { /* non-critical: optional text file read falls back to empty content */
        return '';
    }
}
    ...
    try {
        ...
        for (let attempt = 0; attempt < 8; attempt += 1) {
            try {
                ...
                catch { /* non-critical: best-effort overwrite cleanup before async rename retry */ }
        ...
        catch { /* non-critical: best-effort temp cleanup after async write failure */ }
        throw error;
    }
}
async function writeTextFile(file, value) { await writeFileAtomic(file, String(value)); }
function writeFileAtomicSync(file, value) {
    const fs = require('fs');
    const path = require('path');
    const dir = path.dirname(file);
    const tmp = path.join(dir, `.${path.basename(file)}.${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}.tmp`);
    ...
    try {
        ...
        try {
            ...
            catch { /* non-critical: best-effort overwrite cleanup before sync rename retry */ }
            fs.renameSync(tmp, file);
        }
    }
    catch (error) {
        ...
        catch { /* non-critical: best-effort temp cleanup after sync write failure */ }
        throw error;
    }
}
async function readJsonFile(file, fallback, options = {}) {
    ...
    catch { /* non-critical: optional JSON file read falls back to caller default */
        return fallback;
    }
}
async function writeJsonFile(file, value) { await writeFileAtomic(file, JSON.stringify(value, null, 2)); }
function readJsonFileSync(file, fallback, options = {}) {
    try {
        const fs = require('fs');
        const maxBytes = Math.max(1, parseInt(String(options.maxBytes), 10) || MAX_JSON_FILE_BYTES);
    ...
    catch { /* non-critical: optional sync JSON file read falls back to caller default */
        return fallback;
    }
}
function writeJsonFileSync(file, value) { writeFileAtomicSync(file, JSON.stringify(value, null, 2)); }
async function safeUnlink(file) { try {
    await require('fs/promises').unlink(file);
    return true;
}
catch { /* non-critical: safe unlink reports false when target is missing or locked */
    return false;
} }
...
function getBaseHostname(baseURL = '') { try {
    return new URL(String(baseURL || '')).hostname.toLowerCase();
}
catch { /* non-critical: malformed base URL means unknown hostname */
    return '';
} }
...
function normalizeUrl(raw) { if (!raw)
    return ''; let url = String(raw).replace(/&amp;/g, '&'); if (/^https?:\/\//i.test(url))
...
function legacySafeUserId(value = '') {
    return String(value || 'unknown').replace(/[^a-zA-Z0-9_.:-]/g, '_').slice(0, 100) || 'unknown';
}
function truncateTextValue(value, maxLen) {
...
function isReplyTooSimilar(left = '', right = '') {
    if (!left || !right)
        return false;
    const nl = normalizeReplyFingerprint(left);
    const nr = normalizeReplyFingerprint(right);
    if (!nl || !nr)
        return false;
    const lcs = longestCommonSubstringLength(nl, nr, Math.ceil(Math.min(nl.length, nr.length) * 0.85));
    if (lcs >= Math.ceil(Math.min(nl.length, nr.length) * 0.85))
        return true;
    if (charSetJaccardOverlap(nl, nr) > 0.85)
        return true;
    return false;
}
...
function getSearchCapability(config = {}) {
    ...
    if (isDashScopeConfig(config))
        return { supported: true, mode: 'dashscope-chat', label: 'DashScope Chat Completions enable_search' };
    if (isOpenAIOfficialConfig(config)) {
        if (/^(gpt-5-search-api|gpt-4o-search-preview|gpt-4o-mini-search-preview)$/i.test(model))
            return { supported: true, mode: 'openai-chat-search', label: 'OpenAI Chat Completions web_search_options' };
        if (/^gpt-4\.1-nano$/i.test(model))
            return { supported: false, mode: 'openai-unsupported-model', label: 'OpenAI web_search 不支持 gpt-4.1-nano' };
        return { supported: true, mode: 'openai-responses', label: 'OpenAI Responses API web_search' };
    }
    if (/qwen/i.test(model))
        return { supported: true, mode: 'dashscope-chat', label: 'DashScope Chat Completions enable_search (via OpenCode)' };
...
function sanitizeReply(text = '', userName = '') {
    ...
    if (!t)
        t = String(text).replace(/<tool_call>[\s\S]*$/gi, '').trim();
    if (userName) {
...
function calculateWillFactor(channelKey, personaName, channelSharedCache, personaContent) {
    ...
    if (personaContent) {
        ...
        if (willMatch)
            personaFactor = parseFloat(willMatch[1]);
    }
...
function mergeQQBubbles(parts, targetMax = QQ_BUBBLE_HARD_MAX_PARTS) {
    ...
    if (current)
        merged.push(current);
    if (merged.length <= QQ_BUBBLE_HARD_MAX_PARTS)
        return merged;
...
function splitReplyForQQBubbles(text, options = {}) {
    ...
    if (natural.length > 1) {
        for (const paragraph of natural) {
            if (paragraph.length > QQ_BUBBLE_HARD_CHARS)
                pushWrappedBubble(parts, paragraph, softChars);
            else
