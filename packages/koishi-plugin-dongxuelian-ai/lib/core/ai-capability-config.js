...
function normalizeCapabilityModel(value) {
...
function normalizeCapabilityConfig(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        throw new Error('AI 能力配置必须是对象');
    const source = value;
    if (Number(source.version) !== contract.version)
        throw new Error(`不支持的 AI 能力配置版本: ${String(source.version || '')}`);
    ...
    const rawPriorities = source.priorities && typeof source.priorities === 'object' && !Array.isArray(source.priorities)
        ? source.priorities
        : {};
    if (Object.keys(rawProviders).some(key => !isProviderId(key)))
        throw new Error('AI 能力配置包含未知供应商');
    ...
    for (const providerId of PROVIDER_IDS) {
        ...
        if (!Array.isArray(rawModels))
            throw new Error(`${providerId} 模型池必须是数组`);
        const models = rawModels.map(normalizeCapabilityModel).filter((model) => !!model);
        if (models.length !== rawModels.length)
            throw new Error(`${providerId} 模型池包含无效模型`);
        const ids = new Set();
        for (const model of models) {
            if (ids.has(model.id))
                throw new Error(`${providerId} 模型池包含重复模型: ${model.id}`);
            ids.add(model.id);
        }
    ...
    for (const capability of AI_CAPABILITIES) {
        ...
        if (!Array.isArray(chain))
            throw new Error(`${capability} 优先级必须是数组`);
        const seen = new Set();
        result.priorities[capability] = chain.map((rawStep, index) => {
            if (!rawStep || typeof rawStep !== 'object' || Array.isArray(rawStep))
                throw new Error(`${capability} 第 ${index + 1} 步必须是对象`);
            const provider = String(rawStep.provider || '').trim();
            const model = String(rawStep.model || '').trim();
            if (!isProviderId(provider) || !model)
                throw new Error(`${capability} 第 ${index + 1} 步缺少有效供应商或模型`);
            const poolModel = result.providers[provider].models.find(item => item.id === model);
            if (!poolModel)
                throw new Error(`${capability} 第 ${index + 1} 步引用模型池外模型: ${provider}/${model}`);
            if (!poolModel.capabilities.includes(capability))
                throw new Error(`${provider}/${model} 不支持 ${capability}`);
            const key = `${provider}\u0000${model}`;
            if (seen.has(key))
                throw new Error(`${capability} 优先级包含重复模型: ${provider}/${model}`);
            seen.add(key);
...
function normalizeDiscoveredModels(models) {
    if (!Array.isArray(models))
        throw new Error('发现结果必须是模型数组');
    const result = [];
    const seen = new Set();
    for (const raw of models) {
        const model = normalizeCapabilityModel(raw);
        if (!model)
            throw new Error('发现结果包含无效模型');
...
function readJsonFile(filePath, fallback, maxBytes = MAX_CAPABILITY_CONFIG_BYTES) {
    try {
...
function isProviderKeyConfigured(providerId) {
...
function buildLegacyMigration() {
    const raw = readJsonFile(FALLBACK_CHAINS_FILE, {});
...
function loadCapabilityConfigSync() {
    if (!fs.existsSync(CAPABILITY_CONFIG_FILE))
        return buildLegacyMigration();
    const raw = readJsonFile(CAPABILITY_CONFIG_FILE, null);
    if (raw === null)
        throw new Error('AI 能力配置无法读取或超过大小限制');
    return { config: normalizeCapabilityConfig(raw), diagnostics: [], migrated: false };
}
// 序列化前再次执行完整校验，保证事务写入的是规范配置。
function serializeCapabilityConfig(config) {
...
function replaceCapabilityPriority(current, capability, steps) {
    if (!isAiCapability(capability))
        throw new Error('未知能力');
    if (!Array.isArray(steps))
        throw new Error('模型优先级必须是数组');
    const config = normalizeCapabilityConfig(current);
    config.priorities[capability] = steps.map((rawStep, index) => {
        if (!rawStep || typeof rawStep !== 'object' || Array.isArray(rawStep))
...
function resolveCapabilityRuntimeSteps(capability) {
    if (!isAiCapability(capability))
        throw new Error('未知能力');
