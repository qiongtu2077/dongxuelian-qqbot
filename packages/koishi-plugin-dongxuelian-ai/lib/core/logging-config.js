...
function readDebugLogConfig(force = false) {
    const now = Date.now();
    if (!force && cachedConfig && now < nextCheckAt)
        return cachedConfig;
    nextCheckAt = now + CONFIG_CHECK_INTERVAL_MS;
    try {
        const stat = fs.statSync(DEBUG_LOG_CONFIG_FILE);
        if (!stat.isFile() || stat.size > MAX_DEBUG_LOG_CONFIG_BYTES)
            throw new Error('debug log config too large');
        if (!force && cachedConfig && cachedFromFile && stat.mtimeMs === cachedMtimeMs)
            return cachedConfig;
...
async function writeDebugLogConfig(input = {}) {
    ...
    try {
        cachedMtimeMs = fs.statSync(DEBUG_LOG_CONFIG_FILE).mtimeMs;
    }
    catch {
...
function isDebugLogEnabled(moduleName = '') {
    ...
    if (key && Object.prototype.hasOwnProperty.call(config.modules, key))
        return !!config.modules[key];
    return true;
}
function logDebug(ctx, moduleName, message) {
    if (!isDebugLogEnabled(moduleName))
        return;
