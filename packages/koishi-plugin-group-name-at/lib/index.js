...
function isStoreAccessError(error) {
    return !!(error && typeof error === 'object' && error.code === 'GROUP_NAME_AT_STORE_ACCESS');
}
...
async function handleNicknameBlacklistCommand(session, command) {
    ...
    if (!permission.ok)
        return permission.message || TEXT.blacklistPermissionDenied;
...
async function ensureStore() {
    if (storeLoaded) {
        if (storeLoadError)
            throw createStoreAccessError(TEXT.storeReadFailed, storeLoadError);
        return;
    }
    try {
...
function getScopeStore(session) {
    ...
    if (!nicknameStore.scopes[scopeId])
        nicknameStore.scopes[scopeId] = { aliases: {} };
    if (!nicknameStore.scopes[scopeId].aliases)
        nicknameStore.scopes[scopeId].aliases = {};
...
async function addMembers(session, alias, userIds) {
    ...
    for (const userId of userIds) {
        if (entry.members.some((member) => member.userId === String(userId)))
            continue;
...
async function removeAliasBinding(session, alias, targetUserId) {
    ...
    if (entry.members.length === before)
...
async function mergeCollection(session, targetAlias, sourceAlias) {
    ...
    for (const member of source.members) {
        if (target.members.some((item) => item.userId === member.userId))
            continue;
