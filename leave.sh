...
for (const line of text.split(/\r?\n/)) {
  if (/^\s*group-leave-notice/.test(line)) existingCount++
}

if (existingCount === 1) {
  console.log('group-leave-notice already enabled once in koishi.yml, nothing to do')
  process.exit(0)
}

if (existingCount > 1) {
  console.log('found ' + existingCount + ' duplicate group-leave-notice entries, removing duplicates')
  const lines = text.split(/\r?\n/)
  const filtered = []
  let kept = false
  for (const line of lines) {
    if (/^\s*group-leave-notice/.test(line)) {
      if (!kept) { filtered.push(line); kept = true }
    } else {
      filtered.push(line)
    }
  }
  fs.writeFileSync(configFile, filtered.join('\n'), 'utf8')
  console.log('cleaned up duplicate entries, kept only one')
  process.exit(0)
}
