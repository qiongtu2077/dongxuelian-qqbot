# 2026-05-23 服务器搜索与 chat 重构计划废案归档

> 来源：`PLANNNNNNNNNNN/服务器搜索与chat重构计划.md`
> 归档时间：2026-05-23
> 状态：MCP 远程浏览器标记为废案；Chat 轻量工具与图片历史追溯已由后续实现承接。

## 归档结论

MCP 远程浏览器方向不再推进。原因是该方案需要新增服务器 WebSocket 端点、本地 PC 长连服务、token 认证、工具注册与浏览器执行环境，维护面和安全面都偏大；当前搜索/fetch 已转向低成本 HTTP reader、受控 `web_search` / `web_fetch` 协作、Chat heavy handoff 和现有 Agent 工具链，不再为这一路线补实现。

Chat 轻量工具、重型工具转 Agent、图片历史追溯等能力已经在后续代码中以其他文件/模块落地。本文以下内容仅作为历史设计记录保留，不作为后续开发依据。

---

# 原计划记录：MCP 远程浏览器 + Chat 轻量 Agent 能力
